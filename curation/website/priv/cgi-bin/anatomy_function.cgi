...
#
# A bit of explanation text about digits.  2018 08 28
#
# Added to ao_code : Chemogenetics Neuronal_genetics  2020 07 29
#
# get rid of evidence for Remark.  2020 07 30 
#
# use URI::Escape to uri_unescape characters for postgres.  2020 08 18
#
# fromUrlToPostgres updated so that after uri_unescape, if any characters are non-ascii,
# convert them to a '*'  2020 08 19
#
# A lot of data has been copied in postgres to main remark.  Changed &getAceFromHash 
# dumper to no longer dump from original tables into ace hash structure.  2020 11 05
#
# Use ace_dumper.pm to convert any data that is utf-8 into html entities (and back
# convert simple characters defined in the module).  Do not filterAce() since Remark
# is the only field with " and there are no / that need escaping (was simpler to leave
# it the same way).  2021 05 20
...
#   my @row = $result->fetchrow; $theHash{$type}{html_value} = $row[1];
  my @row = $result->fetchrow; $theHash{$type}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[1]);
...
#     $theHash{$g_type}{html_value} = $row[2]; 
    $theHash{$g_type}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[2]);
    my $g_type2 = 'cond_' . $i;
#     $theHash{$g_type2}{html_value} = $row[3];
    $theHash{$g_type2}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[3]); }
...
#   @row = $result->fetchrow; $theHash{$type}{html_value} = $row[1]; 
  @row = $result->fetchrow; $theHash{$type}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[1]);
...
#           $theHash{$g_type_two}{html_value} = $row[3];
          $theHash{$g_type_two}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[3]); } } }
...
#     $theHash{$g_type}{html_value} = $row[2]; 
 ...
 #             $theHash{$g_type_two}{html_value} = $row[4];
             $theHash{$g_type_two}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[4]); } } } } }
...
#       $theHash{$g_type2}{html_value} = $row[4];
      $theHash{$g_type2}{html_value} = &utf8ToHtml($simpleRemapHashRef, $row[4]); } }
...
# 2021 05 20  The form is no longer being used for writing data to postgres, so it does not support utf8 characters, although we could switch it if we ever needed to
sub fromUrlToPostgres {
  my $value = shift;
  $value = uri_unescape($value);                                # all data is getting encodeURIComponent instead of just some characters
  if ($value =~ m/\'/) { $value =~ s/\'/''/g; }                 # escape singlequotes
  if ($value =~ m/([^\x00-\x7F])/g) {
    my $escaped = $value =~ s/(.)(?{ if (ord $1 >127 ){'*'}     # replaced non-ascii with *
                                       else {$1} })/$^R/gr; 
    $value = $escaped; }
  return $value;
} # sub fromUrlToPostgres
...
#     if ($val =~ m/\'/) { $val =~ s/\'/''/g; }
  ...
  for my $i (1 .. $theHash{$type}{html_value}) {
    ...
    if ( ($val1 ne $pg_val1) || ($val2 ne $pg_val2) ) { 
      $val1 = &fromUrlToPostgres($val1);
      $val2 = &fromUrlToPostgres($val2);
#       if ($val1 =~ m/\'/) { $val1 =~ s/\'/''/g; }
#       if ($val2 =~ m/\'/) { $val2 =~ s/\'/''/g; }
...
#       if ($evi_val =~ m/\'/) { evi_$val =~ s/\'/''/g; }
      $evi_val = &fromUrlToPostgres($evi_val);
...
#   @evi = @rem_evi; push @evi, 'none';
  @evi = ('none');					# 2020 07 30 get rid of evidence for Remark
  $type = 'remark';
  for my $i (1 .. $theHash{$type}{html_value}) {
    my $g_type = $type . '_' . $i;
    my $val = $theHash{$g_type}{html_value}; 
...
sub getAceFromHash {		# create and return a .ace file based on data in %theHash
  ...
  for my $i (1 .. $theHash{$type}{html_value}) {
    ...
    if ($theHash{$g_type}{html_value}) {
      $acefile .= "Assay\t\"$theHash{$g_type}{html_value}\"\n";
# Raymond doesn't want Conditions, but still wants Assay.  2020 11 03
#       my $g_type2 = 'cond_' . $i;
...
#         else { $acefile .= "Assay\t\"$theHash{$g_type}{html_value}\"\n"; }
  ...
  if ($theHash{$type}{html_value}) {
    ...
    my $has_data = 0;
    foreach my $anat (@phen_anatfunc) {
     next if ($anat eq 'Remark');							# skip Remark field for Raymond 2020 11 04
      my $g_type_two = $type . '_' . $anat;
      my $g_type_one = $type . '_' . $anat . '_check';
...
#             $acefile .= "Phenotype\t\"$phenotype\"\t$remark{$anat}\t\"$line\"\n";	# skip remark-based tags and text  2020 11 05
  ...
  if ($theHash{$type}{html_value}) {
    ...
    $acefile .= "Gene\t\"$gene\"\n";
# no longer dump gene evidence  2020 11 05
#     my $has_data = 0;
#     foreach my $evi (@gene_evi) {
#       my $g_type_two = $type . '_' . $evi;
#       if ($theHash{$g_type_two}{html_value}) { 
...
# if ($theHash{wbbtf}{html_value} =~ m/0001/) {
# print qq(acefile $tag\t\"$val\"\t$remark{$anat}<br/>);
# }
...
sub printHtmlAssay {
  my @ao_code = qw( Blastomere_isolation Expression_mosaic Genetic_ablation Genetic_mosaic Laser_ablation Optogenetic Chemogenetics Neuronal_genetics );
  my $type = 'assay';
  print "<INPUT TYPE=\"HIDDEN\" NAME=\"html_value_main_$type\" VALUE=\"$theHash{$type}{html_value}\">\n";
  for my $i (1 ..  $theHash{$type}{html_value}) {
...
sub printHtmlRemark {
  ...
  for my $i (1 ..  $theHash{$type}{html_value}) {
    print "<TR><TD ALIGN=\"right\"><STRONG>$theHash{$type}{html_label} $i :</STRONG></TD>";
    my $g_type = $type . '_' . $i;
    my $value = $theHash{$g_type}{html_value};
    if ($value =~ m/\"/) { $value =~ s/"/&quot;/g; }
    print qq(<TD><INPUT NAME="html_value_main_$g_type" VALUE="$value" SIZE=$theHash{$type}{html_size_main}></TD>\n);
# 2020 07 30 get rid of evidence for Remark
#     my @rem_evi = qw( Curator_confirmed Person_evidence );
#     foreach my $evi (@rem_evi) {
#       my $g_type2 = $type . '_' . $evi . '_' . $i;
...
sub printHtmlForm {	# Show the form 
  ...
  &printHtmlInputQuery('wbbtf','20', 'Query WBbtf', ' (enter just digits)');        		# 20 characters
  &printHtmlAssay();
  &printHtmlPhenotype();
  &printHtmlGene();
  &printHtmlInvolved();
...
sub printHtmlInputQuery {		# print html inputs with queries
  my ($type, $size, $message, $additional) = @_;	# get type, use hash for html parts
  ...
  print <<"  EndOfText";
    ...
    <TD ALIGN="left"><INPUT TYPE="submit" NAME="action" VALUE="$message !">$additional</TD></TR>
  EndOfText
