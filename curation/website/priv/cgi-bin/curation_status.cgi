...
#
# Change geneprod and geneint to have different int_type restriction to populate oaData.
# for Chris.  2017 12 16
#
# humandisease in afp cfp is 'humdis'  2018 05 17
#
# exclude _nodump from queries when populating oa data.  2018 06 21
#
# added 3 new datatypes to %datatypesAfpCfp for new afp form 
# rnaseq chemphen envpheno   for Valerio, Daniela, Kimberly. 2018 10 31
#
# added 2 new datatypes to %datatypesAfpCfp for new afp form 
# timeaction siteaction   for Valerio, Daniela, Kimberly. 2018 11 13
#
# skip Retracted_publication, pap_type is 15, for Chris.  2019 12 16
#
# Added cur_nncdata for neural network classification.  2021 01 25
#
# Added new premadeComments for Ranjana.  2021 02 03
#   15 "No disease models"
...
sub printListCurationStatisticsPapers {
  ...
  ($oop, my $displayNnc) = &getHtmlVar($query, "checkbox_nnc");
  if ($displayNnc) { &populateNncData(); push @flaggingMethods, 'checkbox_nnc'; }
  ...
  if      ($method eq 'afpemailed') {		&getCurationStatisticsAfpEmailed(    \@datatypesToShow ); }
    ...
    elsif ($method eq 'nncnotdone') {		&getCurationStatisticsNncNd(         \@datatypesToShow ); }
    elsif ($method =~ m/^nnc/) {		&getCurationStatisticsNnc(           \@datatypesToShow ); }
    elsif ( ($method =~ m/^any/) ||
            ($method =~ m/^int/) ) {		&getCurationStatisticsSvmNd(         \@datatypesToShow);
                                                &getCurationStatisticsSvm(           \@datatypesToShow);
                                                &getCurationStatisticsNncNd(         \@datatypesToShow);
                                                &getCurationStatisticsNnc(           \@datatypesToShow);
...
sub printCurationStatisticsTable {
  ...
  ($oop, my $displayNnc) = &getHtmlVar($query, "checkbox_nnc");                                     
  if ( ($displayAll) || ($displayNnc) ) { &populateNncData();      push @flaggingMethods, 'checkbox_nnc'; }
  ($oop, my $displaySvm) = &getHtmlVar($query, "checkbox_svm");                                     
...
#   tie %{ $curStats{'any'} }, "Tie::IxHash";			# not needed, 'any' only has 'pos'
  ...
  if ( ($displayAll) || ($displayNnc) ) { 
    &getCurationStatisticsNncNd(            \@datatypesToShow );
    &getCurationStatisticsNnc(              \@datatypesToShow ); }
  if ($showTimes) { $end = time; $diff = $end - $start; $start = time; print "getStatsNnc   $diff<br>"; }
...
#     $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncPosTpNC{$datatype} };
#     $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{'ratio'}    = $ratio;
...
sub getCurationStatisticsNncNd {
  my ($datatypesToShow_ref) = @_;
  my @datatypesToShow = @$datatypesToShow_ref;
  my %noNnc; 
  foreach my $datatype (@datatypesToShow) {
    foreach my $joinkey (keys %curatablePapers) { unless ($nncData{$datatype}{$joinkey}) { $noNnc{$datatype}{$joinkey}++; } } } 
  my $countCuratablePapers = scalar keys %curatablePapers;
  $curStats{'dividernnc'}{'allSame'}{'countPap'} = 'blank';
  tie %{ $curStats{'nncnotdone'} }, "Tie::IxHash";
  foreach my $datatype (@datatypesToShow) {
    my $countNncNdPapers = scalar keys %{ $noNnc{$datatype} };
    my $ratio = 0;
    if ($countCuratablePapers > 0) { $ratio = $countNncNdPapers / $countCuratablePapers * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $noNnc{$datatype} }) { $curStats{'nncnotdone'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nncnotdone'}{$datatype}{'countPap'} = scalar keys %{ $noNnc{$datatype} };
    $curStats{'nncnotdone'}{$datatype}{'ratio'}    = $ratio;
  } # foreach my $datatype (@datatypesToShow)
} # sub getCurationStatisticsNncNd
...
sub getCurationStatisticsNnc {
  ...
  foreach my $datatype (@datatypesToShow) {
    my $countYesNncPapers = scalar keys %{ $nncData{$datatype} };
    my $ratio = 0;
    if ($countCuratablePapers > 0) { $ratio = $countYesNncPapers / $countCuratablePapers * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncData{$datatype} }) { $curStats{'nnc'}{$datatype}{papers}{$joinkey}++;
							  $curStats{'any'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{$datatype}{'countPap'} = scalar keys %{ $nncData{$datatype} };
    $curStats{'nnc'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPos = scalar keys %{ $nncPos{$datatype} };
    $ratio = 0;
    if ($countYesNncPapers > 0) { $ratio = $countNncPos / $countYesNncPapers * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPos{$datatype} }) { $curStats{'nnc'}{'pos'}{$datatype}{papers}{$joinkey}++; 
							 $curStats{'any'}{'pos'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{$datatype}{'countPap'} = scalar keys %{ $nncPos{$datatype} };
    $curStats{'nnc'}{'pos'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosVal  = scalar keys %{ $nncPosVal{$datatype} };
    $ratio = 0;
    if ($countNncPos > 0) { $ratio = $countNncPosVal / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosVal{$datatype} }) { $curStats{'nnc'}{'pos'}{'val'}{$datatype}{papers}{$joinkey}++;
							    $curStats{'any'}{'pos'}{'val'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'val'}{$datatype}{'countPap'} = scalar keys %{ $nncPosVal{$datatype} };
    $curStats{'nnc'}{'pos'}{'val'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosTP  = scalar keys %{ $nncPosTP{$datatype} };
    $ratio = 0;
    if ($countNncPosVal > 0) { $ratio = $countNncPosTP / $countNncPosVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosTP{$datatype} }) { $curStats{'nnc'}{'pos'}{'val'}{'tp'}{$datatype}{papers}{$joinkey}++;
 							   $curStats{'any'}{'pos'}{'val'}{'tp'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{$datatype}{'countPap'} = scalar keys %{ $nncPosTP{$datatype} };
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosTpCur  = scalar keys %{ $nncPosTpCur{$datatype} };
    $ratio = 0;
    if ($countNncPosTP > 0) { $ratio = $countNncPosTpCur / $countNncPosTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosTpCur{$datatype} }) { $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++;
							      $curStats{'any'}{'pos'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'cur'}{$datatype}{'countPap'} = scalar keys %{ $nncPosTpCur{$datatype} };
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'cur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosTpNC  = scalar keys %{ $nncPosTpNC{$datatype} };
    $ratio = 0;
    if ($countNncPosTP > 0) { $ratio = $countNncPosTpNC / $countNncPosTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosTpNC{$datatype} }) { $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++;
							     $curStats{'any'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncPosTpNC{$datatype} };
    $curStats{'nnc'}{'pos'}{'val'}{'tp'}{'ncur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosFP  = scalar keys %{ $nncPosFP{$datatype} };
    $ratio = 0;
    if ($countNncPosVal > 0) { $ratio = $countNncPosFP / $countNncPosVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosFP{$datatype} }) { $curStats{'nnc'}{'pos'}{'val'}{'fp'}{$datatype}{papers}{$joinkey}++;
							   $curStats{'any'}{'pos'}{'val'}{'fp'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'val'}{'fp'}{$datatype}{'countPap'} = scalar keys %{ $nncPosFP{$datatype} };
    $curStats{'nnc'}{'pos'}{'val'}{'fp'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosNV  = scalar keys %{ $nncPosNV{$datatype} };
    $ratio = 0;
    if ($countNncPos > 0) { $ratio = $countNncPosNV / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosNV{$datatype} }) { $curStats{'nnc'}{'pos'}{'nval'}{$datatype}{papers}{$joinkey}++;
							   $curStats{'any'}{'pos'}{'nval'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'nval'}{$datatype}{'countPap'} = scalar keys %{ $nncPosNV{$datatype} };
    $curStats{'nnc'}{'pos'}{'nval'}{$datatype}{'ratio'}    = $ratio;

    my $countNncPosNC  = scalar keys %{ $nncPosNC{$datatype} };
    $ratio = 0; 
    if ($countNncPos > 0) { $ratio = $countNncPosNC / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncPosNC{$datatype} }) { $curStats{'nnc'}{'pos'}{'ncur'}{$datatype}{papers}{$joinkey}++; 
							   $curStats{'any'}{'pos'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'pos'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncPosNC{$datatype} };
    $curStats{'nnc'}{'pos'}{'ncur'}{$datatype}{'ratio'}    = $ratio;


    my $countNncHig = scalar keys %{ $nncHig{$datatype} };
    $ratio = 0;
    if ($countNncPos > 0) { $ratio = $countNncHig / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHig{$datatype} }) { $curStats{'nnc'}{'hig'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{$datatype}{'countPap'} = scalar keys %{ $nncHig{$datatype} };
    $curStats{'nnc'}{'hig'}{$datatype}{'ratio'}    = $ratio;

    my $countNncHigVal  = scalar keys %{ $nncHigVal{$datatype} };
    $ratio = 0;
    if ($countNncHig > 0) { $ratio = $countNncHigVal / $countNncHig * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigVal{$datatype} }) { $curStats{'nnc'}{'hig'}{'val'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'val'}{$datatype}{'countPap'} = scalar keys %{ $nncHigVal{$datatype} };
    $curStats{'nnc'}{'hig'}{'val'}{$datatype}{'ratio'}    = $ratio;
    ...
    my $countNncHigTpCur  = scalar keys %{ $nncHigTpCur{$datatype} };
    $ratio = 0;
    if ($countNncHigTP > 0) { $ratio = $countNncHigTpCur / $countNncHigTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigTpCur{$datatype} }) { $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++;
							      $curStats{'any'}{'hig'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'cur'}{$datatype}{'countPap'} = scalar keys %{ $nncHigTpCur{$datatype} };
    $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'cur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncHigTpNC  = scalar keys %{ $nncHigTpNC{$datatype} };
    $ratio = 0;
    if ($countNncHigTP > 0) { $ratio = $countNncHigTpNC / $countNncHigTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigTpNC{$datatype} }) { $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++;
							     $curStats{'any'}{'hig'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncHigTpNC{$datatype} };
    $curStats{'nnc'}{'hig'}{'val'}{'tp'}{'ncur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncHigFP  = scalar keys %{ $nncHigFP{$datatype} };
    $ratio = 0;
    if ($countNncHigVal > 0) { $ratio = $countNncHigFP / $countNncHigVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigFP{$datatype} }) { $curStats{'nnc'}{'hig'}{'val'}{'fp'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'val'}{'fp'}{$datatype}{'countPap'} = scalar keys %{ $nncHigFP{$datatype} };
    $curStats{'nnc'}{'hig'}{'val'}{'fp'}{$datatype}{'ratio'}    = $ratio;

    my $countNncHigNV  = scalar keys %{ $nncHigNV{$datatype} };
    $ratio = 0;
    if ($countNncHig > 0) { $ratio = $countNncHigNV / $countNncHig * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigNV{$datatype} }) { $curStats{'nnc'}{'hig'}{'nval'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'nval'}{$datatype}{'countPap'} = scalar keys %{ $nncHigNV{$datatype} };
    $curStats{'nnc'}{'hig'}{'nval'}{$datatype}{'ratio'}    = $ratio;

    my $countNncHigNC  = scalar keys %{ $nncHigNC{$datatype} };
    $ratio = 0; 
    if ($countNncHig > 0) { $ratio = $countNncHigNC / $countNncHig * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncHigNC{$datatype} }) { $curStats{'nnc'}{'hig'}{'ncur'}{$datatype}{papers}{$joinkey}++; 
							   $curStats{'any'}{'hig'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'hig'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncHigNC{$datatype} };
    $curStats{'nnc'}{'hig'}{'ncur'}{$datatype}{'ratio'}    = $ratio;


    my $countNncMed = scalar keys %{ $nncMed{$datatype} };
    $ratio = 0;
    if ($countNncPos > 0) { $ratio = $countNncMed / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMed{$datatype} }) { $curStats{'nnc'}{'med'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{$datatype}{'countPap'} = scalar keys %{ $nncMed{$datatype} };
    $curStats{'nnc'}{'med'}{$datatype}{'ratio'}    = $ratio;

    my $countNncMedVal  = scalar keys %{ $nncMedVal{$datatype} };
    $ratio = 0;
    if ($countNncMed > 0) { $ratio = $countNncMedVal / $countNncMed * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedVal{$datatype} }) { $curStats{'nnc'}{'med'}{'val'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'val'}{$datatype}{'countPap'} = scalar keys %{ $nncMedVal{$datatype} };
    $curStats{'nnc'}{'med'}{'val'}{$datatype}{'ratio'}    = $ratio;
    ...
    my $countNncMedTpCur  = scalar keys %{ $nncMedTpCur{$datatype} };
    $ratio = 0;
    if ($countNncMedTP > 0) { $ratio = $countNncMedTpCur / $countNncMedTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedTpCur{$datatype} }) { $curStats{'nnc'}{'med'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++;
							      $curStats{'any'}{'med'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'val'}{'tp'}{'cur'}{$datatype}{'countPap'} = scalar keys %{ $nncMedTpCur{$datatype} };
    $curStats{'nnc'}{'med'}{'val'}{'tp'}{'cur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncMedTpNC  = scalar keys %{ $nncMedTpNC{$datatype} };
    $ratio = 0;
    if ($countNncMedTP > 0) { $ratio = $countNncMedTpNC / $countNncMedTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedTpNC{$datatype} }) { $curStats{'nnc'}{'med'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++;
							     $curStats{'any'}{'med'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'val'}{'tp'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncMedTpNC{$datatype} };
    $curStats{'nnc'}{'med'}{'val'}{'tp'}{'ncur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncMedFP  = scalar keys %{ $nncMedFP{$datatype} };
    $ratio = 0;
    if ($countNncMedVal > 0) { $ratio = $countNncMedFP / $countNncMedVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedFP{$datatype} }) { $curStats{'nnc'}{'med'}{'val'}{'fp'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'val'}{'fp'}{$datatype}{'countPap'} = scalar keys %{ $nncMedFP{$datatype} };
    $curStats{'nnc'}{'med'}{'val'}{'fp'}{$datatype}{'ratio'}    = $ratio;

    my $countNncMedNV  = scalar keys %{ $nncMedNV{$datatype} };
    $ratio = 0;
    if ($countNncMed > 0) { $ratio = $countNncMedNV / $countNncMed * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedNV{$datatype} }) { $curStats{'nnc'}{'med'}{'nval'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'nval'}{$datatype}{'countPap'} = scalar keys %{ $nncMedNV{$datatype} };
    $curStats{'nnc'}{'med'}{'nval'}{$datatype}{'ratio'}    = $ratio;

    my $countNncMedNC  = scalar keys %{ $nncMedNC{$datatype} };
    $ratio = 0; 
    if ($countNncMed > 0) { $ratio = $countNncMedNC / $countNncMed * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncMedNC{$datatype} }) { $curStats{'nnc'}{'med'}{'ncur'}{$datatype}{papers}{$joinkey}++; 
							   $curStats{'any'}{'med'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'med'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncMedNC{$datatype} };
    $curStats{'nnc'}{'med'}{'ncur'}{$datatype}{'ratio'}    = $ratio;


    my $countNncLow = scalar keys %{ $nncLow{$datatype} };
    $ratio = 0;
    if ($countNncPos > 0) { $ratio = $countNncLow / $countNncPos * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLow{$datatype} }) { $curStats{'nnc'}{'low'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{$datatype}{'countPap'} = scalar keys %{ $nncLow{$datatype} };
    $curStats{'nnc'}{'low'}{$datatype}{'ratio'}    = $ratio;

    my $countNncLowVal  = scalar keys %{ $nncLowVal{$datatype} };
    $ratio = 0;
    if ($countNncLow > 0) { $ratio = $countNncLowVal / $countNncLow * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowVal{$datatype} }) { $curStats{'nnc'}{'low'}{'val'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'val'}{$datatype}{'countPap'} = scalar keys %{ $nncLowVal{$datatype} };
    $curStats{'nnc'}{'low'}{'val'}{$datatype}{'ratio'}    = $ratio;
    ...
    my $countNncLowTpCur  = scalar keys %{ $nncLowTpCur{$datatype} };
    $ratio = 0;
    if ($countNncLowTP > 0) { $ratio = $countNncLowTpCur / $countNncLowTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowTpCur{$datatype} }) { $curStats{'nnc'}{'low'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++;
							      $curStats{'any'}{'low'}{'val'}{'tp'}{'cur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'val'}{'tp'}{'cur'}{$datatype}{'countPap'} = scalar keys %{ $nncLowTpCur{$datatype} };
    $curStats{'nnc'}{'low'}{'val'}{'tp'}{'cur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncLowTpNC  = scalar keys %{ $nncLowTpNC{$datatype} };
    $ratio = 0;
    if ($countNncLowTP > 0) { $ratio = $countNncLowTpNC / $countNncLowTP * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowTpNC{$datatype} }) { $curStats{'nnc'}{'low'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++;
							     $curStats{'any'}{'low'}{'val'}{'tp'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'val'}{'tp'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncLowTpNC{$datatype} };
    $curStats{'nnc'}{'low'}{'val'}{'tp'}{'ncur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncLowFP  = scalar keys %{ $nncLowFP{$datatype} };
    $ratio = 0;
    if ($countNncLowVal > 0) { $ratio = $countNncLowFP / $countNncLowVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowFP{$datatype} }) { $curStats{'nnc'}{'low'}{'val'}{'fp'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'val'}{'fp'}{$datatype}{'countPap'} = scalar keys %{ $nncLowFP{$datatype} };
    $curStats{'nnc'}{'low'}{'val'}{'fp'}{$datatype}{'ratio'}    = $ratio;

    my $countNncLowNV  = scalar keys %{ $nncLowNV{$datatype} };
    $ratio = 0;
    if ($countNncLow > 0) { $ratio = $countNncLowNV / $countNncLow * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowNV{$datatype} }) { $curStats{'nnc'}{'low'}{'nval'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'nval'}{$datatype}{'countPap'} = scalar keys %{ $nncLowNV{$datatype} };
    $curStats{'nnc'}{'low'}{'nval'}{$datatype}{'ratio'}    = $ratio;

    my $countNncLowNC  = scalar keys %{ $nncLowNC{$datatype} };
    $ratio = 0; 
    if ($countNncLow > 0) { $ratio = $countNncLowNC / $countNncLow * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncLowNC{$datatype} }) { $curStats{'nnc'}{'low'}{'ncur'}{$datatype}{papers}{$joinkey}++; 
							   $curStats{'any'}{'low'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'low'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncLowNC{$datatype} };
    $curStats{'nnc'}{'low'}{'ncur'}{$datatype}{'ratio'}    = $ratio;


    my $countNncNeg = scalar keys %{ $nncNeg{$datatype} };
    $ratio = 0;
    if ($countYesNncPapers > 0) { $ratio = $countNncNeg / $countYesNncPapers * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNeg{$datatype} }) { $curStats{'nnc'}{'neg'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{$datatype}{'countPap'} = scalar keys %{ $nncNeg{$datatype} };
    $curStats{'nnc'}{'neg'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegVal  = scalar keys %{ $nncNegVal{$datatype} };
    $ratio = 0;
    if ($countNncNeg > 0) { $ratio = $countNncNegVal / $countNncNeg * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegVal{$datatype} }) { $curStats{'nnc'}{'neg'}{'val'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'val'}{$datatype}{'countPap'} = scalar keys %{ $nncNegVal{$datatype} };
    $curStats{'nnc'}{'neg'}{'val'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegTN  = scalar keys %{ $nncNegTN{$datatype} };
    $ratio = 0;
    if ($countNncNegVal > 0) { $ratio = $countNncNegTN / $countNncNegVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegTN{$datatype} }) { $curStats{'nnc'}{'neg'}{'val'}{'tn'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'val'}{'tn'}{$datatype}{'countPap'} = scalar keys %{ $nncNegTN{$datatype} };
    $curStats{'nnc'}{'neg'}{'val'}{'tn'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegFN  = scalar keys %{ $nncNegFN{$datatype} };
    $ratio = 0;
    if ($countNncNegVal > 0) { $ratio = $countNncNegFN / $countNncNegVal * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegFN{$datatype} }) { $curStats{'nnc'}{'neg'}{'val'}{'fn'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{$datatype}{'countPap'} = scalar keys %{ $nncNegFN{$datatype} };
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegFnCur  = scalar keys %{ $nncNegFnCur{$datatype} };
    $ratio = 0;
    if ($countNncNegFN > 0) { $ratio = $countNncNegFnCur / $countNncNegFN * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegFnCur{$datatype} }) { $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'cur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'cur'}{$datatype}{'countPap'} = scalar keys %{ $nncNegFnCur{$datatype} };
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'cur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegFnNC  = scalar keys %{ $nncNegFnNC{$datatype} };
    $ratio = 0;
    if ($countNncNegFN > 0) { $ratio = $countNncNegFnNC / $countNncNegFN * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegFnNC{$datatype} }) { $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncNegFnNC{$datatype} };
    $curStats{'nnc'}{'neg'}{'val'}{'fn'}{'ncur'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegNV  = scalar keys %{ $nncNegNV{$datatype} };
    $ratio = 0;
    if ($countNncNeg > 0) { $ratio = $countNncNegNV / $countNncNeg * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegNV{$datatype} }) { $curStats{'nnc'}{'neg'}{'nval'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'nval'}{$datatype}{'countPap'} = scalar keys %{ $nncNegNV{$datatype} };
    $curStats{'nnc'}{'neg'}{'nval'}{$datatype}{'ratio'}    = $ratio;

    my $countNncNegNC  = scalar keys %{ $nncNegNC{$datatype} };
    $ratio = 0; 
    if ($countNncNeg > 0) { $ratio = $countNncNegNC / $countNncNeg * 100; $ratio = FormatSigFigs($ratio, 2); }
    foreach my $joinkey (keys %{ $nncNegNC{$datatype} }) { $curStats{'nnc'}{'neg'}{'ncur'}{$datatype}{papers}{$joinkey}++; }
    $curStats{'nnc'}{'neg'}{'ncur'}{$datatype}{'countPap'} = scalar keys %{ $nncNegNC{$datatype} };
    $curStats{'nnc'}{'neg'}{'ncur'}{$datatype}{'ratio'}    = $ratio;
  } # foreach my $datatype (@datatypesToShow)
} # sub getCurationStatisticsNnc
...
sub printCheckboxesCurationSources {
  ...
  if ($datatypeSource eq 'caltech') {
    ...
    print qq(<input type="checkbox" name="checkbox_nnc"      checked="checked">nnc flags cur_nncdata<br/>\n);
...
#     $result = $dbh->prepare( "SELECT DISTINCT(pro_paper.pro_paper) FROM pro_process, pro_paper, pro_topicpaperstatus WHERE pro_process.joinkey = pro_paper.joinkey AND pro_process.joinkey = pro_topicpaperstatus.joinkey AND (pro_topicpaperstatus.pro_topicpaperstatus = 'unchecked' OR pro_topicpaperstatus.pro_topicpaperstatus = 'relevant') AND pro_process.pro_process = '$topicID' AND pro_process.joinkey NOT IN (SELECT joinkey FROM pro_curationstatusomit)" );	# created a new flag in the topic OA for papers that should be omitted from this filter.  2014 02 06
  ...
  ($oop, my $displayNnc) = &getHtmlVar($query, "checkbox_nnc");   unless ($displayNnc) { $displayNnc = ''; }
  ...
  foreach my $datatype (keys %nncData) { foreach my $joinkey (keys %{ $nncData{$datatype} }) { $allPaperData{$joinkey}{$datatype}++; } }
...
#     foreach my $datatype (sort keys %{ $allPaperData{$joinkey} }) { # } used to default to how only those that have data, now defaults to show all datatypes.  for Chris and Raymond 2014 07 09
    foreach my $datatype (sort keys %datatypes) {
      ...
      if ($displayNnc) {
        my $nncResult = '';
        if ($nncData{$datatype}{$joinkey}) { $nncResult = $nncData{$datatype}{$joinkey}; }
        my $bgcolor = 'white';
        if ($nncResult eq 'HIGH')      { $bgcolor = '#FFA0A0'; }
        elsif ($nncResult eq 'MEDIUM') { $bgcolor = '#FFC8C8'; }
        elsif ($nncResult eq 'LOW')    { $bgcolor = '#FFE0E0'; }
        $nncResult = qq(<span style="background-color: $bgcolor">$nncResult</span>);
        push @dataRow, $nncResult; 
      } # if ($displayNnc)
...
#       $curatorTextareaComment .= qq(<textarea rows="4" cols="80" id="textarea_curator_comment_$trCounter" name="textarea_curator_comment_$trCounter" style="display:none" onblur="document.getElementById('div_curator_comment_$trCounter').style.display = ''; document.getElementById('textarea_curator_comment_$trCounter').style.display = 'none'; document.getElementById('div_curator_comment_$trCounter').innerHTML = document.getElementById('textarea_curator_comment_$trCounter').value.substring(0,20)">$txtcomment</textarea>\n);			# to get the first 20 characters without adding ...
  ...
  if ($displayNnc)    { push @headerRow, "NNC Prediction"; }
...
#     $result = $dbh->prepare( "SELECT * FROM int_name WHERE joinkey IN (SELECT joinkey FROM int_type WHERE int_type IS NOT NULL) AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'Physical') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinProtein') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinDNA') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinRNA') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'Predicted') AND joinkey NOT IN (SELECT joinkey FROM int_nodump)" );
#     $result = $dbh->prepare( "SELECT * FROM int_name WHERE joinkey IN (SELECT joinkey FROM int_type WHERE int_type IS NOT NULL) AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'Physical' OR int_type = 'ProteinProtein' OR int_type = 'ProteinDNA' OR int_type = 'ProteinRNA' OR int_type = 'Predicted') AND joinkey NOT IN (SELECT joinkey FROM int_nodump)" );
#     $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
#     while (my @row = $result->fetchrow) { $objsCurated{'geneint'}{$row[1]}++; }
#     $result = $dbh->prepare( "SELECT * FROM int_paper WHERE joinkey IN (SELECT joinkey FROM int_type WHERE int_type IS NOT NULL) AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'Physical') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinProtein') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinDNA') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'ProteinRNA') AND joinkey NOT IN (SELECT joinkey FROM int_type WHERE int_type = 'Predicted') AND joinkey NOT IN (SELECT joinkey FROM int_nodump)" );
#     $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
#     while (my @row = $result->fetchrow) {
#       my (@papers) = $row[1] =~ m/WBPaper(\d+)/g;
#       foreach my $paper (@papers) {
...
sub populateCurators {
  ...
  $curators{"two40194"}  = 'April Jauhal';
  $curators{"two42118"}  = 'Marie-Claire Harrison';
...
sub populateStatisticsHashToDescription {
  ...
  $statsHashToDescription{"nncnotdone"}          = 'Papers that have not gone through the Neural Network Classification (NNC) pipeline for the indicated data type';
  $statsHashToDescription{"nnc"}                 = 'Papers that have gone through the Neural Network Classification (NNC) pipeline for the indicated data type';
  $statsHashToDescription{"nnc pos"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low)';
  $statsHashToDescription{"nnc pos val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low) and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc pos val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low) and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"nnc pos val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low), have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"nnc pos val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low), have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"nnc pos val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low) and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"nnc pos nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low) and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc pos ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at any confidence level (high, medium, or low) and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"nnc hig"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence';
  $statsHashToDescription{"nnc hig val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc hig val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"nnc hig val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"nnc hig val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"nnc hig val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"nnc hig nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc hig ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at high confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"nnc med"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence';
  $statsHashToDescription{"nnc med val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc med val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"nnc med val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"nnc med val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"nnc med val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"nnc med nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc med ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at medium confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"nnc low"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence';
  $statsHashToDescription{"nnc low val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc low val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"nnc low val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"nnc low val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"nnc low val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"nnc low nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc low ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (NNC) at low confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"nnc neg"}             = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC)';
  $statsHashToDescription{"nnc neg val"}         = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC) and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc neg val tn"}      = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC) and have been confirmed by a curator to lack the data type and, hence, be a true negative';
  $statsHashToDescription{"nnc neg val fn"}      = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC) and have been confirmed by a curator to have the data type and, hence, be a false negative';
  $statsHashToDescription{"nnc neg val fn cur"}  = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC), have been confirmed by a curator to have the data type and, hence, be a false negative, and have been curated';
  $statsHashToDescription{"nnc neg val fn ncur"} = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC), have been confirmed by a curator to have the data type and, hence, be a false negative, and have not yet been curated';
  $statsHashToDescription{"nnc neg nval"}        = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC) and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"nnc neg ncur"}        = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (NNC) and have not been curated (True negatives + Not validated)';

  $statsHashToDescription{"svmnotdone"}          = 'Papers that have not gone through the Neural Network Classification (SVM) pipeline for the indicated data type';
  $statsHashToDescription{"svm"}                 = 'Papers that have gone through the Neural Network Classification (SVM) pipeline for the indicated data type';
  $statsHashToDescription{"svm pos"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low)';
  $statsHashToDescription{"svm pos val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low) and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm pos val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low) and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"svm pos val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low), have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"svm pos val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low), have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"svm pos val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low) and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"svm pos nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low) and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm pos ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at any confidence level (high, medium, or low) and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"svm hig"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence';
  $statsHashToDescription{"svm hig val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm hig val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"svm hig val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"svm hig val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"svm hig val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"svm hig nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm hig ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at high confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"svm med"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence';
  $statsHashToDescription{"svm med val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm med val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"svm med val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"svm med val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"svm med val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"svm med nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm med ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at medium confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"svm low"}             = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence';
  $statsHashToDescription{"svm low val"}         = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm low val tp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence and have been confirmed by a curator to have the data type and, hence, be a true positive';
  $statsHashToDescription{"svm low val tp cur"}  = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have been curated';
  $statsHashToDescription{"svm low val tp ncur"} = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence, have been confirmed by a curator to have the data type and, hence, be a true positive, and have not yet been curated';
  $statsHashToDescription{"svm low val fp"}      = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence and have been confirmed by a curator to lack the data type and, hence, be a false positive';
  $statsHashToDescription{"svm low nval"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm low ncur"}        = 'Papers that have been flagged as positive for the indicated data type by the Neural Network Classification (SVM) at low confidence and have not been curated (True positives, not curated + Not validated)';
  $statsHashToDescription{"svm neg"}             = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM)';
  $statsHashToDescription{"svm neg val"}         = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM) and have been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm neg val tn"}      = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM) and have been confirmed by a curator to lack the data type and, hence, be a true negative';
  $statsHashToDescription{"svm neg val fn"}      = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM) and have been confirmed by a curator to have the data type and, hence, be a false negative';
  $statsHashToDescription{"svm neg val fn cur"}  = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM), have been confirmed by a curator to have the data type and, hence, be a false negative, and have been curated';
  $statsHashToDescription{"svm neg val fn ncur"} = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM), have been confirmed by a curator to have the data type and, hence, be a false negative, and have not yet been curated';
  $statsHashToDescription{"svm neg nval"}        = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM) and have not been confirmed by a curator either to have the indicated data type or not';
  $statsHashToDescription{"svm neg ncur"}        = 'Papers that have been flagged as negative for the indicated data type by the Neural Network Classification (SVM) and have not been curated (True negatives + Not validated)';
...
# TODO not sure these are useful/needed
  ...
  $statsHashToLabel{"nncnotdone"}          = 'NNC no nnc processed';
  $statsHashToLabel{"nnc"}                 = 'NNC has nnc';
  $statsHashToLabel{"nnc neg"}             = 'NNC negative';
  $statsHashToLabel{"nnc neg val"}         = 'NNC negative validated';
  $statsHashToLabel{"nnc neg val tn"}      = 'NNC negative validated true  negative';
  $statsHashToLabel{"nnc neg val fn"}      = 'NNC negative validated false negative';
  $statsHashToLabel{"nnc neg val fn cur"}  = 'NNC negative validated false negative curated';
  $statsHashToLabel{"nnc neg val fn ncur"} = 'NNC negative validated false negative not curated';
  $statsHashToLabel{"nnc neg nval"}        = 'NNC negative not validated';
  $statsHashToLabel{"nnc neg ncur"}        = 'NNC negative not curated';
  $statsHashToLabel{"nnc pos"}             = 'NNC positive any';
  $statsHashToLabel{"nnc pos val"}         = 'NNC positive any validated';
  $statsHashToLabel{"nnc pos val tp"}      = 'NNC positive any validated true  positive';
  $statsHashToLabel{"nnc pos val tp cur"}  = 'NNC positive any validated true  positive curated';
  $statsHashToLabel{"nnc pos val tp ncur"} = 'NNC positive any validated true  positive not curated';
  $statsHashToLabel{"nnc pos val fp"}      = 'NNC positive any validated false positive';
  $statsHashToLabel{"nnc pos nval"}        = 'NNC positive any not validated';
  $statsHashToLabel{"nnc pos ncur"}        = 'NNC positive any not curated';
  $statsHashToLabel{"nnc hig"}             = 'NNC positive high';
  $statsHashToLabel{"nnc hig val"}         = 'NNC positive high validated';
  $statsHashToLabel{"nnc hig val tp"}      = 'NNC positive high validated true  positive';
  $statsHashToLabel{"nnc hig val tp cur"}  = 'NNC positive high validated true  positive curated';
  $statsHashToLabel{"nnc hig val tp ncur"} = 'NNC positive high validated true  positive not curated';
  $statsHashToLabel{"nnc hig val fp"}      = 'NNC positive high validated false positive';
  $statsHashToLabel{"nnc hig nval"}        = 'NNC positive high not validated';
  $statsHashToLabel{"nnc hig ncur"}        = 'NNC positive high not curated';
  $statsHashToLabel{"nnc med"}             = 'NNC positive medium';
  $statsHashToLabel{"nnc med val"}         = 'NNC positive medium validated';
  $statsHashToLabel{"nnc med val tp"}      = 'NNC positive medium validated true  positive';
  $statsHashToLabel{"nnc med val tp cur"}  = 'NNC positive medium validated true  positive curated';
  $statsHashToLabel{"nnc med val tp ncur"} = 'NNC positive medium validated true  positive not curated';
  $statsHashToLabel{"nnc med val fp"}      = 'NNC positive medium validated false positive';
  $statsHashToLabel{"nnc med nval"}        = 'NNC positive medium not validated';
  $statsHashToLabel{"nnc med ncur"}        = 'NNC positive medium not curated';
  $statsHashToLabel{"nnc low"}             = 'NNC positive low';
  $statsHashToLabel{"nnc low val"}         = 'NNC positive low validated';
  $statsHashToLabel{"nnc low val tp"}      = 'NNC positive low validated true  positive';
  $statsHashToLabel{"nnc low val tp cur"}  = 'NNC positive low validated true  positive curated';
  $statsHashToLabel{"nnc low val tp ncur"} = 'NNC positive low validated true  positive not curated';
  $statsHashToLabel{"nnc low val fp"}      = 'NNC positive low validated false positive';
  $statsHashToLabel{"nnc low nval"}        = 'NNC positive low not validated';
  $statsHashToLabel{"nnc low ncur"}        = 'NNC positive low not curated';
