...
//   alert("yes");
  ...
  if (whichPage === 'displayNumber') {
    ...
    if (document.getElementById("species_max_order") !== null) {		// if there are species
      if ( (document.getElementById("species_max_order").value !== undefined) && (document.getElementById("species_max_order").value !== '') ) {
        setSpeciesAutocompleteListeners();
    } }
  }
  ...
  else if (whichPage === 'mergePage') {
    setEnterPaperSpeciesAutocompleteListeners();
  }


}); // YAHOO.util.Event.addListener(window, "load", function()

function setEnterPaperSpeciesAutocompleteListeners() {
  var papersCount = document.getElementById("papersCount").value;
  var curator_id = document.getElementById("curator_id").value;
  for (var i=0; i < papersCount; i++) {
    for (var j=1; j <= 10; j++) {
      var inputId = 'species_' + i + '_' + j;
      settingAutocompleteListeners = function() {
        var url = "paper_editor.cgi?action=autocompleteXHR&field=species&curator_two=" + curator_id + "&";
...
//         forcedOAC.minQueryLength = 0;					// setting this to zero doesn't allow deletions
        ...
        forcedOAC.itemSelectEvent.subscribe(onAutocompleteEnterPaperSpeciesSelect);
...
function onAutocompleteEnterPaperSpeciesSelect(oSelf, elItem, oData) {              // if an autocomplete item is selected
  var match = elItem[0]._sName.match(/species_(.*)_(.*?)$/);                     // get the field
  var count = match[1];
  var order = match[2];
  console.log("count : " + count + " order : " + order);
  var next = parseInt(order) + 1;
  if (next <= 10) {
    var nextElement = 'div_AutoComplete_species_' + count + '_' + next;
    document.getElementById(nextElement).style.display = ''; }
}

function setSpeciesAutocompleteListeners() {		// set species autocompletes
  var arrInputIds = document.getElementById("species_input_ids").value.split(", ");       // split by comma into array
...
} // function setSpeciesAutocompleteListeners()
...
function checkInputEmptyUpdatePg(table, joinkey, order, curator_id) {		// toggle autocomplete to display (and save data + reload page)
  console.log("checkInputEmptyUpdatePg");
  var inputElement = document.getElementById('input_' + table + '_' + joinkey + '_' + order);
  if (inputElement.value === '') { updatePostgresTableField(table, joinkey, order, curator_id, inputElement.value, '', 'reloadtrue'); } 
...
//           if (divElement.innerHTML === '') {				// autocompleteItemSelect picks it up to update if there's a new value, but doesn't if no value was chosen, so if no value chosen (to delete), need to update here onBlur
...
//               Response.Redirect(Request.Url.ToString(), false); 
              if (redirectUrl === undefined) { window.location.reload(false); }			// if undefined value, reload page (default action)
                ...
                else if (redirectUrl === 'reloadtrue') { window.location.reload(true); }	// sometimes want a full refresh
...
function matchGeneTextarea(order, type, datatype) {
    var pap_table = 'gene_';
    if (datatype === 'genecomparator') { pap_table = 'gene_comp_'; }
    var geneTextareaValue = document.getElementById('textarea_' + pap_table + order).value;
    if (type === 'evi') { geneTextareaValue = document.getElementById('textarea_evi_' + pap_table + order).value; }
    ...
    var callbacks = {
        success : function (o) {                                // Successful XHR response handler
            if (o.responseText !== undefined) { 
                var displayElement = document.getElementById('div_' + pap_table + 'display');
                if (type === 'evi') { displayElement = document.getElementById('div_evi_' + pap_table + 'display'); }
...
function mergePaper(joinkey, curator_id) {		// redirect to merge page
  ...
  window.location = 'paper_editor.cgi?curator_id=' + curator_id + '&action=Merge&joinkey=' + joinkey + '&merge_into=' + mergeIntoPaper;
...
function toggleGeneTextareaToDiv(table, joinkey, order, curator_id) {
    ...
    var dataElement = document.getElementById('div_' + table + '_display');
    divElement.style.display = '';
    textareaElement.style.display = 'none';
    if (divElement.innerHTML !== textareaElement.value) {
        divElement.innerHTML = textareaElement.value;
        console.log('updatePostgresTableField(table, joinkey, order, curator_id, dataElement.innerHTML)');
...
function toggleTdToGeneEvi(table, joinkey, order) {
  document.getElementById('td_' + table + '_placeholder').style.display = 'none';
  document.getElementById('td_' + table + '_info').style.display = '';
...
function verifyEviFocusOnGene(table, joinkey, order, curator_id) {
  ...
  if (eviElement.value !== '') { 		// there is some evidence data
    ...
    document.getElementById('td_' + table + '_placeholder').style.display = '';
    document.getElementById('td_' + table + '_info').style.display = 'none';
...
function geneEviToDiv(table, joinkey, order, curator_id) {
    var geneValue = document.getElementById('div_evi_' + table + '_display').innerHTML; 
    var eviValue = document.getElementById('input_' + table + '_published_as').value;
...
function toggleDivDoubleToggle(table, joinkey, order, curator_id, one, two) {
    var divElement = document.getElementById('div_' + table + '_' + joinkey + '_' + order);
    var setTo = two;							// set two by default
    if (divElement.innerHTML === one) { setTo = two; }			// if one, set two
    else if (divElement.innerHTML === two) { setTo = one; }		// if two, set one
