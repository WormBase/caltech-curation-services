...
function postgresQueryField(field, userValue, amountRowsToHighlight) {				// ajax call to query postgres by field and userValue
  ...
  userValue = convertDisplayToUrlFormat(userValue); 		// convert <userValue> to URL format by escaping characters
...
// chrome does not like size=1 select to be multiple, it adds a checkbox.  this fix turns it into a dropdown on firefox, so seems worse, but curators are okay with this.  2026 02 23
        ...
        success : function (o) {						// Successful XHR response handler 
            if (o.responseText !== undefined) { 
                if (o.responseText === "true") { 
                    newValue = decodeURIComponent(newValue)			// escape ontology/dropdown values to allow for ; in object names
...
console.log('checkValueIsValidAndUpdate before ' + newValue);
    newValue = convertDisplayToUrlFormat(newValue); 				// convert <newValue> to URL format by escaping characters
console.log('checkValueIsValidAndUpdate converted ' + newValue);
...
// console.log('convertDisplayToUrlFormat ' + value);
    }
...
//         newValue = decodeURIComponent(newValue)					// already happened during checkValueIsValidAndUpdate
...
console.log('updated to ' + displayValue);
                var record = myDataTable.getRecord(selectedRows[i]);		// get the record
                displayValue = decodeURIComponent(displayValue)			// escape all values to allow for ; in object names
console.log('decode to ' + displayValue);
...
function updatePostgresTableField(pgid, field, newValue, dataTableFlag, selectedRows, hashPgidsChanged) {
    ...
    var callbacks = { 
        success : function (o) {				// Successful XHR response handler 
            ...
            else { alert("ERROR not OK response for pgid " + pgid + ", field " + field + "\n\n" + o.responseText + "\n\noriginal value:\n" + newValue); }
//             else { alert("ERROR not OK response for pgid " + pgid + ", field " + field + ", and newValue : " + newValue + "\n\n" + o.responseText); }
//             else { alert("ERROR not OK response for pgid " + pgid + ", field " + field + "\n\n" + o.responseText); }
            undisableForm();
        },
        failure:function(o) {
...
//         var postData = "action=updatePostgresTableField&pgid="+pgid+"&field="+field+"&newValue="+escape(newValue)+"&datatype="+datatype+"&curator_two="+curatorTwo;	// escape now happenning in convertDisplayToUrlFormat
        var postData = "action=updatePostgresTableField&pgid="+pgid+"&field="+field+"&newValue="+newValue+"&datatype="+datatype+"&curator_two="+curatorTwo;
...
//         var sUrl = cgiUrl + "?action=updatePostgresTableField&pgid="+pgid+"&field="+field+"&newValue="+escape(newValue)+"&datatype="+datatype+"&curator_two="+curatorTwo;	// escape now happenning in convertDisplayToUrlFormat
        var sUrl = cgiUrl + "?action=updatePostgresTableField&pgid="+pgid+"&field="+field+"&newValue="+newValue+"&datatype="+datatype+"&curator_two="+curatorTwo;
...
function updatePostgresColumn(newValue, field, columnAction) {
    ...
    newValue = convertDisplayToUrlFormat(newValue); 		// convert <newValue> to URL format by escaping characters
//     var sUrl = cgiUrl + "?action=updatePostgresColumn&column_action="+columnAction+"&datatype="+datatype+"&curator_two="+curatorTwo+"&field="+field+"&newValue="+escape(newValue);	// escape now happenning in convertDisplayToUrlFormat
    var sUrl = cgiUrl + "?action=updatePostgresColumn&column_action="+columnAction+"&datatype="+datatype+"&curator_two="+curatorTwo+"&field="+field+"&newValue="+newValue;
    YAHOO.util.Connect.asyncRequest('GET', sUrl, callbacks);	// Make the call to the server to update postgres
