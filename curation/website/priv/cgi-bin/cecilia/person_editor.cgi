...
#
# Checkout Papers should only show you papers that have a PMID.
# Picking a paper, like 00061215 will show you the ORCID if the author has it in the XML.
# Create people from XML should also create the ORCID for the newly created person, but it's not tested.  2021 09 02
#
# Added Chris Grove to curators.  2021 09 16
#
...
#   $result = $dbh->prepare( "SELECT joinkey FROM pap_curation_flags WHERE pap_curation_flags = 'author_person' AND joinkey NOT IN (SELECT joinkey FROM pap_curation_done WHERE pap_curation_done = 'author_person') ORDER BY joinkey::INTEGER DESC" );
  $result = $dbh->prepare( "SELECT joinkey FROM pap_curation_flags WHERE pap_curation_flags = 'author_person' AND joinkey NOT IN (SELECT joinkey FROM pap_curation_done WHERE pap_curation_done = 'author_person') AND joinkey IN (SELECT joinkey FROM pap_identifier WHERE pap_identifier ~ 'pmid') ORDER BY joinkey::INTEGER DESC" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
  my $to_display = '';
...
sub createPeopleFromXml {
  ...
  my @two_fields = qw( old_inst_choice inst_choice aid highest_join standardname firstname middlename lastname orcid email );
  my @two_tables = qw( standardname firstname middlename lastname orcid email );
...
#       if ($xml_authors{$aname}{standardname}) { $standardname = $xml_authors{$aname}{standardname}; $xml_authors_found{$aname}++; }
    ...
    $line .= "<td><input name=\"orcid_$i\" value=\"$orcid\"></td>";
...
sub firstPage {
  ...
  print "<form name='form1' method=\"get\" action=\"person_editor.cgi\">\n";
  print "<table border=0 cellspacing=5>\n";
  print "<tr><td colspan=\"2\">Select your Name : <select name=\"curator_two\" onChange=\"saveCuratorIdInCookieFromSelect(this)\" size=\"1\">\n";
  &populateCurators();
  my $saved_curator = &readSavedCuratorFromCookie();
#   my @curator_list = ('Juancarlos Chan', 'Cecilia Nakamura');
  my @curator_list = ('two1823', 'two2987', 'two1', 'two12028', 'two2970', 'two712');
...
sub getCuratorFromForm {
  ...
  if ($curator_two) {
    #&updateCurator($curator_two);
    1;
  } else { print "ERROR : No curator chosen, using two1<br />\n"; $curator_two = 'two1'; }
  print "<input type=\"hidden\" name=\"curator_two\" id=\"curator_two\" value=\"$curator_two\">";
  return $curator_two;
} # sub getCuratorFromForm
