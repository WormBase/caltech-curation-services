...
# Form to submit strain requests
#
# changed ip to come from header HTTP_X_REAL_IP if it exists.  2016 02 08
#
# Add Daniela to the email recepients.  2017 05 18
#
# take out line break for Paul Davis.  2021 06 14
#
# Updated for https like webinar.cgi  2021 10 01
#
...
#   if ($userValue =~ m/(.*?) \( (.*?) \)/) { $standard_name = $1; $person_id = $2; } else { $person_id = $userValue; }
  ...
  $to_print = join"\n", @data;
#   $to_print = qq(Click <a href="strain_request.cgi?action=personPublication&personId=${person_id}&personName=${standard_name}&personEmail=${first_email}" target="new" style="font-weight: bold; text-decoration: underline;">here</a> to review your publications and see which are in need of allele-sequence curation<br/>\n) . $to_print;
...
#   print qq(<span>We would appreciate your help in adding allele-sequence data to WormBase.<br/>Please fill out the form below. All fields with a <span style="color: red;">*</span> are required.<br/>If you have any questions, please do not hesitate to contact WormBase at <a href="mailto:genenames\@wormbase.org">genenames\@wormbase.org</a></span><br/><br/>\n);
  print qq(<span style="font-size: 24pt;">Request knockout strain</span><br/><br/>\n);
  print qq(<span>The C. elegans Knockout Consortium, funded by NIH R24OD023041, will entertain requests for knockouts of C. elegans orthologs of human Alzheimer's Disease and Related Dementia (ADRD) genes. They will be provided at no charge and with no expectation of collaboration.  Strains will be sent to the Caenorhabditis Genetics Center upon production.  You will be notified by email of their construction.</span><br/><br/>\n);
#   print qq(Please request by filling out the simple webform. All fields with a <span style="color: red;">*</span> are required.<br/><br/>\n);
  print qq(Please request by filling out the simple webform.<br/><br/>\n);
  print qq(Thank you for your interest in using C. elegans genetics to model human disease.<br/><br/>\n);
...
#   unless ($fields{person}{termidvalue}{1}) {		# getting user by IP is not working because of the reverse proxy at WormBase giving everyone the same IP 2022 02 10
...
#   $td .= qq(<option value=""></option>); 	# strain_request doesn't have default blank, has explicit 'no preference'
...
# This is how phenotype.cgi handles radios, copied here to model.
...
#   return qq(<td style="min-width: 300px; border-style: solid; border-color: #000000;"><input name="input_${i}_$field" id="input_${i}_$field" style="width: 97%;" $readonly $placeholder value="$inputvalue"></td>\n);
  my $td = qq(<td style="min-width: 300px; max-width: 300px;" colspan="$colspan">);
  foreach my $value (keys %{ $dropdown{$field} }) {
    # my $selected = '';
    # if ($fields{$field}{inputvalue}{$i} eq $value) { $selected = qq(selected="selected"); }
    # $td .= qq(<option $selected value="$value">$dropdown{$field}{$value}</option>);
    my $checked = '';
    if ($fields{$field}{inputvalue}{$i} eq $value) { $checked = qq(checked="checked"); }
    $td .= qq(<input type="radio" id="input_${i}_$value" name="input_${i}_$field" value="$value" $checked> $dropdown{$field}{$value}<br/>); }
  $td .= qq(</td>\n);
  return $td;
...
#   my $labelTdStyle   = qq(style="min-width: $minwidth; border-style: solid; border-color: #000000; $fontsize");
  ...
  my $mandatory = '';
#   if ($fields{$field}{mandatory}) { $mandatory = '<span style="color: red;">*</span>'; }	# in strain request all fields are required
...
#       $warningvalue = qq(Click <a href='strain_request.cgi?action=personPublication&personId=${person_id}&personName=${person_name}&personEmail=${person_email}' target='new' style='font-weight: bold; text-decoration: underline;'>here</a> to review your publications and see which are in need of allele-sequence curation<br/>\n); } }
...
# print qq(J $j I $i FIELD $field TAG $tag TV $tag_value E<br>);
      ...
      if ($fields{$field}{type} eq 'text') {          $td_text .= &printEditorText($i, $field, $colspan);     }
        ...
        elsif ($fields{$field}{type} eq 'radio') {    $td_text .= &printEditorRadio($i, $field, $colspan);    }
      my $td_warnings .= &printEditorWarnings($i, $field);
      $trToPrint .= $td_indent; 
      $trToPrint .= $td_label; 
      $trToPrint .= $td_text; 
...
sub printTypeKnockoutField {             printArrayEditorNested('typeknockout');        }
...
sub showForm {
  ...
  return if $goodOrBad;
  print qq(<form method="post" action="strain_request.cgi" enctype="multipart/form-data">);
...
sub tableDisplayArray {
  ...
  for my $i (1 .. $amount) {
    ...
    foreach my $field (@fields) {
      ...
      if ($inputtermidvalue =~ m/\r\n/) { $inputtermidvalue =~ s/\r\n/ /g; }	# take out line break for Paul Davis.  2021 06 14
      if ($inputtermidvalue =~ m/\n/) { $inputtermidvalue =~ s/\n/ /g; }	# take out line break for Paul Davis.  2021 06 14
...
sub submit {
  ...
  $form_data    .= &tableDisplayArray('typeknockout'); 
...
#           $form_data = qq(Dear $fields{person}{inputvalue}{1}, thank you for submitting Allele-Sequence data.<br/>A WormBase curator will be in touch if there are any questions<br/><br/>) . $form_data;			# prepend thank you message
          my $messageToUser = qq(Dear $fields{person}{inputvalue}{1}, thank you for requesting a knockout strain.<br/>The Consortium will email you if there are any questions.<br/>);
          print qq($messageToUser<br/>);
          print qq(<br/>$form_data);
          print qq(<br/>Return to the <a href="strain_request.cgi">Strain Request Form</a>.<br/>\n);
          my $user = 'strain_request_form@' . $hostfqdn;	# who sends mail
...
#   $fields{person}{haschecks}                                  = 'person';
  $fields{person}{terminfo}                                   = qq(Start typing and select your name from the list of registered WormBase persons and IDs. If you do not have a WBPerson ID, fill out our <a href="http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/person.cgi" target="new">Person Update Form</a>, or type your name and click out without selecting an existing WBPerson.);
...
#   $fields{pmid}{label}                                        = 'PubMed ID';
  $fields{pmid}{label}                                        = 'Reference/source for ADRD association';
...
#   tie %{ $fields{gene}{field} }, "Tie::IxHash";
  ...
  $fields{gene}{label}                                        = 'C. elegans Gene';
  $fields{gene}{example}                                      = 'e.g. lin-3';
  $fields{gene}{mandatory}                                    = 'mandatory';
  $fields{gene}{ontology_type}                                = 'WBGene';

#   tie %{ $fields{typeknockout}{field} }, "Tie::IxHash";
  $fields{typeknockout}{multi}                                = '1';
#   $fields{typeknockout}{type}                                 = 'dropdown';
  $fields{typeknockout}{type}                                 = 'radio';
  $fields{typeknockout}{label}                                = 'Type of Knockout';
#   $fields{typeknockout}{terminfo}                             = qq(If applicable, choose the inheritance pattern of the allele with respect to the phenotype(s) entered.);
  $fields{typeknockout}{mandatory}                            = 'mandatory';
...
# #   $fields{comment}{startHidden}                               = 'startHidden';
  ...
  tie %{ $dropdown{typeknockout} }, "Tie::IxHash";
  $dropdown{typeknockout}{"No Preference"}                    = "No Preference";
  $dropdown{typeknockout}{"GFP insertion cassette"}           = "<a href='https://pubmed.ncbi.nlm.nih.gov/30224336/' target='_blank'>GFP insertion cassette</a>";
  $dropdown{typeknockout}{"STOP-IN"}                          = "<a href='https://pubmed.ncbi.nlm.nih.gov/30420468/' target='_blank'>STOP-IN</a>";
...
# <script type="text/javascript" src="https://yui.yahooapis.com/2.7.0/build/datasource/datasource-min.js"></script>
# <script type="text/javascript" src="https://yui.yahooapis.com/2.7.0/build/autocomplete/autocomplete-min.js"></script>
# <script type="text/javascript" src="https://yui.yahooapis.com/2.7.0/build/json/json-min.js"></script>
-->
...
<script type="text/javascript" src="$baseUrl/javascript/strain_request.js"></script>
...
# sub getUserByIp {		# getting user by IP is not working because of the reverse proxy at WormBase giving everyone the same IP  2022 02 10
