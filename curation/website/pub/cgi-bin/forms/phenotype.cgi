...
#
# Added green ? mouseover pointer to cursor for Mary Ann.  2017 03 14
#
...
#
# Changed some text, urls for deciding whether a paper has been flagged, pmid xml source.
# Changed some .js and .css to be https for WormBase redirect.  2018 04 05
#
# Changed some email text to send pgids from both OAs as appropriate.  2018 06 22
#
# flaggedPapers for personPublication should include papers validated positive (not necessarily flagged by any method)  2018 07 03
#
# Even though suggested / not-suggested are not optional and their definitions + remarks are optional, some users do just those,
# so trigger off of any of the 3, not only on the suggested term.  2018 07 16
#
# Added a javascript back a page link after submit. 
# Form no longer loads user person based on ip, for Chris.  
# Added an action  &noNematodePhenotypes  to email Chris when a user reports that for the paper.  2018 10 18
#
# Person term info now shows all emails in reverse timestamp order, so that extracting the first email from the term info into the
# form's email field will populate the most recent one.  2021 09 16
#
# Variations in obo_data_variation might be 'Dead', in which case don't show them on the autocomplete.  2021 09 17
#
# Added &getAnySpecificNameToId and &getGenericNameToId so that on form submission the termid field is ignored, and the object ID
# is derived from the input name.  
# A flaw in the javascript allows a valid name to be typed and mapped to an ID, then the name changed to another valid name, 
# and not update the termid to map to the new valid name.  So always ignore the termid values and derive them from the name.
# This is an issue because the ontology fields also allow non-ontology strings that could match a proper name, so use the name
...
sub noNematodePhenotypes {
  print "Content-type: text/html\n\n";
  print $header;
  ($var, my $userEmail)  = &getHtmlVar($query, 'email');
  ($var, my $wbperson)   = &getHtmlVar($query, 'wbperson');
  ($var, my $name)       = &getHtmlVar($query, 'name');
  ($var, my $pmid)       = &getHtmlVar($query, 'pmid');
  ($var, my $wbpaper)    = &getHtmlVar($query, 'wbpaper');
  my $paperInfo = &getPaperInfo($wbpaper, $pmid);
# email=closertothewake@gmail.com&wbperson=WBPerson2735&name=Dayong+Wang&pmid=28738528&wbpaper=00051554
  my $message = qq(Thank you for indicating that the following paper\n\n$paperInfo\nreports no nematode phenotypes. A WormBase curator will be in touch if there are any questions.<br/>);
  $message =~ s/\n/<br\/>/g;
  print qq($message);
...
#   my $email = 'closertothewake@gmail.com';
  my $subject = "Phenotype form $wbpaper flagged no nematode phenotypes";		# subject of mail
  my $body = qq(The following paper has been flagged as not having any nematode phenotypes:\n\nWBPaper$wbpaper\n$paperInfo\nBy the author: $name\n$wbperson\n$userEmail); 
  $body =~ s/\n/<br\/>/g;
...
sub getPaperInfo {
  ...
  my %papTitle; my %papYear; my %papJournal; my %papAuthor; my %papAuthorIndex;
  $result = $dbh->prepare( "SELECT * FROM pap_title WHERE joinkey = '$paper'" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) {
    if ($row[0]) { $papTitle{$row[0]} = $row[1]; } }
  $result = $dbh->prepare( "SELECT * FROM pap_year WHERE joinkey = '$paper'" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) {
    if ($row[0]) { $papYear{$row[0]} = $row[1]; } }
  $result = $dbh->prepare( "SELECT * FROM pap_journal WHERE joinkey = '$paper'" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) {
    if ($row[0]) { $papJournal{$row[0]} = $row[1]; } }
  $result = $dbh->prepare( "SELECT * FROM pap_author_index WHERE author_id IN (SELECT pap_author FROM pap_author WHERE joinkey = '$paper');" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) {
    if ($row[0]) { $papAuthorIndex{$row[0]} = $row[1]; } }
  $result = $dbh->prepare( "SELECT * FROM pap_author WHERE pap_order < 5 AND joinkey = '$paper'" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) {
    if ($row[0]) {
  ...
  if ($pmid)               { $paperInfo .= qq(PubMed ID: $pmid\n); }
  return $paperInfo;
} # sub getPaperInfo
...
#   (@paps) = split/\s+/, $curStatusJoinkeys; foreach (@paps) { $flaggedPapers{"WBPaper$_"}{rna}++; }
  ...
  $result = $dbh->prepare( "SELECT * FROM app_curator WHERE joinkey IN ('$pgidHasApp') AND app_curator != 'WBPerson29819' AND joinkey NOT IN (SELECT joinkey FROM app_nodump);" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
...
sub getAnySpecificNameToId {
  my ($ontology_type, $userValue) = @_; my $match = '';
  if ($ontology_type eq 'WBPerson') {          ($match) = &getAnyWBPersonNameToId($userValue); }
    elsif ($ontology_type eq 'WBTransgene') {  ($match) = &getAnyWBTransgeneNameToId($userValue); }
    elsif ($ontology_type eq 'WBGene') {       ($match) = &getAnyWBGeneNameToId($userValue); }
  return $match;
} # sub getAnySpecificNameToId

sub getAnyWBGeneNameToId {
  my ($inputvalue) = @_;
  $inputvalue = lc($inputvalue);
  my @tables = qw( gin_locus gin_synonyms gin_seqname gin_wbgene );
  my $match = '';
  foreach my $table (reverse @tables) {
    my $result = $dbh->prepare( "SELECT * FROM $table WHERE LOWER($table) = '$inputvalue';" );
    $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; my @row = $result->fetchrow();
    if ($row[0]) { $match = 'WBGene' . $row[0]; } }
  return $match; }

sub getAnyWBPersonNameToId {
  my ($inputvalue) = @_;
  $result = $dbh->prepare( "SELECT * FROM two_standardname WHERE two_standardname = '$inputvalue';" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; my @row = $result->fetchrow();
  my $match = '';
  if ($row[0]) { $match = $row[0]; $match =~ s/two/WBPerson/; }
  return $match; }

sub getAnyWBTransgeneNameToId {
  my ($inputvalue) = @_;
  $inputvalue = lc($inputvalue);
  my $table = 'trp_publicname';
  $result = $dbh->prepare( "SELECT trp_name.trp_name, ${table}.$table FROM trp_name, $table WHERE trp_name.joinkey = ${table}.joinkey AND LOWER(${table}.$table) = '$inputvalue';" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; my @row = $result->fetchrow();
  my $match = '';
  if ($row[0]) { $match = $row[0]; }
  return $match; }

sub getGenericNameToId {
  my ($field, $inputvalue) = @_;
  my $ontology_table  = $fields{$field}{ontology_table};
  $result = $dbh->prepare( "SELECT * FROM obo_name_${ontology_table} WHERE obo_name_${ontology_table} = '$inputvalue';" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; my @row = $result->fetchrow();
  my $match = '';
  if ($row[0]) { $match = $row[0]; }
  return $match; }
...
sub getAnyWBGeneTermInfo {
  ...
  my $wormbase_link = "http://www.wormbase.org/db/get?name=WBGene$joinkey;class=Gene";
...
#   if ($userValue =~ m/(.*?) \( (.*?) \)/) { $standard_name = $1; $person_id = $2; } else { $person_id = $userValue; }
  ...
  $result = $dbh->prepare( "SELECT * FROM two_email WHERE joinkey = '$joinkey' ORDER BY two_timestamp DESC;" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
  while (my @row = $result->fetchrow) { if ($row[2]) { push @emails, $row[2]; } }
  ($joinkey) = $joinkey =~ m/(\d+)/;
...
#   if ( $ebiData =~ m/<authorString>(.*?)<\/authorString>/sm) { $authors = $1; }       # get the authors from the xml
  my $pmidData = get "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=" . $pmid . "&retmode=xml";
  if ( $pmidData =~ m/<ArticleTitle>(.*?)<\/ArticleTitle>/sm) {                        $title   = $1; }       # get the title from the xml
  if ( $pmidData =~ m/<Journal>.*?<Title>(.*?)<\/Title>.*?<\/Journal>/sm) {            $journal = $1; }       # get the journal from the xml
  if ( $pmidData =~ m/<DateCompleted>.*?<Year>(.*?)<\/Year>.*?<\/pubYear>/sm) {        $year    = $1; }       # get the year from the xml
  if ( $pmidData =~ m/<LastName>(.*?)<\/LastName>/sm) {                                $authors = $1; }       # get the authors from the xml
  if ($title || $journal || $year || $authors) {
    ...
    if ($authors) { $toPrint .= qq(First Author: $authors\n); } }
  my (@data) = split/\n/, $toPrint;
  foreach my $data_line (@data) { $data_line =~ s/^(.*?):/<span style=\"font-weight: bold\">$1 : <\/span>/; }
  my $toPrint = join"<br />\n", @data;
  return $toPrint;
...
sub getGenericOboAutocomplete {
  ...
  my $query_modifier  = qq(AND joinkey NOT IN (SELECT joinkey FROM $obodata_table WHERE $obodata_table ~ 'is_obsolete' OR $obodata_table ~ 'Dead') ); 
...
#   print qq(<span>We would appreciate your help in adding phenotype data from published papers to WormBase.<br/>Please fill out the form below. <b>Watch a short video tutorial <a style='font-weight: bold; text-decoration: underline;' href="https://www.youtube.com/watch?v=_gd87S1h3zg&feature=youtu.be" target="_blank">here</a> or read the user guide <a style='font-weight: bold; text-decoration: underline;' href="http://wiki.wormbase.org/index.php/Contributing_Phenotype_Connections" target="_blank">here</a></b>.<br/>If you would prefer to fill out a spreadsheet with this information, please download and fill out our<br/><a href="https://dl.dropboxusercontent.com/u/4290782/WormBase_Phenotype_Worksheet.xlsx" target="_blank">WormBase Phenotype Worksheet</a> and e-mail as an attachment to <a href="mailto:curation\@wormbase.org">curation\@wormbase.org</a><br/>If you have any questions, please do not hesitate to contact WormBase at <a href="mailto:help\@wormbase.org">help\@wormbase.org</a></span><br/><br/>\n);
  print qq(<span>We would appreciate your help in adding phenotype data from published papers to WormBase.<br/>Please fill out the form below. <b>Read the user guide <a style='font-weight: bold; text-decoration: underline;' href="http://wiki.wormbase.org/index.php/Contributing_Phenotype_Connections" target="_blank">here</a></b>.<br/>If you would prefer to fill out a spreadsheet with this information, please download and fill out our<br/><a href="https://tazendra.caltech.edu/~acedb/chris/WormBase_Phenotype_Worksheet.xlsx" target="_blank">WormBase Phenotype Worksheet</a> and e-mail as an attachment to <a href="mailto:curation\@wormbase.org">curation\@wormbase.org</a><br/>If you have any questions, please do not hesitate to contact WormBase at <a href="mailto:help\@wormbase.org">help\@wormbase.org</a></span><br/><br/>\n);
...
#   my ($i, $field, $group, $inputvalue, $termidvalue, $input_size, $colspan, $fieldclass, $placeholder, $cols_size, $rows_size) = @_;
  my ($i, $field, $colspan) = @_;
  my $inputvalue  = ''; my $textareavalue; my $termidvalue = ''; my $placeholder = ''; my $readonly = ''; 
  my $cols_size  = '40';
  my $rows_size  = '5';
  if ($fields{$field}{inputvalue}{$i})     { 
    $inputvalue     = $fields{$field}{inputvalue}{$i}; 		# previous form value
    if ($inputvalue =~ m/"/) { $inputvalue =~ s/\"/&quot;/g; }	# if it had doublequotes, escape of they'll become part of the html
    $textareavalue  = $fields{$field}{textareavalue}{$i}; }	# previous form value
  if ($fields{$field}{termidvalue}{$i})    { $termidvalue = $fields{$field}{termidvalue}{$i}; }
  if ($fields{$field}{example})            { $placeholder = qq(placeholder="$fields{$field}{example}"); }
  if ($fields{$field}{type} eq 'readonly') { $readonly    = qq(readonly="readonly"); }
  ...
  foreach my $field (keys %fields) {
    ...
    for my $i (1 .. $amount) {
      ...
      if ($inputvalue) { 
        ...
        if ( $fields{$field}{type} eq 'ontology' ) {
          if ($fields{$field}{ontology_type} eq 'obo') { 
              my $match = &getGenericNameToId($field, $inputvalue); 
              $fields{$field}{termidvalue}{$i} = $match; }
            else {
              my $ontology_type = $fields{$field}{ontology_type};
              my $match = &getAnySpecificNameToId($ontology_type, $inputvalue);
              $fields{$field}{termidvalue}{$i} = $match; } }
      }
# ignore termid_I_value, flaw in javascript allows a valid name to be typed and mapped to an ID, then the name changed to another valid name, and not update the termid to map to the new valid name.
...
#     my $email      = 'closertothewake@gmail.com';
    my $subject    = qq(Phenotype Form: Unregistered gene alert);
    if ($newPgidsApp) { $emailGene .= qq(New PGIDs in phenotype OA $newPgidsApp\n); }
    if ($newPgidsRna) { $emailGene .= qq(New PGIDs in RNAi OA $newPgidsRna\n); }
...
sub writePgRowFieldsApp {
  ...
  &writePgField($datatype, $pgid, 'genotype', $genotype);
  &writePgField($datatype, $pgid, 'controlstrain', $strain);
...
#   tie %{ $fields{allele}{field} }, "Tie::IxHash";
  ...
  $fields{transgene}{label}                                   = 'Overexpression Transgene';
  ...
  $fields{transgenegene}{label}                               = 'Overexpressed Gene that causes Phenotype';
...
#   tie %{ $fields{obssuggestedterm}{field} }, "Tie::IxHash";
  ...
  $fields{obssuggestedterm}{terminfo}                         = qq(Provide the name of a new phenotype term and a brief definition. This phenotype will be annotated to all alleles, RNAi clones, and trsansgenes you have indicated above.);
...
<link rel="stylesheet" type="text/css" href="stylesheets/yui_edited_autocomplete.css" />
...
<script type="text/javascript" src="javascript/phenotype.js"></script>
