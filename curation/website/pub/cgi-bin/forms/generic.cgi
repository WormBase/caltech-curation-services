...
#
# added 'year' as output to PapIdToWBPaper for Chris.  2018 07 10
# 
# allow variations to have up to 4 letters.  2020 01 15
#
# allow temp strains along with temp variations.  2020 03 05
#
# &convertPaperIdentifiers() now has an option to select "Persons verified in paper set", which 
# gets all verified people for papers found and lists them based on frequency.  
# For Daniela and Karen.  2020 07 10
# 
# &showAntibodyData() not used by anyone, uses textpresso-dev, so getting rid of it.  2021 01 28
#
...
#         $maps{papToData}{wbpaper}{"WBPaper$row[0]"}    = "WBPaper$row[0]";	# get these from pap_status instead
        ...
        $maps{idToPap}{$prefix}{$1}                    = "$row[0]";
        $maps{idToPap}{$prefix}{$identifier}           = "$row[0]"; } } }
...
#   $table_header = join"\t", @table_header; push @paps, $table_header;
  ...
  foreach my $pap_identifier (@pap_identifiers) {
    ...
    my $joinkey;
    if ($identifier_type eq 'wbpaper') { ($joinkey) = $pap_identifier =~ m/(\d+)/; }
      else { 
        if ($maps{idToPap}{$identifier_type}{$pap_identifier}) { $joinkey = $maps{idToPap}{$identifier_type}{$pap_identifier}; }
          else { push @bads, $pap_identifier; next; } }
    $joinkeys{$joinkey}++;
    my $row_has_data = 0;
    my $wbpaper = 'WBPaper' . $joinkey;
    foreach my $output_type (@output_types) {
      ($var, my $output_type_chosen)   = &getHtmlVar($query, "output_$output_type");
...
#         my $row_output = join"\t", @row_output; push @paps, $row_output;
  ...
  ($var, my $persons_verified_flag)   = &getHtmlVar($query, "output_persons_verified");
  if ($persons_verified_flag) { 
    my $joinkeys = join"','", sort keys %joinkeys;
    my %aids;
    $result = $dbh->prepare( "SELECT * FROM pap_author WHERE joinkey IN ('$joinkeys');" );
    $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
    while (my @row = $result->fetchrow) { $aids{$row[1]}++; }
    my $aids = join"','", sort keys %aids;
    my %ver; my %pos;
    $result = $dbh->prepare( "SELECT * FROM pap_author_verified WHERE pap_author_verified ~ 'YES' AND author_id IN ('$aids');" );
    $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
    while (my @row = $result->fetchrow) { $ver{$row[0]}{$row[2]} = $row[1]; }
    $result = $dbh->prepare( "SELECT * FROM pap_author_possible WHERE author_id IN ('$aids');" );
    $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
    while (my @row = $result->fetchrow) { $pos{$row[0]}{$row[2]} = $row[1]; }
    my %twos;
    foreach my $aid (sort keys %pos) {
      foreach my $join (sort keys %{ $pos{$aid} }) {
        if ($ver{$aid}{$join}) { $twos{$pos{$aid}{$join}}++; } } }
    ...
    print qq(<br/><br/><table border="1">);
    print qq(<tr><th>person</th><th>mentions</th></tr>);
    foreach my $two (sort { $twos{$b} <=> $twos{$a} } keys %twos) {
      print qq(<tr><td><a href="http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/person.cgi?action=Query&number=$two" target="_blank">$names{$two}</a></td><td>$twos{$two}</td></tr>\n);
    }
    print qq(</table>);
  }
  print "$footer"; 		# make end of HTML page
...
sub papIdToWBPaper {
  ...
  $identifier_labels{pmid}    = 'pmid2537252 or simply 2537252';
...
#   print qq(Paper Identifiers (ids without prefix nor leading zeros will be treated as pmid)\n);
  ...
  print qq(<input type="checkbox" NAME="output_persons_verified" VALUE=persons_verified"> Persons verified in paper set<br/>\n);
...
sub listPmids {			# list pmids for alliance to download and convert to json
  ...
  print "Content-type: text/plain\n\n";
  my %papHash;
  $result = $dbh->prepare( "SELECT pap_identifier FROM pap_identifier WHERE pap_identifier ~ 'pmid';" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) { $row[0] =~ s/pmid//; print qq($row[0]\n); }
} # sub wpaXref

# sub showAntibodyData {					# removed 2021 01 28
#   print "Content-type: text/html\n\n";
...
sub tempVariationObo { &tempObjectObo('variation'); }
sub tempStrainObo    { &tempObjectObo('strain');    }
...
#   ($var, my $pubname) = &getHtmlVar($query, 'pubname');
  ...
  foreach my $entry (@entries) {
    ...
    if ($datatype eq 'variation') {
        if ($entry =~ m/^(.*?) (WBVar\d{8})\s*$/) { ($pubname, $objid) = ($1, $2); }
          else { $entry_error .= "No match for $datatype entry $entry<br/>"; }
...
#           unless ($pubname =~ m/^[A-Za-z]{1,3}\d{1,5}$/) { $error_message .= "$pubname does not match 1-3 letters and 1-5 digits<br/>\n"; }
            unless ($pubname =~ m/^([A-Za-z]{1,4}\d{1,6}){1,2}$/) { $entry_error .= "$pubname does not match 1 or 2 sets of 1-4 letters and 1-6 digits<br/>\n"; } }
          else   { $entry_error .= "No public name<br/>"; } }
      elsif ($datatype eq 'strain') {
        if ($entry =~ m/^(.*?) (WBStrain\d{8})\s*$/) { ($pubname, $objid) = ($1, $2); }
          else { $entry_error .= "No match for $datatype entry $entry<br/>"; }
        if ($pubname) {
            $pubname =~ s/\s+//g;
            unless ($pubname =~ m/^([A-Za-z]{2,4}\d{1,5}){1,2}$/) { $entry_error .= "$pubname does not match 1 or 2 sets of 2-4 uppercase letters and 1-5 digits<br/>\n"; } }
          else   { $entry_error .= "No public name<br/>"; } }

#     if ($entry_error) { &printTempVariationForm($entry_error, $objid, $pubname); print $footer; return; }
    $result = $dbh->prepare( "SELECT * FROM obo_name_$datatype WHERE joinkey = '$objid';" ); $result->execute;
    my @row = $result->fetchrow(); 
    if ($row[0]) { $entry_error .= qq($objid already exists associated to $row[1]<br/>\n); }
    $result = $dbh->prepare( "SELECT * FROM obo_name_$datatype WHERE obo_name_$datatype = '$pubname';" ); $result->execute;
    @row = $result->fetchrow(); 
...
#     if ($entry_error) { &printTempVariationForm($entry_error, $objid, $pubname); print $footer; return; }
    ...
    print OUT qq($objid\t$pubname\t$pgDate\t$comment\n);
    close (OUT) or die "Cannot append to $obotempfile : $!";
    my $terminfo = qq(id: $objid\nname: "$pubname"\ntimestamp: "$pgDate"\ncomment: "$comment");
    $result = $dbh->do( "INSERT INTO obo_name_$datatype VALUES('$objid', '$pubname');" );
    $result = $dbh->do( "INSERT INTO obo_data_$datatype VALUES('$objid', '$terminfo');" );
    print "Added $pgDate $objid $pubname to obo_name_$datatype and obo_data_$datatype<br/>\n";
  }
  if ($error_message) { &printTempObjectForm($datatype, $error_message, '', ''); }
  print "$footer"; 		# make end of HTML page
} # sub addTempObjectObo

sub printTempObjectForm {
  my ($datatype, $error_message, $varid, $pubname) = @_;
  ...
  if ($datatype eq 'variation') { print qq(Public name SPACE WBVarID in separate rows<br/>Example:<br/>tn1455 WBVar02143955<br/>tn1455 WBVar02143955<br/>tn1409 WBVar02143956<br/>tn1408 WBVar02143957<br/>tn1431 WBVar02143958<br/>tn1392 WBVar02143959<br/>\n); }
    elsif ($datatype eq 'strain') { print qq(Public name SPACE WBStrainID in separate rows<br/>Example:<br/>N2 WBStrain00000001<br/>AA1 WBStrain00000005<br/>AA6 WBStrain00000006<br/>\n); }
  print qq(<textarea name="objectidnamebatch" rows="8" cols="80"></textarea><br/>\n);
#   print qq(WBVarID <input name="varid" value="$varid"><br/>\n);
#   print qq(Public name <input name="pubname" value="$pubname"><br/>\n);
  print qq(<input type="submit" NAME="action" VALUE="AddTempObjectObo"><br/>\n);
  print qq(</form>\n);
