...
# get a list of curated (and valid for curation) papers that have nothing flagged for any field that gets emailed to someone.
# for Karen  2009 06 04
...
my %hash;				# which fields have emails (someone to curate them)
my %theHash;				# FP data
my %invalidWpas;			# wpas that are invalid, no curatable, functional annotation, meeting, or abstract
my %curated;				# wpas that have been curated
my %allWpas;				# list of all Wpas;

&hashName();
&listInvalidNocuratableFunctional();	# added type filtering

foreach my $field (sort keys %{ $hash{mail} }) { 
#   print "$field\n";
  my $result = $dbh->prepare( "SELECT * FROM cfp_$field" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
  while (my @row = $result->fetchrow) {
    $theHash{$field}{$row[0]} = $row[1];
    $theHash{something}{$row[0]}++;
# if ($field eq 'rnai') { print "$field\t$row[0]\t$row[1]\n"; }
  } # while (@row = $result->fetchrow)
} # foreach my $field (sort keys %{ $hash{mail} })

foreach my $pap (sort keys %allWpas ) {
  next unless ($curated{$pap});
  next if ($invalidWpas{$pap});
  next if ($theHash{something}{$pap});
  print "WBPaper$pap\n"; 
} # foreach my $pap (sort keys %{ $theHash{something} } )
...
#   $result = $conn->exec( "SELECT * FROM wpa ORDER BY wpa_timestamp; "); 
  ...
  $result->execute;
  while (my @row = $result->fetchrow) { if ($row[3] eq 'valid') { $allWpas{$row[0]}++; } else { delete $allWpas{$row[0]}; } }	# populate most invalid joinkeys for type
  $result = $dbh->prepare( "SELECT * FROM wpa ORDER BY wpa_timestamp; "); 
  $result->execute;
...
#   $result = $conn->exec( "SELECT * FROM cfp_comment WHERE cfp_comment ~ 'unctional annotations' ORDER BY cfp_timestamp; ");
  $result = $dbh->prepare( "SELECT * FROM cfp_comment WHERE cfp_comment ~ 'unctional annotations' ORDER BY cfp_timestamp; ");
  $result->execute;
  my %type;
  $result = $dbh->prepare( "SELECT * FROM wpa_type ORDER BY wpa_timestamp; "); 
  $result->execute;
  while (my @row = $result->fetchrow) { if ($row[3] eq 'invalid') { $type{$row[0]} = $row[1]; } else { delete $type{$row[0]}; } }	# populate most invalid joinkeys for type
  foreach my $pap (sort keys %type) {
    if ( ($type{$pap} eq '2') || ($type{$pap} eq '3') ||($type{$pap} eq '4') ) { $wpas{$pap}++; } }
  ...
  $result = $dbh->prepare( "SELECT * FROM cfp_curator ORDER BY cfp_timestamp; "); 
  $result->execute;
  while (my @row = $result->fetchrow) { $curated{$row[0]}++; }	# add no curatable 
}
