...
# output.  2005 09 01
#
# Appended normal script to exclude loci from got_curator curated after
# a given date to dump .go file when Ranjana forgot to.  2005 09 15
...
my $pgdate = '2005-08-18';	# date to exclude from forward
# my $pgdate = '2005-07-29';	# date to exclude from forward
my $date = $pgdate; $date =~ s/\D//g; 
...
$result = $conn->exec( "SELECT * FROM got_curator WHERE joinkey ~ '[A-Za-z]' AND joinkey != 'cgc3' AND joinkey != 'abcd' AND joinkey != 'test-1' AND joinkey != 'asdf' AND joinkey != 'zk512.1' > AND got_timestamp > '$pgdate 23:59:59';");
