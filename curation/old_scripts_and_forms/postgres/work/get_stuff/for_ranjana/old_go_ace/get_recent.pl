...
my $result = $conn->exec( "SELECT joinkey FROM got_sequence WHERE got_timestamp > '2003-01-03';" );
...
#                   $ace_entry .= "GO_term\t\"$goid\"\tPaper_evidence\t\"[$theHash{$tag}{value}]\"\n";
		    ...
		    $is_evidence_flag++;
              ...
              if ($theHash{$tag}{value}) {
                if ($theHash{$tag}{value} =~ m/Kishore/) {
                  $ace_entry .= "GO_term\t\"$goid\"\tPerson_evidence\t\"WBPerson324\"\n";
#                   $ace_entry .= "GO_term\t\"$goid\"\t\"$inference\"\tPerson_evidence\t\"WBPerson324\"\n";
                } elsif ($theHash{$tag}{value} =~ m/Schwarz/) {
                  $ace_entry .= "GO_term\t\"$goid\"\tPerson_evidence\t\"WBPerson567\"\n";
#                   $ace_entry .= "GO_term\t\"$goid\"\t\"$inference\"\tPerson_evidence\t\"WBPerson567\"\n";
                } else {
                  $ace_entry .= "GO_term\t\"$goid\"\tPerson_evidence\t\"$theHash{$tag}{value}\"\n";
#                   $ace_entry .= "GO_term\t\"$goid\"\t\"$inference\"\tPerson_evidence\t\"$theHash{$tag}{value}\"\n";
                }
		$is_evidence_flag++;
              } # if ($theHash{$tag}{value})

# comment this out to revert to bad new format -v
              $tag = $ontology . '_goinference' . $i;
              if (($theHash{$tag}{value} ne 'NULL') && ($theHash{$tag}{value} ne '')) {
                $theHash{$tag}{value} =~ s/ --.*$//g;
                $ace_entry .= "GO_term\t\"$goid\"\tGO_inference_type\t\"$theHash{$tag}{value}\"\n";
              } # if ($theHash{$tag}{value})

              $tag = $ontology . '_goinference_two' . $i;
              if (($theHash{$tag}{value} ne 'NULL') && ($theHash{$tag}{value} ne '')) {
                $theHash{$tag}{value} =~ s/ --.*$//g;
                $ace_entry .= "GO_term\t\"$goid\"\tGO_inference_type\t\"$theHash{$tag}{value}\"\n";
              } # if ($theHash{$tag}{value})
# comment this out to revert to bad new format -^

              $tag = $ontology . '_similarity' . $i;
              if ($theHash{$tag}{value}) {
                $ace_entry .= "GO_term\t\"$goid\"\tProtein_id_evidence\t\"$theHash{$tag}{value}\"\n";
