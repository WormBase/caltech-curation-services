...
#
# Changed to 1 / 5 / 52 weeks back format.  2009 07 06
#
# Changed to work off of person_editor.cgi  2011 06 20
#
#
# Updated for WBPerson curation stats.  Email Cecilia, Paul. 
# 0 2 * * mon /home/postgres/work/get_stuff/for_paul/curation_stats/wbperson_creation_stats/get_recent.pl
# 2007 05 24
#
# moved inside :
...
my $outfile = "/home/postgres/work/get_stuff/for_paul/curation_stats/wbperson_creation_stats/outfile";
open(OUT, ">$outfile") or die "Cannot create $outfile : $!";

my %theHash;
my %perWeek;
my $result = $dbh->prepare( "SELECT two, two_timestamp FROM two ORDER BY two_timestamp;" );
...
%theHash = ();
undef $/;
# my $cgi_file = '/home/postgres/public_html/cgi-bin/cecilia/two_display.cgi';
my $cgi_file = '/home/postgres/public_html/cgi-bin/cecilia/person_editor.cgi';
open (IN, "<$cgi_file") or die "Cannot open $cgi_file : $!";
my $all_cgi = <IN>; $/ = "\n";
close (IN) or die "Cannot close $cgi_file : $!";
my ($two_tables) = $all_cgi =~ m/\nmy \@normal_tables = qw\( (.*?) \)/;
# print "T $two_tables T\n";
my @two_tables = split/\s+/, $two_tables;
# my ($two_tables) = $all_cgi =~ m/\nmy \@two_tables = qw\( (.*?) \)/;
# my @two_tables = split/\s+/, $two_tables;
# my ($two_simpler) = $all_cgi =~ m/\nmy \@two_simpler = qw\( (.*?) \)/;
# my @two_simpler = split/\s+/, $two_simpler;
# foreach my $table (@two_tables, @two_simpler) 
foreach my $table (@two_tables) {
#   $result = $dbh->prepare( "SELECT joinkey, two_timestamp FROM $table ORDER BY two_timestamp;" );
  $result = $dbh->prepare( "SELECT joinkey, two_timestamp FROM two_$table ORDER BY two_timestamp;" );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) { 
    next unless ($row[1]);
...
my $subject = 'Automated WBPerson creation Stat output';
