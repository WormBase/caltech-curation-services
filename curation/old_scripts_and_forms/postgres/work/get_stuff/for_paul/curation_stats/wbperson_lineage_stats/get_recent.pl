...
#
# Updated for 1 / 5 / 52 weeks back format.  2009 07 06
#
#
# Updated for WBPerson lineage curation stats.  Email Cecilia, Paul. 
...
my $outfile = "/home/postgres/work/get_stuff/for_paul/curation_stats/wbperson_lineage_stats/outfile";
open(OUT, ">$outfile") or die "Cannot create $outfile : $!";

my %theHash;
my %perWeek;
my $result = $dbh->prepare( "SELECT joinkey, two_number, two_timestamp FROM two_lineage WHERE joinkey IS NOT NULL AND joinkey !~ 'NO' AND two_number IS NOT NULL AND two_number !~ 'NO' ORDER BY two_timestamp;");
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ($row[0]) = $row[0] =~ m/(\d+)/;
    ($row[1]) = $row[1] =~ m/(\d+)/;
    if ($row[0] > $row[1]) { ($row[0], $row[1]) = ($row[1], $row[0]); }
    my $key = $row[0] . '_' . $row[1];
    next if ($theHash{key_gene}{$key});
    $theHash{key_gene}{$key}++;
    push @{ $theHash{key_time}{$row[2]} }, $key; }
}
...
my $subject = 'Automated WBPerson lineage Stat output';
