...
# Chris no longer wants to filter out people that have been emailed before  2018 03 02
...
# there are no results prioritizing to afp, ignore these
...
# For this script don't care about what's been emailed for concise.  2018 03 02
...
#   my $countThreshold = 100;
  my $countThreshold = 100000;
#   print qq(Showing most recent $countThreshold entries\n);
...
#   print $header;
  my $count = 0;
  my %personPaperCount; my %personPapers;
  my $papers_type_who_file = 'papers_type_who';
  my $person_papers_file   = 'person_papers';
  open (PTW, ">$papers_type_who_file") or die "Cannot create $papers_type_who_file : $!";
  open (PPF, ">$person_papers_file") or die "Cannot create $person_papers_file : $!";
...
#               if ($recentpeople{$two}) { $recentlyEmailed++; }	# 2017 07 18 no longer prevent showing on the list from an pdf email having been emailed
        ...
        foreach my $pdfemail (@pdfemails) {
          ...
          foreach my $category (sort keys %categories) {
            my $color = $categories{$category}{color} || 'yellow';
            next if ( ($color eq 'orange') || ($color eq 'red') );
            if ($emails{$category}{$pdfemail}) {
              my $two = $emails{$category}{$pdfemail} || ''; 
#               push @emails, qq(<span style='color: $color'>$pdfemail</span>);
              push @emails, qq($pdfemail);
              if ($two ne 'nowbperson') {
                my $wbperson = $two; $wbperson =~ s/two/WBPerson/g; 
#                 push @twos, qq(<span style='color: $color'>$wbperson</span>);
                push @twos, qq($wbperson);
                if ($two{name}{$two}) { 
#                   push @names, qq(<span style='color: $color'>$two{name}{$two}</span>);
                  push @names, qq($two{name}{$two});
                } } } } }
...
#       print qq($row);
#       print qq(</form>);
      my $actualEmail = ''; my $actualPerson = ''; my $actualName = ''; my $category = '';
      if ($emails) {         $actualEmail = $emails;   $actualPerson = $person; $actualName = $personName; $category = 'FirstAuthor';   }
        elsif ($cEmail) {    $actualEmail = $cEmail;   $actualPerson = $cTwo;   $actualName = $cName;      $category = 'Corresponding'; }
#         elsif ($afpEmail) {  $actualEmail = $afpEmail; $actualPerson = $afpTwo; $actualName = $afpName;    $category = 'AFP';           }
        elsif ($pdfEmail) {  $actualEmail = $pdfEmail; $actualPerson = $pdfTwo; $actualName = $pdfName;    $category = 'PDF';           }
# UNCOMMENT FOR papers_type_who
      print PTW qq($pap\t$category\t$actualPerson\t$actualName\t$actualEmail\n);
      $personPapers{$actualPerson}{$pap}++;
      $personPaperCount{$actualPerson}++;
      $count++; last if ($count > $countThreshold);
    }
  }
  foreach my $person (sort { $personPaperCount{$b} <=> $personPaperCount{$a} } keys %personPapers) {
    my @papers = keys %{ $personPapers{$person} };
    my $papers = join", ", @papers;
# UNCOMMENT FROM person_papers
    print PPF qq($person\t$papers\n);
  }
  close (PTW) or die "Cannot close $papers_type_who_file : $!";
  close (PPF) or die "Cannot close $person_papers_file : $!";
#   print qq(<table>);
