...
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ...
    $row[0] =~ s///g;
    $row[2] =~ s///g;
    if ($row[3] eq 'valid') { $hash{aid}{$row[1]}++; }
...
#   my ($lastname, $two_key, $curator) = @_;
  ...
  if ($show_all) {			# show different menus depending on whether showing all paper or only unverified  2006 02 22
      ...
      print "<FORM METHOD=\"POST\" ACTION=\"http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/confirm_paper.cgi\"><INPUT TYPE=HIDDEN NAME=curator_name VALUE=\"$curator\">\n";
...
Thank you very much for helping associate your C. elegans publications and
abstracts.

Your updated profile will show in our next upload (approx 6 weeks) in our Home Page
http://www.wormbase.org/ under Author/Person search
