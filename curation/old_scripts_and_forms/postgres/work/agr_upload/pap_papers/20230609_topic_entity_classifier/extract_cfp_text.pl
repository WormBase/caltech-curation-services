...
# Check what the data is for each datatype in cfp_ tables, sorted by frequency, to try to get rid of things that mean FALSE POSITIVE or negative.
# 2023 10 11
...
sub extractCfpText {
  foreach my $datatype (sort keys %datatypesAfpCfp) {
    $result = $dbh->prepare( "SELECT cfp_$datatypesAfpCfp{$datatype}, COUNT(*) AS count FROM cfp_$datatypesAfpCfp{$datatype} GROUP BY cfp_$datatypesAfpCfp{$datatype} ORDER BY count DESC" );
    $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
    while (my @row = $result->fetchrow) {
      $row[0] =~ s/\n/ /g;
      print qq($datatype\t$row[1]\t$row[0]\n);
} } }
