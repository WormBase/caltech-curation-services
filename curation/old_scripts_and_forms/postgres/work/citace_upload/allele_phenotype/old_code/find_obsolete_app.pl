...
#
# Update for app_ tables.  2008 03 04
...
my %terms;
my $result = $conn->exec( "SELECT * FROM app_term ORDER BY app_timestamp ;" );
while (my @row = $result->fetchrow) { 
#   my $key = $row[0] . '_' . $row[1] . '_' . $row[2];
  my $key = $row[0];
  if ($row[1]) { if ($row[1] =~ m/(WBPhenotype\d+)/) {
...
foreach my $key (sort keys %{ $terms{key} }) {
  ...
  if ($phenotypeTerms{obs}{$term}) { 
    $body .= "In app_term Postgres Database ID $key has term $term which is obsolete\n"; } }
...
foreach my $term (sort keys %{ $terms{term}} ) {
  unless ($phenotypeTerms{exist}{$term}) {
    if ($phenotypeTerms{alt_id}{$term}) {
        ...
        my $result = $conn->exec( "SELECT joinkey FROM app_term WHERE app_term ~ '$term';" );
...
#         print "Term $term is now $new_term, replacing annotations $annots in Postgres with $new_term ($phenotypeTerms{exist}{$new_term})\n";
        ...
        my $command = "UPDATE app_term SET app_term = '$new_term' WHERE app_term ~ '$term';";
#         print "$command\n";
