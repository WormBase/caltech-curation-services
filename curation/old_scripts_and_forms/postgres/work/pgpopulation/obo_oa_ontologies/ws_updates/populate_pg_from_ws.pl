...
# gin_molname, gin_sequence, gin_protein, gin_cds.  
...
#
# trigger updates only when Wen has sent a new WSGene.ace file to her directory in the last 24 hours. 
# start processing gene comparator data into gic_ tables.   2022 02 10
#
# updated gic_ tables to be more similar with gic_dead instead of gic_status (has no data because they're all live).
# uses gic_cds instead of gic_seqprot because cds are not like gin_seqname nor gin_sequence   also the WBCDS
# proteins mapping to the corresponding cds look the same.  The paper editor pap_gene autocomplete autocompletes
# on gin_wbgene gin_locus gin_synonyms gin_sequence gin_seqname, so we don't need a gic_protein because it's not used.
# 2022 02 11
...
# Previously was adding the date to a 'latestDate' file.  Wen is now generating this, so check the file timestamp of the WBGene.ace file instead.
...
#   &processGeneCds();
# #   &processExprCluster();					# Karen doesn't want this anymore, Wen agrees.  2022 02 10
...
my $timestamp = &getPgDate();
my $datefile = '/home2/acedb/cron/dump_from_ws/files/WSGene.ace';

if (-M $datefile < 1) {           # datefile modified in the last 24 hours
  &processGeneComparator();
  &processGeneCds();
#   &processExprCluster();					# Karen doesn't want this anymore, Wen agrees.  2022 02 10
#   &processFeature();						# get features from nightly geneace dump instead  2014 03 19
#   print "DONE\n";
...
sub processGeneComparator {
  my %comparaSpecies = ();
  $comparaSpecies{"Caenorhabditis angaria"}++;
  $comparaSpecies{"Caenorhabditis becei"}++;
  $comparaSpecies{"Caenorhabditis bovis"}++;
  $comparaSpecies{"Caenorhabditis inopinata"}++;
  $comparaSpecies{"Caenorhabditis latens"}++;
  $comparaSpecies{"Caenorhabditis nigoni"}++;
  $comparaSpecies{"Caenorhabditis panamensis"}++;
  $comparaSpecies{"Caenorhabditis parvicauda"}++;
  $comparaSpecies{"Caenorhabditis quiockensis"}++;
  $comparaSpecies{"Caenorhabditis sinica"}++;
  $comparaSpecies{"Caenorhabditis sulstoni"}++;
  $comparaSpecies{"Caenorhabditis tribulationis"}++;
  $comparaSpecies{"Caenorhabditis tropicalis"}++;
  $comparaSpecies{"Caenorhabditis uteleia"}++;
  $comparaSpecies{"Caenorhabditis waitukubuli"}++;
  $comparaSpecies{"Caenorhabditis zanzibari"}++;
  $comparaSpecies{"Oscheius tipulae"}++;
  $comparaSpecies{"Panagrellus redivivus"}++;

  my $wbgenefile = $pg_files_dir . 'gic_wbgene.pg';
  my $pubnamefile = $pg_files_dir . 'gic_pubname.pg';
  my $speciesfile = $pg_files_dir . 'gic_species.pg';
  my $deadfile = $pg_files_dir . 'gic_dead.pg';
  my $cdsfile = $pg_files_dir . 'gic_cds.pg';
  open (WBG, ">$wbgenefile") or die "Cannot create $wbgenefile : $!";
  open (PUB, ">$pubnamefile") or die "Cannot create $pubnamefile : $!";
  open (SPE, ">$speciesfile") or die "Cannot create $speciesfile : $!";
  open (DEA, ">$deadfile") or die "Cannot create $deadfile : $!";
  open (SPR, ">$cdsfile") or die "Cannot create $cdsfile : $!";

  my $count = 0;
  $/ = "";
  my $infile = $ws_files_dir . 'WSGene.ace';
#   my $infile = 'meh';			# pared down file with only relevant species Genes
...
#     print qq(SP $species\n);
#     last if ($count > 2);
    next unless ($comparaSpecies{$species});
#     print qq(ENT $count $entry END\n);
#     next;
#     last if ($count > 2);
    ...
    my $wbgene = '';
    if ($header =~ m/Gene : \"([^"]+)\"/) { $wbgene = $1; }
    next unless $wbgene;
    print WBG qq($wbgene\t$wbgene\t$timestamp\n);

    # my @tags = qw( Public_name Species Corresponding_CDS Live Dead Suppressed Split_into Merged_into );
    # decided we don't want 'Live' data to make it consistent with gin_dead  
    # (at first this will mean there's no data in the gin_dead table because there's only Live data)
    my @tags = qw( Public_name Species Corresponding_CDS Dead Suppressed Split_into Merged_into );

    foreach my $line (@lines) {
      foreach my $tag (@tags) { 
        if ($line =~ m/^$tag\t \"(.*)\"/) { $data{$tag}{$1}++; }
        elsif ($line =~ m/^$tag\t$/) { $data{$tag}{$tag}++; }
        elsif ($line =~ m/^$tag$/) { $data{$tag}{$tag}++; } } }
    foreach my $species (sort keys %{ $data{"Species"} }) {
      print SPE qq($wbgene\t$species\t$timestamp\n); }
    foreach my $publicname (sort keys %{ $data{"Public_name"} }) {
      print PUB qq($wbgene\t$publicname\t$timestamp\n); }
    foreach my $cds (sort keys %{ $data{"Corresponding_CDS"} }) {
      print SPR qq($wbgene\t$cds\t$timestamp\n); }
    my @status;
    # foreach my $live (sort keys %{ $data{"Live"} }) { push @status, $live; }
    foreach my $dead (sort keys %{ $data{"Dead"} }) { push @status, $dead; }
    foreach my $suppressed (sort keys %{ $data{"Suppressed"} }) { push @status, $suppressed; }
    foreach my $split (sort keys %{ $data{"Split_into"} }) { push @status, qq(split_into $split); }
    foreach my $merged (sort keys %{ $data{"Merged_into"} }) { push @status, qq(merged_into $merged); }
    my $status = join", ", @status;
    if ($status) { print DEA qq($wbgene\t$status\t$timestamp\n); }
  }
  $/ = "\n";

  close (WBG) or die "Cannot close $wbgenefile : $!";
  close (PUB) or die "Cannot close $pubnamefile : $!";
  close (SPE) or die "Cannot close $speciesfile : $!";
  close (DEA) or die "Cannot close $deadfile : $!";
  close (SPR) or die "Cannot close $cdsfile : $!";

  my @pgcommands;
  push @pgcommands, qq(DELETE FROM gic_wbgene; );
  push @pgcommands, qq(DELETE FROM gic_pubname; );
  push @pgcommands, qq(DELETE FROM gic_species; );
  push @pgcommands, qq(DELETE FROM gic_dead; );
  push @pgcommands, qq(DELETE FROM gic_cds; );
  push @pgcommands, qq(COPY gic_wbgene   FROM '$wbgenefile';);
  push @pgcommands, qq(COPY gic_pubname  FROM '$pubnamefile';);
  push @pgcommands, qq(COPY gic_species  FROM '$speciesfile';);
  push @pgcommands, qq(COPY gic_dead     FROM '$deadfile';);
  push @pgcommands, qq(COPY gic_cds      FROM '$cdsfile';);
