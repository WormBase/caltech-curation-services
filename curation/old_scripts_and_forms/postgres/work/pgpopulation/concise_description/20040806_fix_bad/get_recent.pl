#!/usr/bin/perl -w
#
# Find concise description paragraphs that were separated through stuff like C50H2.1
# as two sentences, when those didn't really split a sentence.  2004 08 06
...
my $result = $conn->exec( "SELECT * FROM car_ext_maindata WHERE car_ext_maindata ~ '^[0-9]';" );
