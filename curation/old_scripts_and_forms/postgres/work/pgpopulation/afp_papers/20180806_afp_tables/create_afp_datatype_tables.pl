...
# create some new tables, 3 of which are needed for afp + curation status form, 7 others needed for afp + tfp, version only needed for afp.  also creating all tables for afp / tfp for simplicity, and creating for cfp for curation status form not to break nor require recoding to separate afp and cfp requirements.  2018 10 31
...
my @users_all = ('apache', 'azurebrd', 'cecilia', '"www-data"', 'acedb', 'valerio');	# NOT ON TAZENDRA
...
my @tables = qw( rnaseq chemphen envpheno variation othervariation strain otherstrain otherantibody species othertransgene version additionalexpr );
# my @tables = qw(  additionalexpr );
