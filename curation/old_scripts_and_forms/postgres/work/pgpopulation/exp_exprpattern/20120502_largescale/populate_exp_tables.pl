...
#
# slight modification to get the latest pgid from exp_curator, run on largescale set WS232LargeScaleExpr.ace  2012 05 03
#
# large scale live run on tazendra  2012 05 08
...
my $pgid = 0;
$result = $dbh->prepare( "SELECT joinkey FROM exp_curator ORDER BY joinkey::INTEGER DESC ;" );
...
my $infile = 'WS232LargeScaleExpr.ace';
