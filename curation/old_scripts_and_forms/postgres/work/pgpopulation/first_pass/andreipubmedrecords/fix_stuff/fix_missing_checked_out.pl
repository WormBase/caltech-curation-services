...
my %pgRefTifPdf;		# hash of pmids with ref_checked_out
...
sub populatePgTifPdf {
  my $result = $conn->exec( "SELECT joinkey FROM ref_checked_out");
  while (my @row = $result->fetchrow) {
...
#   unless ($pgRefTifPdf{$pmid}) { print "$pmid\n"; }
  unless ($pgRefTifPdf{$pmid}) { 
    my $result = $conn->exec( "INSERT INTO ref_checked_out VALUES (\'$pmid\', NULL);" );
  } # unless ($pgRefTifPdf{$pmid})
} # foreach my $pmid (sort keys %pgPmid)
