...
#
# edited to add author data to pap_possible, pap_email, and pap_verified so it shows
# when querying pap_view.  (very necessary since pap_author is nearly deprecated)
# 2003 04 24

use strict;
use diagnostics;

my $infile = "whitfeld_papers.ace";
my $outfile = "whitfeld_papers.out";
...
foreach my $key (sort keys %hash) {
  ...
  foreach my $field ( keys %{ $hash{$key} }) {
    foreach my $value ( @{ $hash{$key}{$field} }) {
      ...
      if ($field eq 'author') {
        print OUT "\$result = \$conn->exec( \"INSERT INTO pap_possible VALUES ('$key', '$value');\" );\n";
        print OUT "\$result = \$conn->exec( \"INSERT INTO pap_email VALUES ('$key', '$value');\" );\n";
        print OUT "\$result = \$conn->exec( \"INSERT INTO pap_verified VALUES ('$key', '$value');\" );\n";
      }
    }
