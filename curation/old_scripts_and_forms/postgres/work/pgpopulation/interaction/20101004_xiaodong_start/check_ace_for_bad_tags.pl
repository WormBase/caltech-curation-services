...
# check data to be read in with parse_ace_interaction.pl to make sure all tags and subtags are being accounted for.
# resuls say Non_directional is not being accounted for by the parser, and there are some entries with Variation
# subtag, but no data.  2010 10 05
...
my $errfile = 'parse_ace_interaction.err';
...
my %intType;
foreach my $intType (@noeff) { $intType{$intType}++; }
foreach my $intType (@yeseff) { $intType{$intType}++; }

my %tags;
$tags{"Interaction_phenotype"}++;
$tags{"Interaction_RNAi"}++;
$tags{"Remark"}++;
$tags{"Paper"}++;
# $tags{"Interactor"}++;
# $tags{"Effector"}++;
...
my %intSubtags;
$intSubtags{"Interaction_phenotype"}++;
$intSubtags{"Interaction_RNAi"}++;
$intSubtags{"Non_directional"}++;
$intSubtags{"Effector"}++;
$intSubtags{"Effected"}++;
...
while (my $entry = <IN>) {
  ...
  foreach my $line (@lines) {
    my ($tag, @rest) = split/\s+/, $line;
    if ($tags{$tag}) { 1; }
      elsif ($intType{$tag}) { 
        my $subtag = shift @rest;
        unless ($intSubtags{$subtag}) { print "Invalid intType subtag $line\n"; }
        my $gene = shift @rest;
        unless ($gene) { print "No gene for $line\n"; }
        if (scalar(@rest) > 0) { print "EXTRA @rest stuff for $tag : $line\n"; } }
      elsif ($tag eq 'Interactor') { 
        my $gene = shift @rest;
        unless ($gene) { print "No gene for $line\n"; }
        if (scalar(@rest) > 0) {
          my ($subtag) = shift @rest;
          unless ( ($subtag eq 'Variation') || ($subtag eq 'Transgene') ) { print "Improper subtag $subtag for : $line\n"; }
          my ($object) = shift @rest;
          unless ($object) { print "No object for $line\n"; }
          if (scalar(@rest) > 0) { print "EXTRA @rest stuff for Interactor : $line\n"; } } }
      else { $badTags{$tag}{$line}++; }
  } # foreach my $line (@lines)
} # while (my $entry = <IN>)
close (IN) or die "Cannot close $infile : $!";

foreach my $tag (sort keys %badTags) {
  print "$tag\n";
  foreach my $line (sort keys %{ $badTags{$tag} }) {
    print "$tag\t$line\n";
  }
} # foreach my $tag (sort keys %badTags)
...
my %already_in;
my $id = 0;
my $result = $dbh->prepare( "SELECT * FROM int_name ORDER BY joinkey::INTEGER DESC;" );
