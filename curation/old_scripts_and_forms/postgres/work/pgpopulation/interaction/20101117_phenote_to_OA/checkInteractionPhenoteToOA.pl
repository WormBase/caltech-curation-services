...
while (my @row = $result->fetchrow) { if ($row[0]) { 
  ...
  foreach my $term (@terms) { 
    unless ($phenotypes{$term}) { print ERR "bad $term int_phenotype @row\n"; } } } }
# TODO  join with ", "
...
foreach my $table (@transgene_tables) {
  ...
  while (my @row = $result->fetchrow) { if ($row[0]) { 
    my @terms = split/\|/, $row[1];
    foreach my $term (@terms) { 
      unless ($transgenes{$term}) { print ERR "bad $term $table @row\n"; } } } } }
# TODO  join with ", "
...
foreach my $table (@gene_tables) {
  ...
  while (my @row = $result->fetchrow) { if ($row[0]) { 
    ...
    my $good_terms = join'","', @good_terms; $good_terms = '"' . $good_terms . '"';
} } }
# TODO  join with ", "
...
foreach my $table (@variation_tables) {
  ...
  while (my @row = $result->fetchrow) { if ($row[0]) { 
    ...
    foreach my $term (@terms) { 
      my $var = $term;
      if ($variations{id}{$term}) { 1; }	# this is okay
        elsif ($variations{nameToID}{$term}) { $var = $variations{nameToID}{$term}; }	# convert name to ID
        else { print ERR "bad $term $table @row\n"; } } } } }
# TODO  convert from name to ID, join with ", "
...
while (my @row = $result->fetchrow) { if ($row[1]) { 
  ...
  if ($papers{$row[1]}) { $paper = $row[1]; }
    elsif ($paperToPaper{$row[1]}) { 			# paper should be remapped
...
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) { if ($row[1]) { unless ($persons{$row[1]}) { print ERR "bad int_person @row\n"; } } }
