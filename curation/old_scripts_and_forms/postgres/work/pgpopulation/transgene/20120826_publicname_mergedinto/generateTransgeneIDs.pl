...
# generate transgene IDs for trp_name as trp_name data was moved to trp_publicname  2012 08 26
#
...
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ...
    my $objName = 'WBTransgene' . $trpId;
    push @pgcommands, "DELETE FROM trp_name WHERE joinkey = '$pgid'";
    push @pgcommands, "INSERT INTO trp_name VALUES ('$pgid', '$objName')";
    push @pgcommands, "INSERT INTO trp_name_hst VALUES ('$pgid', '$objName')";
  } # if ($row[0])
