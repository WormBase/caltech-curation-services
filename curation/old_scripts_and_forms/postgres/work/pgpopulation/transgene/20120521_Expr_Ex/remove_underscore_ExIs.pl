...
# look up transgene with name Expr.*_Ex to see if they exist in other transgene tables.  Turns out they do, so we can't just delete the name and move to synonym because then in the other table there would be no object name to associate to.  2012 05 21
...
$result = $dbh->prepare( "SELECT * FROM trp_name WHERE trp_name ~ '^Expr.*_Ex';" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
