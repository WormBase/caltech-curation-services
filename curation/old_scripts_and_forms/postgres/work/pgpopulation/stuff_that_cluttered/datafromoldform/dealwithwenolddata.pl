...
# do the same as raymond's.  manually went through wen_from_athena.txt because a
# lot of data there had fields in multiple lines without the nice ^M character
# to make joins into single lines.  manually joined the lines.
...
my $oldweninsert = "/home/postgres/work/pgpopulation/datafromoldform/oldweninsert.pl";
open (OUT, ">$oldweninsert") or die "Cannot create $oldweninsert : $!";
...
# print "ENTRY : $each_entry\n\n\n";
    ...
    $joinkey =~ s/ //g;
    $joinkey =~ s/_//g;
    $joinkey =~ s///g;	# filter out weird case pmid11161562
    $joinkey = lc($joinkey);
#     print "$joinkey\n";
    ...
    foreach my $entry_line (@individual_entries) {
      ...
      unless ($entry_line !~ /:/) {
        ...
        unless ( ($tag eq "pubID") || ($tag eq "wormID") || ($tag eq "Curator") || ($tag eq "Date") || ($tag eq "Reference") || ($tag eq "NONONO") || ($tag eq "curator") || ($tag eq "reference") ) {
          ...
          unless ($colon_array[1]) { print "WARN CAN'T SPLIT : $entry_line\n"; }
          else { # unless ($colon_array[1]) 

            if ($colon_array[0] eq "Gene Function") { print "HERE : $entry_line\n"; }

            $colon_array[1] =~ s/"\s+$//g;
            $colon_array[1] =~ s/'/\\'/g;
            $colon_array[1] =~ s/"/\\"/g;
            $colon_array[1] =~ s/@/\\@/g;
            $colon_array[1] =~ s//\n/g;	# filter out weird case
            $colon_array[1] =~ s//\n/g;
            ...
            else { $bad{$colon_array[0]}++; 
#               print "WARN : $colon_array[0] not a valid entry\n"; 
            }
...
#     print OUT "\$result = \$conn->exec( \"INSERT INTO cgc VALUES ('$_', '$1')\");\n";
  ...
  print OUT "\$result = \$conn->exec( \"INSERT INTO curator VALUES ('$_', 'Wen Chen')\");\n";
