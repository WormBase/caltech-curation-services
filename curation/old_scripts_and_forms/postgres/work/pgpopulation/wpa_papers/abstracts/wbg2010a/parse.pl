...
# to conver to good format
# Needed to change the type to 4
# Needed to separately insert Affiliation to paper.  
...
my $count = 0;
my $joinkey = 35616;			# this was the latest used joinkey at the time
while (my $entry = <IN>) {
  next unless ($entry =~ m/title\t(.*)/);
  ($title) = $entry =~ m/title\t(.*)/;
  ($volume) = $entry =~ m/volume\t(.*)/;
  ($pages) = $entry =~ m/page\t(.*)/;
  my ($affiliation) = $entry =~ m/affiliation\t(.*)/;
  ($fulltext_url) = $entry =~ m/url\t(.*)/;
  ($authors) = $entry =~ m/author\t(.*)/;
  $count++; $joinkey++;
  my $identifier = 'wbg18.1.' . &padZeros($count);
  my $line = "$identifier\t$authors\t$title\t$journal\t$volume\t$pages\t$year\t$abstract\t$genes\t$type\t$editor\t$fulltext_url";
  print "$line\n\n";
  $affiliation =~ s/\'/''/g;
  print "INSERT INTO wpa_affiliation VALUES ('000$joinkey', '$affiliation', NULL, 'valid', 'two736');\n";			# print this out to insert later, wpa_match.pm doesn't handle affiliations
# UNCOMMENT TO CREATE
#   &processWormbook( 'two736', 'wormbook', $line );		# daniel
}
close (IN) or die "Cannot close $infile : $!";
$/ = "\n";


# Needed this, Gazette_article didn't work, but should in the future.
# UPDATE wpa_type SET wpa_type = '4'  WHERE wpa_timestamp > '2010-01-20 19:40';
