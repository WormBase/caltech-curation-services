...
$/ = undef;
my (@files) = </home/postgres/work/pgpopulation/wpa_papers/abstracts/iwm2009/AbsFiles/*.txt>;
foreach my $infile (@files) {
  open (IN, "<$infile") or die "Cannot open $infile : $!";
#   my ($abs_num) = $infile =~ m/\/(\d+).*?txt$/;
  my $file = <IN>;
  ($file) = &fixHtml($file);
  ($file) = &findJunk($file);
  ...
  my $identifier = 'wm2009ab' . $abs_num;
...
#   if ($authors) { &addPg($two_number, $joinkey, 'wpa_author', $authors); }
  ...
  foreach my $num (sort {$a<=>$b} keys %authors) {
    my $author = "$num : $authors{$num}{name} ($authors{$num}{aff})";
    &addPg($two_number, $joinkey, 'wpa_author', $author); 
    print "AUTHOR $author\n"; 
  }
  print "\n";
}
...
sub addPg {
  ...
  if ($pgtable eq 'wpa_author') {
    my ($author_rank, $author, $affi) = $pm_value =~ m/(.*?) : (.*?) \((.*?)\)/;
    $auth_joinkey++; 
    if ($author =~ m/\'/) { $author =~ s/\'/''/g; }
    if ($affi =~ m/\'/) { $affi =~ s/\'/''/g; }
    $pg_command = "INSERT INTO wpa_author_index VALUES ($auth_joinkey, '$author', '$affi', 'valid', '$two_number', CURRENT_TIMESTAMP);";
    print PG "$pg_command\n"; 
# UNCOMMENT
...
# UNCOMMENT
#     $result = $dbh->do( $pg_command );
    print LOG "add author $joinkey $pgtable $auth_joinkey $author_rank $author $affi\n";
  }
