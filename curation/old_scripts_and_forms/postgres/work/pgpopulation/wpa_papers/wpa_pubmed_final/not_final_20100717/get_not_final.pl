...
my %pmids;
$result = $dbh->prepare( "SELECT * FROM pap_identifier WHERE pap_identifier ~ 'pmid' AND joinkey IN (SELECT joinkey FROM pap_pubmed_final WHERE pap_pubmed_final != 'final'); " );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) { 
  $row[1] =~ s/pmid//;
  $pmids{$row[1]}{$row[0]}++; }

# SELECT * FROM pap_pubmed_final WHERE joinkey IN (SELECT joinkey FROM pap_pubmed_final GROUP BY joinkey HAVING COUNT(joinkey) > 1);
...
#   print "getting $url\n";
  ...
  my $outfile = $directory . "/not_final_20100717/$pmid";
#   print "writing $pmid to $outfile\n";
  open (OUT, ">$outfile") or die "Cannot create $outfile : $!";
  print OUT $page;
  close (OUT) or die "Cannot close $outfile : $!";
  my $set_to = 'not_final';
  if ($page =~ m/Status=\"MEDLINE\"/) { $set_to = 'final'; print "$pmid FINAL\n"; }
