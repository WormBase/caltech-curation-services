...
#
# 0 6 * * * /home/postgres/work/pgpopulation/wpa_papers/pmid_downloads/get_new_elegans_xml.pl
...
my %pmids;
my %hash;
# $result = $dbh->prepare( "SELECT * FROM wpa_pubmed_final WHERE wpa_pubmed_final != 'final' AND wpa_pubmed_final != 'no_pmid' AND wpa_pubmed_final != 'not_final';" );
$result = $dbh->prepare( "SELECT * FROM wpa_identifier WHERE wpa_identifier ~ 'pmid' ORDER BY wpa_timestamp;" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) { 
  if ($row[3] eq 'valid') { $hash{id}{$row[0]}{$row[1]}++; }
    else { delete $hash{id}{$row[0]}{$row[1]}; }
}
...
foreach my $joinkey (sort keys %{ $hash{id} }) {
  next unless ($hash{valid}{$joinkey});
  foreach my $pmid (sort keys %{ $hash{id}{$joinkey} }) {
    $pmid =~ s/pmid//; $pmids{$pmid}{$joinkey}++; 
} }

# foreach my $pmid (sort {$a<=>$b} keys %pmids) {
#   next if ($haveXml{$pmid});
#   print "$pmid\n";
# }
# 
# __END__
