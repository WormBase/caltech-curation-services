...
my $counter = 27221;			# put highest wpa here, to create from next one

# my $infile = 'wormbook_chapters_04_27_2006.txt';
my $infile = 'wormbook_chapters_doi_04_27_2006.txt';
...
while (my $para = <IN>) {
  ...
  foreach my $line (@lines) {
    ...
    if ($line =~ m/^Type\t\"(.*)\"$/) { if ($1 eq 'BOOK_CHAPTER') { $entry{$num}{type} = $1; } else { print "ERROR $1 not the expected type\n"; } }
