...
sub populateHardcopy {
  ...
  while (my @row = $result2->fetchrow) { 
    if ($row[0]) { $row[0] =~ s///g; $row[0] =~ s/[a-zA-Z]//g; $row[0] = "\'$row[0]\'";
      if ($row[0] =~ m/15692856/) { print "HARDCOPY $row[0] $row[2]\n"; }
      $hardcopy{$row[0]} = $row[2]; }
  }
} # sub populateHardcopy
...
#   if ($entry =~ m/\nPMID/) { ($pmid_name) = $entry =~ m/PMID\s+\"([\w\.]+)\"/; $pmid_name = 'pmid' . $pmid_name; $pmid_name = "\'$pmid_name\'"; }
  ...
  if ( ($hardcopy{$pmid_name}) || ($hardcopy{$cgc_name}) ) { 
    my $ref_timestamp;
if ($pmid_name =~ m/15692856/) { print "HARD pmid_name $pmid_name hardc-pmid $hardcopy{$pmid_name}\n"; }
    if ($hardcopy{$pmid_name}) { $ref_timestamp = $hardcopy{$pmid_name}; }
    if ($hardcopy{$cgc_name}) { $ref_timestamp = $hardcopy{$cgc_name}; }
