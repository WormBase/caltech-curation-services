...
my $conn = Pg::connectdb("dbname=eimear_papers");
...
while (my $entry = <IN>) {
  ...
  $result = $conn->exec( "INSERT INTO wbp VALUES (DEFAULT, DEFAULT, CURRENT_TIMESTAMP, '1');");
  ...
  $result = $conn->exec( "INSERT INTO wbp VALUES ($joinkey, $wpa_oth, $cgc_name, $pmid_name, $med_name, $meet_name, $wbg_name, $oth_name, CURRENT_TIMESTAMP, '1');");
  print "INSERT INTO wbp_identifier VALUES ($joinkey, $wpa_oth, $cgc_name, $pmid_name, $med_name, $meet_name, $wbg_name, $oth_name, CURRENT_TIMESTAMP, '1');\n";
  
  if ($entry =~ m/\nTitle/) { 
#     my ($title) = $entry =~ m/Title\s+\"([\-\{\}\w\.]+)\"/; 
    my ($title) = $entry =~ m/Title\s+\"([^\"]+)\"/; $title = "\'$title\'";
    $result = $conn->exec( "INSERT INTO wbp_title VALUES ($joinkey, $title, CURRENT_TIMESTAMP, '1');"); 
    print "INSERT INTO wbp_title VALUES ($joinkey, $title, CURRENT_TIMESTAMP, '1');\n" }
  if ($entry =~ m/\nPublisher/) { 
#     my ($publisher) = $entry =~ m/Publisher\s+\"([\w\.]+)\"/; 
    my ($publisher) = $entry =~ m/Publisher\s+\"([^\"]+)\"/; $publisher = "\'$publisher\'";
    $result = $conn->exec( "INSERT INTO wbp_publisher VALUES ($joinkey, $publisher, CURRENT_TIMESTAMP, '1');"); }
  if ($entry =~ m/\nJournal/) { 
#     my ($journal) = $entry =~ m/Journal\s+\"([\w\.]+)\"/; 
    my ($journal) = $entry =~ m/Journal\s+\"([^\"]+)\"/; $journal = "\'$journal\'";
    $result = $conn->exec( "INSERT INTO wbp_journal VALUES ($joinkey, $journal, CURRENT_TIMESTAMP, '1');"); 
    print "INSERT INTO wbp_journal VALUES ($joinkey, $journal, CURRENT_TIMESTAMP, '1');\n" }
  if ($entry =~ m/\nVolume/) { 
#     my ($volume) = $entry =~ m/Volume\s+\"([\w\.]+)\"/; 
    my ($volume) = $entry =~ m/Volume\s+\"([^\"]+)\"/; $volume = "\'$volume\'";
    $result = $conn->exec( "INSERT INTO wbp_volume VALUES ($joinkey, $volume, CURRENT_TIMESTAMP, '1');"); }
  if ($entry =~ m/\nPage/) { 
#     my ($page) = $entry =~ m/Page\s+\"([\w\.]+)\"/; 
    my ($page) = $entry =~ m/Page\s+\"([^\"]+)\"/; $page = "\'$page\'";
    $result = $conn->exec( "INSERT INTO wbp_page VALUES ($joinkey, $page, CURRENT_TIMESTAMP, '1');"); }
  if ($entry =~ m/\nYear/) { 
#     my ($year) = $entry =~ m/Year\s+([\w\.]+)/; 
    my ($year) = $entry =~ m/Year\s+([^\"]+)/; $year = "\'$year\'";
    $result = $conn->exec( "INSERT INTO wbp_year VALUES ($joinkey, $year, CURRENT_TIMESTAMP, '1');");
    print "INSERT INTO wbp_year VALUES ($joinkey, $year, CURRENT_TIMESTAMP, '1');\n" }
  if ($entry =~ m/\nAbstract/) { 
#     my ($abstract) = $entry =~ m/Abstract\s+\"([\w\.]+)\"/; 
    my ($abstract) = $entry =~ m/Abstract\s+\"([^\"]+)\"/; $abstract = "\'$abstract\'";
    print "INSERT INTO wbp_abstract VALUES ($joinkey, $abstract, CURRENT_TIMESTAMP, '1');\n";
    $result = $conn->exec( "INSERT INTO wbp_abstract VALUES ($joinkey, $abstract, CURRENT_TIMESTAMP, '1');"); }
  if ($entry =~ m/\nAffiliation/) { 
#     my ($affiliation) = $entry =~ m/Affiliation\s+\"([\w\.]+)\"/; 
    my ($affiliation) = $entry =~ m/Affiliation\s+\"([^\"]+)\"/; $affiliation = "\'$affiliation\'";
    $result = $conn->exec( "INSERT INTO wbp_affiliation VALUES ($joinkey, $affiliation, CURRENT_TIMESTAMP, '1');"); }
...
#     my (@authors) = $entry =~ m/Author\s+\"([\w\.]+)\"/g; 
    ...
    foreach my $author (@authors) {
      ...
      $result = $conn->exec( "INSERT INTO wbp_author_idx VALUES (DEFAULT, $author, $affiliation, CURRENT_TIMESTAMP, '1');"); 
      print "INSERT INTO wbp_author_idx VALUES (DEFAULT, $author, $affiliation, CURRENT_TIMESTAMP, '1')\n";
      my $auth_joinkey = '';
      $result = $conn->exec( "SELECT author_id FROM wbp_author_idx ORDER BY entered_when DESC;");
      my @row = $result->fetchrow;
      if ($row[0]) { $row[0] =~ s///g; $auth_joinkey = $row[0]; }
      $result = $conn->exec( "INSERT INTO wbp_author VALUES ($joinkey, $auth_joinkey, NULL, $rank, NULL, CURRENT_TIMESTAMP, '1');"); 
      print "INSERT INTO wbp_author VALUES ($joinkey, $auth_joinkey, NULL, $rank, NULL, CURRENT_TIMESTAMP, '1')\n";
    }
  }
