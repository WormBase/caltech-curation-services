...
my $directory = '/home/postgres/work/pgpopulation/wpa_papers/genes_cds';
...
sub getAlreadyInPg {
  ...
  while (my @row = $result->fetchrow) {
    ...
    if ($row[3] eq 'valid') { $already_in{$row[0]}{$wbg}++; }
      else { delete $already_in{$row[0]}{$wbg}; } }
} # sub getAlreadyInPg
...
#   last if ($count > 1000);
  ...
  foreach my $word (sort keys %filtered_loci) {
    $frequency{$word}++;
    if ($gene_words{$word}{loc}) { 
      foreach my $gene (@{ $gene_words{$word}{loc} }) {
        next if ($already_in{$abs_join}{$gene});
        my $command = "INSERT INTO wpa_gene VALUES ('$abs_join', 'WBGene$gene($word)', 'Inferred_automatically\t\"update3_gene_cds.pl\"', 'valid', 'two1823', CURRENT_TIMESTAMP);";
...
#       my $result = $conn->exec( $command );
      ...
      foreach my $gene (@{ $gene_words{$word}{syn} }) {
        next if ($already_in{$abs_join}{$gene});
        my $command = "INSERT INTO wpa_gene VALUES ('$abs_join', 'WBGene$gene($word)', 'Inferred_automatically\t\"update3_gene_cds.pl\"', 'valid', 'two1823', CURRENT_TIMESTAMP);";
        print OUT "$abs_join\t$gene($word)\n";
        print OUT "$command\n";
...
sub getListFromPostgres {
  my $result = $conn->exec( "SELECT * FROM gin_synonym WHERE gin_syntype = 'locus' ORDER BY gin_timestamp;");
  while (my @row = $result->fetchrow) { push @{ $gene_words{$row[1]}{syn} }, $row[0]; }
  $result = $conn->exec( "SELECT * FROM gin_locus ORDER BY gin_timestamp;");
  while (my @row = $result->fetchrow) { push @{ $gene_words{$row[1]}{loc} }, $row[0]; }
} # sub getListFromPostgres
