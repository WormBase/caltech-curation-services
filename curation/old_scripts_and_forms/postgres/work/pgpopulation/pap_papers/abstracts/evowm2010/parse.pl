...
foreach my $entry (@entries) {
  ...
  $count++; 
#   last if ($count > 3);
  my (@auts) = $entry =~ m/Author\t(.*)/g;
  my @authors;
  foreach my $aut (@auts) {
    my ($first, $middle, $last) = split/\t/, $aut;
    my $author = '';
    if ($middle) { $author = "$first $middle $last"; } 
      else { $author = "$first $last"; } 
    $author =~ s/\s+/ /g; push @authors, $author;
  }
  my ($title) = $entry =~ m/Title\t(.*)/;
  unless ($title) { print "BAD NO TITLE $entry NO TITLE END\n"; }
  if ($title =~ m/^\s*\"\s*/) { $title =~ s/^\s*\"\s*//; }
  if ($title =~ m/\s*\"\s*$/) { $title =~ s/\s*\"\s*$//; }
  my ($abstract) = $entry =~ m/Abstract\t(.*)/;
  if ($abstract =~ m/^\s*\"\s*/) { $abstract =~ s/^\s*\"\s*//; }
  if ($abstract =~ m/\s*\"\s*$/) { $abstract =~ s/\s*\"\s*$//; }
  my $identifier = 'evowm2010_ab';
  my $journal = 'Evolutionary Biology of Caenorhabditis and Other Nematodes';
  my $year = '2010';
  my $month = '6';
