...
# 2015 07 24
#
# some extra charNameConvert values   2017 07 24
...
# my $indirectory = 'Fromdos';
my $indirectory = 'AbsFilesOrig';
my $outdirectory = 'UnaccentHtmlUTF8';		# uncomment to overwrite files
# my $outdirectory = 'UnaccentHtml';		# uncomment to overwrite files
# my $outdirectory = 'UnaccentHtmlFromdos';		# uncomment to overwrite files
  ...
  while (my $line = <IN>) {
    ...
    if ($line =~ m/&[\w]+;/) { 					# to find html characters to map them manually
      ...
      foreach my $char (@charsName) { 
        $charsName{$char}++; 
        if (defined ($charNameConvert{$char})) { $line =~ s/&$char;/$charNameConvert{$char}/g; }
    } }
#     print OUT $line;
...
sub populateCharConvert {
  ...
  $charNameConvert{'iuml'}     = 'i';
  $charNameConvert{'szlig'}    = 'B';
  $charNameConvert{'shy'}      = '';
} # sub populateCharConvert
