...
#
# modified to account for a lot of pap_species_index entries that only have one word, or don't begin with an uppercase letter.  2019 09 19
#
# skip species where the name starts with 'sp' because the Genus init + sp gets false positives like  Acrobeles sp or Rhizobium sp
# blacklist the taxon id  32644  unidentified.  2022 04 08
...
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ...
    next unless $ginit;
    next if ($species =~ m/^sp/);	# these cause false positives for species like Acrobeles sp or Rhizobium sp
...
# foreach my $key (sort keys %speciesTaxon) {
#   print qq($key\t$speciesTaxon{$key}\n);
# } 
...
my %paps;
$result = $dbh->prepare( "SELECT * FROM pap_status WHERE pap_status = 'valid' AND pap_timestamp > '2022-04-07 10:00'" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
...
my %abstracts;
$result = $dbh->prepare( "SELECT * FROM pap_abstract WHERE pap_timestamp > '2022-04-07 10:00'" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
...
my $outfile = 'abstract_species.out';
...
#       print qq($species IN $pap IN $abstract\n); 
  ...
  print OUT qq($pap\t$species\t$taxons\t$abstract\n);
