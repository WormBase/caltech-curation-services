...
my %paps;
$result = $dbh->prepare( "SELECT * FROM pap_status WHERE pap_status = 'valid' AND pap_timestamp > '2019-09-19 14:00'" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
...
my %abstracts;
$result = $dbh->prepare( "SELECT * FROM pap_abstract WHERE pap_timestamp > '2019-09-19 14:00'" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
