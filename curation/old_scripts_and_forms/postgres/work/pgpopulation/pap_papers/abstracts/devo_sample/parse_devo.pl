...
foreach my $entry (@entries) {
  ...
  $hash{'journal'} = 'Development & Evolution Meeting';
  $hash{'year'} = '2008';
  $hash{'month'} = '7';
  $hash{'status'} = 'valid';
  ...
  foreach my $author (@authors) { $author =~ s/\s+/ /g; }
  if (scalar @authors > 0) { $hash{'author'} = \@authors; }
  my $identifier = 'devevowm08abs' . &padZeros($id);
...
sub padZeros {
  ...
  if ($joinkey < 10) { $joinkey = '0000' . $joinkey; }
  elsif ($joinkey < 100) { $joinkey = '000' . $joinkey; }
  elsif ($joinkey < 1000) { $joinkey = '00' . $joinkey; }
  elsif ($joinkey < 10000) { $joinkey = '0' . $joinkey; }
  return $joinkey;
} # sub padZeros
