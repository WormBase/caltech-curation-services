...
our @ISA        = qw(Exporter);
our @EXPORT     = qw( getPubmedPage processXmlIds processArrayOfHashes filterForeign );
our $VERSION    = 1.00;
...
#   my $curator_id = 'two10877';           # pubmed, data from pubmed, override actual curator_id
  ...
  if ( ($joinkey eq '') && ($doi) ) {				# if pmid didn't match an existing paper, and there is a doi, check the doi is not in a paper
    if ($journal) { 
      if ( ($journal eq 'Genetics') ||		# only genetics papers
           ($journal eq 'MicroPubl Biol') ) {	# or micropubs
        ...
        if ($row[0]) { 			 			# doi matched wbpaper add pmid and set joinkey, else joinkey is blank
           ...
           if ($journal eq 'Genetics') {	# only genetics papers update authors
             if ($authors[0]) { &changeXmlPg('new', 'pap_author', $joinkey, $pubmed_curator_id, $timestamp, \@authors); } }	# genetics papers don't have authors, if adding a pmid, also tack on authors.
  } } } }
...
#       my $result = $dbh->prepare( " SELECT pap_order FROM $pgtable WHERE joinkey = '$joinkey' AND ( pap_identifier ~ '^pmid' OR pap_identifier ~ '^doi' OR pap_identifier ~ '^pii' ) AND pap_curator = 'two10877' " );
      ...
      my $order = 0;
      my $result = $dbh->prepare( "SELECT pap_order FROM $pgtable WHERE joinkey = '$joinkey' ORDER BY pap_order DESC " );
      $result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
      my @row = $result->fetchrow();
      ...
      foreach my $speciesTaxon (@speciesTaxons) {
        ...
        push @pgcommands, $command;
        $order++;
        $command = "INSERT INTO   $pgtable VALUES ('$joinkey', '$taxon', '$order', '$curator_id', $timestamp, 'Manually_connected\t\"$species\"')";
        push @pgcommands, $command;
