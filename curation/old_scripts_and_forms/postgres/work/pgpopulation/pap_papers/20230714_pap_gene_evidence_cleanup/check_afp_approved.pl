...
# Check what the data is for each afp_ datatype afp_approve column, to make sure we only have null / approved / rejected, and yes, that's the case.
# 2023 10 11
...
sub checkAfpApproved {
  foreach my $datatype (sort keys %datatypesAfpCfp) {
    ...
    $result = $dbh->prepare( "SELECT afp_approve, COUNT(*) AS count FROM afp_$datatypesAfpCfp{$datatype} GROUP BY afp_approve ORDER BY count DESC" );
