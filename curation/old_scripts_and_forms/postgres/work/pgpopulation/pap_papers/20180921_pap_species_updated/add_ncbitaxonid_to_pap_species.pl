...
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ...
    my $timestamp = $row[2];
    unless ($name{$id}) {
      push @pgcommands, qq(INSERT INTO pap_species_index VALUES ('$id', '$name', NULL, '$curator', '$timestamp'););
      push @pgcommands, qq(INSERT INTO h_pap_species_index VALUES ('$id', '$name', NULL, '$curator', '$timestamp'););
    }
  }
} # while (@row = $result->fetchrow)
