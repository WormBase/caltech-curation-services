...
# create ccc_ curation tables for curating sentence validity+comment, and for sentence annotation curation.  
# for both tables we will only store the most recent value.  for sentence annotation curation, they'll be
# generated as valid and the only allowed change is to make invalid, or add a separate annotation for the 
# same sentence.   2013 05 17
...
# TO RENAME tables, first delete them by changing the names here and uncommenting
#   my $table = 'curatesentence';		# will store data pipe-joined in column ccc_curatesentence, comment in ccc_comment, curator in ccc_curator
#   $result = $dbh->do( "DROP TABLE ${datatype}_$table;" );
#   $table = 'curateannotation';		# will store data pipe-joined in column ccc_curatesentence, comment in ccc_comment
#   $result = $dbh->do( "DROP TABLE ${datatype}_$table;" );

  my $table = 'sentenceclassification';		# will store data pipe-joined in column ccc_curatesentence, comment in ccc_comment, curator in ccc_curator
  $result = $dbh->do( "DROP TABLE ${datatype}_$table;" );
  $result = $dbh->do( "CREATE TABLE ${datatype}_$table (
                         ${datatype}_mod text,
                         ${datatype}_file text,
                         ${datatype}_paper text,
                         ${datatype}_section text,
                         ${datatype}_sentnum text,
  ...
  $result = $dbh->do( "CREATE INDEX ${datatype}_${table}_idx ON ${datatype}_$table USING btree (${datatype}_mod); ");

  $table = 'sentenceannotation';		# will store data pipe-joined in column ccc_curatesentence, comment in ccc_comment
  $result = $dbh->do( "DROP TABLE ${datatype}_$table;" );
  $result = $dbh->do( "CREATE TABLE ${datatype}_$table (
                         ${datatype}_mod text,
                         ${datatype}_file text,
                         ...
                         ${datatype}_section text,
                         ${datatype}_sentnum text,
                         ${datatype}_geneproduct text,
                         ${datatype}_component text,
                         ${datatype}_goterm text,
                         ${datatype}_evidencecode text,
                         ${datatype}_with text,
                         ${datatype}_alreadycurated text,
                         ${datatype}_comment text,
                         ${datatype}_valid text,
                         ${datatype}_ptgoid text,
                         ${datatype}_curator text,
                         ${datatype}_timestamp timestamp with time zone DEFAULT \"timestamp\"('now'::text)); " );
  $result = $dbh->do( "REVOKE ALL ON TABLE ${datatype}_$table FROM PUBLIC; ");
  foreach my $user (@users_select) { 
...
# my @tables = qw( geneprodindex componentindex );
