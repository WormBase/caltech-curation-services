...
# Abstracts to approve or reject with elegans match from pubmed

# for Kimberly and partly for Arun.  2009 02 13
#
# moved all this into WBPaper Editor, obsolete.  2009 02 19
...
my ($oop, $action) = &getHtmlVar($query, 'action');
if ($action eq 'submit') { &submit(); }
elsif ($action eq 'txt') { &showTxt(); }
else { &showForm(); }

sub submit {
  &printHeader('pmid approval form');
...
sub showForm {
  ...
  print "<TR><TD>pmid</TD><TD>abstract</TD><TD>textpresso</TD><TD>Approve</TD><TD>Reject</TD></TR>\n";
  my $count = 0;
  foreach my $infile (@read_pmids) {
    $/ = undef;
    open (IN, "<$infile") or die "Cannot open $infile : $!";
    ...
    my ($abstract) = $file =~ /\<AbstractText\>(.+?)\<\/AbstractText\>/i;
    my ($pmid) = $infile =~ m/(\d+)$/;
    print "<TR><TD>$pmid</TD><TD>$abstract</TD>";
    print "<TD>&nbsp;</TD>";
    print "<INPUT TYPE=HIDDEN NAME=pmid_$count VALUE=$pmid>\n";
    print "<TD><INPUT TYPE=RADIO NAME=approve_reject_$count VALUE=approve></TD>";
    print "<TD><INPUT TYPE=RADIO NAME=approve_reject_$count VALUE=reject></TD>";
    print "</TR>\n";
    $count++;
  } # foreach my $infile (@read_pmids)
  print "</TABLE>\n";
...
sub ShowPgQuery {
  ...
  <FORM METHOD="POST" ACTION="http://tazendra.caltech.edu/~postgres/cgi-bin/referenceform.cgi">
...
sub ProcessTable {
    ...
    if ($MaxEntries eq 'all') { 
        ...
        print "<FORM METHOD=\"POST\" ACTION=\"http://tazendra.caltech.edu/~postgres/cgi-bin/referenceform.cgi\">";
