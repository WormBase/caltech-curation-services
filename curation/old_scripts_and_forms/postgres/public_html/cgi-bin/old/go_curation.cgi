...
use LWP;
use lib qw( /home2/acedb/ranjana/citace_upload/go_curation/ );
...
sub checkGinTables {
  my $joinkey = shift; my $found = 0;
  ($joinkey) = $joinkey =~ m/(\d+)/;
  my $result = $conn->exec( "SELECT * FROM gin_locus WHERE joinkey = '$joinkey';" );
  my @row = $result->fetchrow; if ($row[0]) { $found++; }
  $result = $conn->exec( "SELECT * FROM gin_sequence WHERE joinkey = '$joinkey';" );
  @row = $result->fetchrow; if ($row[0]) { $found++; }
  $result = $conn->exec( "SELECT * FROM gin_protein WHERE joinkey = '$joinkey';" );
  @row = $result->fetchrow; if ($row[0]) { $found++; }
  unless ($found) { print "<FONT COLOR=red>WBGene$joinkey not in gin_locus, gin_sequence, nor gin_protein (probably merged)</FONT><BR><BR>\n"; }
} # sub checkGinTables
