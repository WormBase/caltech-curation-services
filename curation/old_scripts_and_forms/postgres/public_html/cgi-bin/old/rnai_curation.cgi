...
# Checkout papers to curate rnai only.
...
# This CGI http://tazendra.caltech.edu/~postgres/cgi-bin/rnai_curation.cgi
# 
...
# identity, which can be seen if the rnai_curation.cgi is refreshed.
# 
...
###################################################################################
#
# Adapted to curate RNAi data.  Added curator choice, pass curator_name to all pages
# added IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) to SELECT query
# to narrow down choices.   2002 12 19
#
# Changed the cgc_pgcommand and pmid_pgcommand to futher also check if the cur_rnai
# data has been curated under a cgc for pmid or pmid for cgc by checking ref_xref
# 2003 03 24
#
# Initialize some %variables since they were showing up as errors in the serverlog
# from displaying the html page.  2003 04 02
#
# Obsoleted from moving to wpa_ tables.  Put all ref_checked_out data with RNAi
# Only into wpa_rnai_curation  2005 09 19
...
our $default_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_cgc, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out, cur_rnai WHERE ref_cgc.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_cgc.joinkey AND ref_html.joinkey = ref_cgc.joinkey AND ref_tif.joinkey = ref_cgc.joinkey AND ref_lib_pdf.joinkey = ref_cgc.joinkey AND ref_checked_out.joinkey = ref_cgc.joinkey AND ref_pdf.joinkey = (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_cgc DESC;";
				# default query
# our $cgc_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_cgc, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_cgc.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_cgc.joinkey AND ref_html.joinkey = ref_cgc.joinkey AND ref_tif.joinkey = ref_cgc.joinkey AND ref_lib_pdf.joinkey = ref_cgc.joinkey AND ref_checked_out.joinkey = ref_cgc.joinkey AND ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_cgc DESC;";

our $cgc_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_cgc, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_cgc.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_cgc.joinkey AND ref_html.joinkey = ref_cgc.joinkey AND ref_tif.joinkey = ref_cgc.joinkey AND ref_lib_pdf.joinkey = ref_cgc.joinkey AND ref_checked_out.joinkey = ref_cgc.joinkey AND ( (ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL)) OR (ref_pdf.joinkey IN (SELECT ref_cgc FROM ref_xref WHERE ref_pmid IN (SELECT joinkey FROM cur_rnai WHERE joinkey ~ 'pmid' AND cur_rnai IS NOT NULL))) ) ORDER BY ref_cgc DESC;";
				# cgc query
# our $cgc_pgcommand = "SELECT ref_pdf.joinkey, ref_checked_out.ref_checked_out FROM ref_cgc, ref_checked_out WHERE ref_cgc.joinkey = ref_pdf.joinkey AND ref_checked_out.joinkey = ref_cgc.joinkey AND ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_cgc DESC;";
 				# just-as-slow query
our $pmid_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_pmid, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_pmid.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_pmid.joinkey AND ref_html.joinkey = ref_pmid.joinkey AND ref_tif.joinkey = ref_pmid.joinkey AND ref_lib_pdf.joinkey = ref_pmid.joinkey AND ref_checked_out.joinkey = ref_pmid.joinkey AND ( (ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL)) OR (ref_pdf.joinkey IN (SELECT ref_pmid FROM ref_xref WHERE ref_cgc IN (SELECT joinkey FROM cur_rnai WHERE joinkey ~ 'cgc' AND cur_rnai IS NOT NULL))) ) ORDER BY ref_pmid DESC;";

# our $pmid_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_pmid, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_pmid.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_pmid.joinkey AND ref_html.joinkey = ref_pmid.joinkey AND ref_tif.joinkey = ref_pmid.joinkey AND ref_lib_pdf.joinkey = ref_pmid.joinkey AND ref_checked_out.joinkey = ref_pmid.joinkey AND ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_pmid DESC;";
				# pmid query
our $med_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_med, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_med.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_med.joinkey AND ref_html.joinkey = ref_med.joinkey AND ref_tif.joinkey = ref_med.joinkey AND ref_lib_pdf.joinkey = ref_med.joinkey AND ref_checked_out.joinkey = ref_med.joinkey AND ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_med DESC;";
				# med query
our $agp_pgcommand = "SELECT ref_pdf.joinkey, ref_hardcopy.ref_hardcopy, ref_pdf.ref_pdf, ref_html.ref_html, ref_tif.ref_tif, ref_lib_pdf.ref_lib_pdf, ref_checked_out.ref_checked_out FROM ref_agp, ref_pdf, ref_hardcopy, ref_html, ref_tif, ref_lib_pdf, ref_checked_out WHERE ref_agp.joinkey = ref_pdf.joinkey AND ref_hardcopy.joinkey = ref_agp.joinkey AND ref_html.joinkey = ref_agp.joinkey AND ref_tif.joinkey = ref_agp.joinkey AND ref_lib_pdf.joinkey = ref_agp.joinkey AND ref_checked_out.joinkey = ref_agp.joinkey AND ref_pdf.joinkey IN (SELECT joinkey FROM cur_rnai WHERE cur_rnai IS NOT NULL) ORDER BY ref_agp DESC;";
				# agp query
...
$variables{pubID} = '';		# initialize keys for display
$variables{pdffilename} = '';
$variables{reference} = '';
$variables{rnai} = '';
$variables{comment} = '';
...
sub ChoosePaper {
  print "<FORM METHOD=\"POST\" ACTION=\"http://tazendra.caltech.edu/~postgres/cgi-bin/rnai_curation.cgi\">";
  print "<TABLE>\n";
...
#     $curator = 'Gene Function Only';
  ...
  if ($action eq 'Curate !') {
    $curator = &getCuratorFromFormAndAppendRNAi();
    &CuratePopulate();
    &displayHtmlCuration();
  } # if ($action eq 'Curate !')
...
sub getCuratorFromFormAndAppendRNAi {
  if ( $query->param("curator_name") ) { $oop = $query->param("curator_name"); }
  else { $oop = "nodatahere"; }
  $curator = &Untaint($oop);
  $curator .= ' : RNAi Only';
...
sub dealPg {
  ...
  if ($found) {					# do UPDATEs (Update !)
    my $result = $conn->exec( "UPDATE cur_rnai SET cur_rnai = '$variables{rnai}' WHERE joinkey = '$variables{pubID}';" );
    ...
    $result = $conn->exec( "INSERT INTO cur_rnai VALUES ('$variables{pubID}', '$variables{rnai}', CURRENT_TIMESTAMP);" );
...
sub getHtml {
  ...
  ($oop, $variables{rnai1}) = &getHtmlVar($query, 'rnai1');
  ($oop, $variables{rnai2}) = &getHtmlVar($query, 'rnai2');
  ($oop, $variables{rnai}) = &getHtmlVar($query, 'rnai');
  ($oop, $variables{comment}) = &getHtmlVar($query, 'comment');
  if ($variables{rnai}) { $variables{rnai} = $variables{rnai}; }
  elsif ($variables{rnai2}) { $variables{rnai} = $variables{rnai2}; }
  elsif ($variables{rnai1}) { $variables{rnai} = 'yes'; }
  else { $variables{rnai} = ''; }
  delete $variables{rnai2}; delete $variables{rnai1};
} # sub getHtml
...
sub displayVars {
  ...
  print "<FONT COLOR = green>RNAi : $variables{rnai}</FONT><BR>\n";
  print "<INPUT TYPE=\"HIDDEN\" NAME=\"rnai\" VALUE=\"$variables{rnai}\">\n";
...
sub queryPG {
  ...
  ($oop, $variables{pdffilename}) = &getHtmlVar($query, 'pdffilename');
  ($oop, $variables{reference}) = &getHtmlVar($query, 'reference');
  my $result = $conn->exec ( "SELECT * FROM cur_rnai WHERE joinkey = \'$variables{pubID}\';" );
...
sub resetForDisplay {
  ...
  $variables{rnai} = '';
  $variables{comment} = '';
#   $curator = "RNAi Only";	# assign curator name
} # sub resetForDisplay
...
<FORM METHOD="POST" ACTION="http://tazendra.caltech.edu/~postgres/cgi-bin/rnai_curation.cgi">
...
<TR>
  <TD ALIGN="right"><STRONG>RNAi :</TD>
  <TD>
    <TABLE>
      <TR>
        <TD><INPUT NAME="rnai1" TYPE="checkbox" VALUE="yes"></TD>
        <TD> </TD>
        <TD><TEXTAREA NAME="rnai2" ROWS=2 COLS=40>$variables{rnai2}</TEXTAREA></TD>
        <TD> <FONT SIZE=-1>Mail raymond\@its.caltech.edu</FONT></TD>
      </TR>
    </TABLE>
  </TD>
</TR>
...
sub PGQueryRowify {             # Add lines to reference info
  ...
  while (@row = $result->fetchrow) {
    if ($row[1]) { $variables{reference} .= "$row[1]"; }
  } # while (@row = $result->fetchrow) 
} # sub PGQueryRowify 
...
<HR>
<CENTER>Documentation <A HREF="http://tazendra.caltech.edu/~postgres/cgi-bin/rnai_curation_doc.txt" TARGET=NEW>here</A></CENTER><P>
EndOfText
} # sub PrintHeader 
