...
# files
my $data_file = '/home/postgres/public_html/cgi-bin/data/curation_data_azurebrd.txt';
my $save_file = '/home/postgres/public_html/cgi-bin/data/curation_save_azurebrd.txt';
my $html_file = '/home/postgres/public_html/cgi-bin/curation_html_azurebrd.txt';
...
sub ResetForDisplay {
  ...
  $variables{curator} = "Juancarlos Chan";	# assign curator name
} # sub ResetForDisplay 
...
sub SaveState {
  ...
  print "See all <A HREF=\"http://minerva.caltech.edu/~postgres/cgi-bin/data/curation_save_azurebrd.txt\">saved</A>.<P>\n";
...
sub OutputAndMail {
  print "See all <A HREF=\"http://minerva.caltech.edu/~postgres/cgi-bin/data/curation_data_azurebrd.txt\">results</A> in the flatfile.<P>";
...
<HEAD>
<TITLE>Juancarlos's Curation Form</TITLE>
</HEAD>
...
sub AddToPg {			# Add values to Postgres
  ...
  foreach $_ (@valuesforpostgres) {
    ...
    # insert all separate data
  for (0 .. scalar(@PGparameters)-1) {
				# for each parameter from the CGI
    unless ( ($PGparameters[$_] eq 'pubID') || ($PGparameters[$_] eq 'reference') ) {
				# exclude pubID and reference because they have
				# no matching pgsql tables
      if ($valuesforpostgres[$_] eq "") {
				# no entry, enter NULL
...
sub ShowPgQuery {
  ...
  <FORM METHOD="POST" ACTION="http://minerva.caltech.edu/~postgres/cgi-bin/curation_azurebrd.cgi">
