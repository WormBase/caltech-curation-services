#!/usr/bin/perl -w
#
# Oneify grouped data.  Same as Oneifier.cgi
#
...
sub matchWbg {				# make matches for main ace key, show in form
  ...
  print "<FORM METHOD=\"POST\" ACTION=\"http://tazendra.caltech.edu/~postgres/cgi-bin/cecilia/grouper.cgi\">\n";						# compare form, encompass wbg, aces, and wbgs
  ...
  &getWbgGroupedWith($main_wbg_key);	# get the other keys from the groupedwith table
...
sub matchAce {				# make matches for main ace key, show in form
  ...
  &getAceGroupedWith($main_ace_key);	# get the other keys from the groupedwith table
...
<HEAD>
EndOfText
  print "<TITLE>Grouper Form</TITLE>";
					# get user's name 
  print <<"EndOfText";
</HEAD>
...
<CENTER><A HREF="http://tazendra.caltech.edu/~postgres/cgi-bin/cecilia/grouper_doc.txt">Documentation</A></CENTER>
EndOfText
} # sub PrintHeader 
