...
#   my $max = $total_count;
  ...
  my $toprint = qq(<br/>Total count of people that can be emailed $total_count.  Emailing $max.<br/>);
  for my $i (1 .. $total_count) { 
    last if ($i > $max);
    ($var, my $two)    = &getHtmlVar($query, "two_$i");
    ($var, my $email)  = &getHtmlVar($query, "email_$i");
    ...
    my (@papers) = split/, /, $papers;
    my $paper = $papers[0];
    $toprint .= qq(<br/>$two\t$email\t$paper\t$papers<br/>);
    my $paperInfo = '';
    my @authors;
...
sub sendIndividualMassEmail {
  my ($paper, $two, $email, $response, $remark, $twoname, $pmid, $paperInfo) = @_;
  my $toprint = '';
  my $pgcommand = qq(INSERT INTO com_massemail VALUES ('$paper', '$two', '$email'););
  $toprint .= qq($pgcommand<br/>);
  $dbh->do( $pgcommand );
...
#   my $emailaddress = 'cgrove@caltech.edu';
  ...
  my $body = qq(Dear $twoname,\n\nIn an effort to improve WormBase's coverage of phenotypes, we are requesting your assistance to annotate nematode phenotypes from the following paper:\n\n$paperInfo\nWormBase would greatly appreciate if you, or any of the other authors, could take a moment to contribute phenotype connections using our simple web-based tool:\n\nhttps://wormbase.org/submissions/phenotype.cgi?input_1_pmid=$pmid&input_1_email=$emailaddress&input_1_person=$two\n\nIf you have any questions, comments or concerns, please let us know.\n\nThank you so much!\n\nBest regards,\n\nThe WormBase Phenotype Team);
...
#   ($var, my $datatype)       = &getHtmlVar($query, 'datatype');
  ...
  $body =~ s/\n/ /g;
  $toprint .= qq(send email to $emailaddress<br/>from $sender<br/>replyto $replyto<br/>subject $subject<br/>body $body<br/>);
...
#     com_response text,
#     com_remark text,
#     com_timestamp timestamp with time zone DEFAULT ('now'::text)::timestamp without time zone
} # sub sendIndividualMassEmail
