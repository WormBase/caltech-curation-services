...
# revert trp_strain and trp_strain_hst that had the data converted wrongly.  2019 10 08
...
# my $infile = 'update_strain_objects.pg.text';
my $infile = 'update_strain_objects.pg.out';
open (IN, "<$infile") or die "Cannot open $infile : $!";
while (my $line = <IN>) {
  chomp $line;
  next unless ($line =~ m/^UPDATE trp_strain/);
  my ($update) = $line =~ m/UPDATE (.*) SET/;
  my ($set) = $line =~ m/SET (.*) WHERE/;
  my ($where) = $line =~ m/WHERE (.*) AND/;
  my ($and) = $line =~ m/AND (.*);/;
  my $pgcommand = qq(UPDATE $update SET $and WHERE $where AND ${set};);
...
UPDATE trp_strain SET trp_strain = '"WBStrain00045724","WBStrain00045725"' WHERE joinkey = '16042' AND trp_strain = 'UD381 | UD59';
UPDATE trp_strain_hst SET trp_strain_hst = '"WBStrain00045724","WBStrain00045725"' WHERE joinkey = '16042' AND trp_strain_hst = 'UD381 | UD59';
