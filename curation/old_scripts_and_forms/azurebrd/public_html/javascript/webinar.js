...
// javascript for form at ~azurebrd/public_html/cgi-bin/forms/webinar.cgi
var cgiUrl = 'webinar.cgi';
...
YAHOO.util.Event.addListener(window, "load", function() {          // on load assign listeners
    ...
    setInputListeners();
//     checkPerturbationCount()
    resetInputWarnings()					// for some reason the warning fields don't get reset when reloading the form
//     checkRadioState();					// for other form 
...
//                 var noticeText = 'Click ' + urlLink + ' to review your publications and see which are in need of phenotype curation';
...
//       if (field === 'pmid') { asyncPmidTermInfo(); }		// pmids have terminfo on keyup as well as the checks
      ...
      value = convertDisplayToUrlFormat(value);                     // convert <newValue> to URL format by escaping characters
      var ontology = fieldsData[field]["haschecks"]
      var sUrl = cgiUrl + "?action=asyncFieldCheck&field="+field+"&input="+value+"&ontology="+ontology;
console.log('checking ' + field + ' ' + sUrl);
...
//       var amount       = fieldsData[field]["multi"];
//       for (var j = 1; j <= amount; j++ ) {				// for each specific number of that field
//         var elementId = "input_" + j + "_" + field;
//         if ( document.getElementById(elementId).value !== "" ) { count++; } } } }
//   if (count > 1) { 
// // To disable optional Inheritance Pattern and Mutation Effect, Chris didn't want this anymore 2020 05 11
// //       document.getElementById("input_1_allelenature").disabled          = "disabled"; 
// //       document.getElementById("input_1_allelefunction").disabled        = "disabled"; 
// //       document.getElementById("group_1_allelenature").style.color       = "grey"; 
// //       document.getElementById("group_1_allelefunction").style.color     = "grey"; 
//       document.getElementById("input_1_pertmultiple").disabled          = ""; 
//       document.getElementById("input_1_pertsingle").disabled            = ""; 
//       document.getElementById("group_1_pertsinglemulti").style.display  = ""; }
//     else { 
// // To enable optional Inheritance Pattern and Mutation Effect, Chris didn't want this anymore 2020 05 11
// //       document.getElementById("input_1_allelenature").disabled          = ""; 
// //       document.getElementById("input_1_allelefunction").disabled        = ""; 
// //       document.getElementById("group_1_allelenature").style.color       = ""; 
// //       document.getElementById("group_1_allelefunction").style.color     = ""; 
//       document.getElementById("input_1_pertmultiple").disabled          = "disabled"; 
//       document.getElementById("input_1_pertsingle").disabled            = "disabled"; 
//       document.getElementById("group_1_pertsinglemulti").style.display  = "none"; }
// //   console.log('count ' + count);
// } // function checkPerturbationCount()

function resetInputWarnings() {		// for some reason the warning fields don't get reset when reloading the form
					// when page loads, call this to reset warning field when input is blank, 
					// but keep it when input has value
  for (var i = 0; i < myFields.length; i++ ) {                  // for each field type
    if (fieldsData[myFields[i]]["haschecks"]) { 		// if the field has checks
      for (var j = 1; j <= fieldsData[myFields[i]]["multi"]; j++ ) {				// for each specific number of that field
        if (document.getElementById('input_' + j + '_' + myFields[i]).value === '') {
          document.getElementById('tdwarnings_' + j + '_' + myFields[i]).innerHTML     = ''; } } } } }

function setInputListeners() {
  var toAlert = '';
//   setPerturbationListeners();
...
//     toAlert += " " + field + "\n";
    ...
    for (var j = 1; j <= amount; j++ ) {				// for each specific number of that field
      if ( ( fieldsData[myFields[i]]["type"] === "text" ) || ( fieldsData[myFields[i]]["type"] === "ontology" ) ) {
        ...
        var oElement = document.getElementById("input_" + j + '_' + myFields[i]);
        YAHOO.util.Event.addListener(oElement, "keyup", function() {	// when typing in a text input element unhide next multi field
          var match        = this.id.match(/input_(\d+)_(.*)/); 
          var count        = match[1]; count = parseInt(count);
          var field        = match[2];
...
// console.log('count ' + count + ' ' + field + ' groupAmount ' + groupAmount);
...
// To show all elements in group of next count
...
// console.log('display ' + groupField + ' ' + field + ' trGroupElement ' + trGroupElement);
//                 document.getElementById(trGroupElement).style.display = ""; }
// To show only first element in group of next count
              var trGroupElement   = 'group_' + countPlusOne + '_' + field;
// console.log('display ' + groupField + ' ' + field + ' trGroupElement ' + trGroupElement);
              document.getElementById(trGroupElement).style.display = ""; } }
// FOR NOW UNTIL GENE BECOMES ONTOLOGY (maybe not, possibly some future text field will need it)
...
// console.log('displayOntology ' + groupField + ' ' + field);
            document.getElementById(trGroupElement).style.display = ""; }
        });
      } // if ( ( fieldsData[myFields[i]]["type"] === "text" ) || ( fieldsData[myFields[i]]["type"] === "ontology" ) )
