...
# compare gene description json outputs

# http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/agr/genedesc_diff_json.cgi
...
my $dataDir = '/home/azurebrd/public_html/cgi-bin/forms/agr/data/1.7';
my $releaseVersion = 1.7;
...
sub process {                   # see if anything clicked
  ...
  elsif ($action eq 'CompareFiles') {                &compareFiles(); }
...
sub compareFiles {
  ...
  my $title = 'Gene Description JSON comparision';
  &printLocalHeader($title);

  my ($var, $mod)     = &getHtmlVar($query, 'mod');
  ($var, my $date1)   = &getHtmlVar($query, 'date1');
  ($var, my $date2)   = &getHtmlVar($query, 'date2');
  ($var, my $diffField)   = &getHtmlVar($query, 'diffField');
  ($var, my $keywordFilter)   = &getHtmlVar($query, 'keywordFilter');

  my $file1 = $dataDir . '/' . $date1 . '/' . $mod . '_with_stats.json';
  my $file2 = $dataDir . '/' . $date2 . '/' . $mod . '_with_stats.json';
#   print qq(F1 $file1 F2 $file2 E\n);
  $/ = undef;
  open (IN, "<$file1") or die "Cannot open $file1 : $!";
  my $fileData = <IN>;
  close (IN) or die "Cannot close $file1 : $!";
  my $perl_scalar = $json->decode( $fileData );
  my %jsonData1   = %$perl_scalar;                     # decode the json into a hash
  open (IN, "<$file2") or die "Cannot open $file2 : $!";
  $fileData = <IN>;
  close (IN) or die "Cannot close $file2 : $!";
  $perl_scalar    = $json->decode( $fileData );
  my %jsonData2   = %$perl_scalar;                     # decode the json into a hash
  $/ = "\n";

  my $th = qq(th style="border-style: dotted");
  my $th = qq(th style="border-style: dotted");
  my $td = qq(td style="border-style: dotted");
  print qq(General Stats:<br/>\n);
  print qq(<table border="1" style="border-style: dotted">);
  my ($displayDate1) = &convertDateToDisplay($date1);
  my ($displayDate2) = &convertDateToDisplay($date2);
  print qq(<tr style="border-style: dotted"><$th>field</th><$th>$displayDate1</th><$th>$displayDate2</th></tr>\n);
  my $field = 'release_version';
  print qq(<tr><$td>$field</td><$td>$jsonData1{"overall_properties"}{$field}</td><$td>$jsonData2{"overall_properties"}{$field}</td></tr>\n);
  foreach my $field (sort keys %{ $jsonData1{"general_stats"} }) { 
#     print qq(F $field 1 $jsonData1{"general_stats"}  2 $jsonData2{"general_stats"}<br/>\n);
    print qq(<tr><$td>$field</td><$td>$jsonData1{"general_stats"}{$field}</td><$td>$jsonData2{"general_stats"}{$field}</td></tr>\n);
  } # foreach my $field (sort keys %{ $jsonData1{"general_stats"} })
  print qq(</table><br/>\n);

  my %maps;
  my $count = 0;
  foreach my $entry (@{ $jsonData1{"data"} }) { 
    my $gene = $$entry{gene_id};
#     print qq($count - $gene<br>);
    $maps{file1}{$gene}{index} = $count;
    $count++; }
  $count = 0;
  foreach my $entry (@{ $jsonData2{"data"} }) { 
    my $gene = $$entry{gene_id};
    $maps{file2}{$gene}{index} = $count;
    $count++; }

  my %createdGenes;
#   foreach my $gene (sort keys %{ $maps{file2} }) {
#     unless ($maps{file1}{$gene}) { $createdGenes{$gene}++; } }
#   my $createdGenes = join", ", sort keys %createdGenes;
#   print qq(Created Genes:<br/>\n$createdGenes<br/><br/>);
  foreach my $gene (sort keys %{ $maps{file2} }) {
    unless ($maps{file1}{$gene}) { 
      my $index2 = $maps{file2}{$gene}{index};
      my $desc2 = $jsonData2{data}[$index2]{$diffField} || '';
      if ($desc2) { $createdGenes{text}{$gene}++; }
        else {      $createdGenes{null}{$gene}++; } } }
  my $createdGenesText = join", ", sort keys %{ $createdGenes{text} };
  my $createdGenesNull = join", ", sort keys %{ $createdGenes{null} };
  unless ($createdGenesText) { $createdGenesText = 'None'; }
  unless ($createdGenesNull) { $createdGenesNull = 'None'; }
  print qq(Created Genes with Text in $diffField:<br/>\n$createdGenesText<br/><br/>);
  print qq(Created Genes with Null in $diffField:<br/>\n$createdGenesNull<br/><br/>);

  my %removedGenes;
  foreach my $gene (sort keys %{ $maps{file1} }) {
    unless ($maps{file2}{$gene}) { 
      my $index1 = $maps{file1}{$gene}{index};
      my $desc1 = $jsonData1{data}[$index1]{$diffField} || '';
      if ($desc1) { $removedGenes{text}{$gene}++; }
        else {      $removedGenes{null}{$gene}++; } } }
  my $removedGenesText = join", ", sort keys %{ $removedGenes{text} };
  my $removedGenesNull = join", ", sort keys %{ $removedGenes{null} };
  unless ($removedGenesText) { $removedGenesText = 'None'; }
  unless ($removedGenesNull) { $removedGenesNull = 'None'; }
  print qq(Removed Genes with Text in $diffField:<br/>\n$removedGenesText<br/><br/>);
  print qq(Removed Genes with Null in $diffField:<br/>\n$removedGenesNull<br/><br/>);
  

  $count = 0;
  my $toPrint = '';
  foreach my $gene (sort keys %{ $maps{file1} }) {
    next unless ($maps{file2}{$gene});
    my $index1 = $maps{file1}{$gene}{index};
    my $index2 = $maps{file2}{$gene}{index};
    my $desc1 = $jsonData1{data}[$index1]{$diffField} || '';
    my $desc2 = $jsonData2{data}[$index2]{$diffField} || '';
    my $name1 = $jsonData1{data}[$index1]{gene_name}   || '';
    my $name2 = $jsonData2{data}[$index2]{gene_name}   || '';
    my $name = $name1;
    if ($name1 ne $name2) { $name = "$name1, $name2"; }
    next unless ($desc1 || $desc2);
    if ($desc1 ne $desc2) {
      if ($keywordFilter) { next unless ( ($desc1 =~ m/$keywordFilter/) || ($desc2 =~ m/$keywordFilter/) ); }
      $toPrint.= qq(<tr><$td>$gene</td><$td>$name1</td><$td>$desc1</td><$td>$desc2</td></tr>\n);
      $count++;
    } # if ($desc1 ne $desc2)
  } # foreach my $gene (sort keys %{ $maps{file1} })

  if ($keywordFilter) { print qq(Filtering on keyword "$keywordFilter"<br/>\n); }
    else { print qq(Not filtering on a keyword<br/>\n); }
  print qq(Differences in $diffField: $count<br/>);
  print qq(<table border="1" style="border-style: dotted">);
  print qq(<tr><$th>gene</th><$th>name</th><$th>$displayDate1</th><$th>$displayDate2</th></tr>\n);
  print qq($toPrint);
  print qq(</table><br/>\n);
...
sub convertDateToDisplay {
  my ($date) = @_;
  my ($year, $month, $day) = $date =~ m/(\d{4})(\d{2})(\d{2})/; 
  my $displayDate = $year . '-' . $month . '-' . $day;
  return $displayDate;
} # sub convertDateToDisplay
...
sub frontPage {
  ...
  my $title = 'Gene Description JSON comparision';
  &printLocalHeader($title);
  print qq(<form method="post" action="genedesc_diff_json.cgi">\n);
  print qq(Select your Mod:<br/>\n);
  my $size = scalar @mods;
  print qq(<select name="mod" size="$size">\n);
  foreach my $mod (@mods) { print qq(<option value="$mod">$mod</option>\n); }
  print qq(</select><br/><br/>\n);

  my (@dates) = <${dataDir}/*>;
  foreach (@dates) { $_ =~ s|${dataDir}/||; }
  my $max_size = 10; $size = $max_size;
  if (scalar @dates < $max_size) { $size = scalar @dates; }

  print qq(Select old file:<br/>\n);
  print qq(<select name="date1" size="$size">\n);
  foreach my $date (reverse @dates) { 
    my ($displayDate) = &convertDateToDisplay($date);
    print qq(<option value="$date">$displayDate ($releaseVersion)</option>\n); }
  print qq(</select><br/><br/>\n);

  print qq(Select new file:<br/>\n);
  print qq(<select name="date2" size="$size">\n);
  foreach my $date (reverse @dates) { 
    my ($displayDate) = &convertDateToDisplay($date);
    print qq(<option value="$date">$displayDate ($releaseVersion)</option>\n); }
  print qq(</select><br/><br/>\n);

  my @fields = qw( description do_description go_description );
  $max_size = 10; $size = $max_size;
  if (scalar @fields < $max_size) { $size = scalar @fields; }
  my $defaultSelect = 'description';

  print qq(Select field to compare:<br/>\n);
  print qq(<select name="diffField" size="$size">\n);
  foreach my $field (@fields) { 
    my $selected = ''; if ($field eq $defaultSelect) { $selected = 'selected="selected"'; }
    print qq(<option value="$field" $selected>$field</option>\n); }
  print qq(</select><br/><br/>\n);

  print qq(Optional single keyword filter: (if you want multiple keywords filter, do you want it to filter requiring all keywords, or any of the keywords ?)<br/>\n);
  print qq(<input name="keywordFilter"></input><br/><br/>\n);

  print qq(<input type="submit" NAME="action" VALUE="CompareFiles"><br/>\n);
  print qq(</form>\n);
  &printFooter;
} # sub frontPage

sub printLocalHeader { my ($title) = @_; print qq(<html><head><title>$title</title></head><body>); }
# sub printFooter { print qq(</body></html>); }
...
sub convertPaperIdentifiers {
  ...
  while (my @row = $result->fetchrow) { $maps{papToData}{wbpaper}{"WBPaper$row[0]"} = "WBPaper$row[0]"; }	# prepopulate valid papers in case no prefix identifier matches  2018 07 10
  $result = $dbh->prepare( "SELECT * FROM pap_year;" ); $result->execute();
  while (my @row = $result->fetchrow) { $maps{papToData}{year}{"WBPaper$row[0]"} = $row[1]; }
...
#         $maps{papToData}{wbpaper}{"WBPaper$row[0]"}    = "WBPaper$row[0]";	# get these from pap_status instead
  ...
  my @output_types = qw( wbpaper pmid doi title year );
  my @table_header = ( "user input" ); my $table_header;
...
sub papIdToWBPaper {
  ...
  $identifier_labels{wbpaper} = 'WBPaper00050520 or simply 00050520';
...
#   print qq(Paper Identifiers (ids without prefix nor leading zeros will be treated as pmid)\n);
  ...
  foreach my $identifier_type (@identifier_types, "title", "year") {
