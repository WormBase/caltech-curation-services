...
#
# Last batch of emails sent were still going to this form while looking at 
# new paper tables (which have data not in the tables this form looks at,
# so if looking for a paper that's not in the wpa tables, redirect to the
# new paper_editor.cgi  2010 09 17
...
sub pickNumber {
  ...
  if ($row[0]) { &displayOneDataFromKey($number); }
    ...
    my $redirect_url = 'http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/paper_display.cgi?action=Search+!&data_number=' . $number . '&history=off';
    print "<script language=\"JavaScript\">window.location = '$redirect_url'</script>\n"; }	# redirect to new form  2010 09 17
#   else {
