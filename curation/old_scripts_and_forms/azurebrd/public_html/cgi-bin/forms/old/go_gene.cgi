...
# Updated to use gin_ tables instead of loci_all which is no longer updated.  2006 12 22
#
# Added ontology and with fields.
# Added email to Ranjana and Kimberly.  2007 09 13
...
my @block = qw( ontology goterm goinference paper_evidence with comment );
...
sub processValues {
  ...
  if ($theHash{genes}{html_value}) { 
      my $outfile = '/home2/acedb/ranjana/GO/go_user_submissions/listofgenes';
      my $email = 'ranjana@its.caltech.edu, vanauken@its.caltech.edu';
      my $user = 'go_gene cgi form';
      my $subject = "new entry at $outfile";
      my $body = "nt";
      ...
      my $outfile = '/home2/acedb/ranjana/GO/go_user_submissions/userannots';
      my $email = 'ranjana@its.caltech.edu, vanauken@its.caltech.edu';
      my $user = 'go_gene cgi form';
      my $subject = "new entry at $outfile";
      my $body = "nt";
...
sub printHtmlComments {
  print "* Use the <A HREF=\"http://www.wormbase.org/db/ontology/browse\" target=new>Ontology browser</A> to search for appropriate gene ontology terms, e.g. searching with the text string 'transport' returns all GO terms containing the text string 'transport'.<BR>\n";
  print "** Please consult the <A HREF=\"http://www.geneontology.org/GO.evidence.shtml\" target=new>GO evidence code guide</A>.<BR>\n";
  print "*** Please use a valid PubMed identifier for this field.<BR>\n";
  print "**** This field is mandatory for the IPI, IGI, and ISS evidence codes.  Please consult documentation on the <A HREF=\"http://www.geneontology.org/GO.format.annotation.shtml#def\" target=new>\"with\"</A> field.<BR>\n";
} # sub printHtmlComments
...
sub printHtmlSectOne {
  print "<TR><TD colspan=20>Please enter your full name and email address (REQUIRED).</TD></TR>\n";
  &printHtmlInput('name');        	
  &printHtmlInput('email');        	
} # sub printHtmlSectOne

sub printHtmlSectTwo {
  print "<TR><TD colspan=20>Provide a list of genes that you would like WormBase to annotate to the gene ontology (comma-separate if there are multiple genes) : </TD></TR>\n";
...
sub printHtmlSelectBlock {	# print html select blocks for inference type (set of three)
  ...
  for my $i (1 .. $max_columns) {
    ...
    elsif ($main_type =~ m/ontology/) { @choices = ('', 'biological process', 'cellular component', 'molecular function'); }
...
sub initializeHash {
  ...
  $theHash{"gene"}{html_field_name} = 'Gene<BR>(Use the CGC name,<BR>e.g. lin-48)';
  $theHash{"goterm"}{html_field_name} = 'GO Term';
  $theHash{"ontology"}{html_field_name} = 'Ontology *';
  $theHash{"goinference"}{html_field_name} = 'Evidence code **';
  $theHash{"paper_evidence"}{html_field_name} = 'Paper Evidence ***';
  $theHash{"with"}{html_field_name} = 'With ****';
...
sub querySanger {
  ...
  if ($locus_or_gene eq 'locus') {				# updated to use gin_ tables instead of loci_all which is no longer updated.  2006 12 22
  ...
  my @row = $result->fetchrow;
  $theHash{wbgene}{html_value} = 'WBGene' . $row[0];
