...
# Find Gene to RNAi / Transgene / Variation

# Look at parsed files with Gene to RNAi / Transgene / Variation data.  Look at
# PhenOnt.obo from CVS to get a hash (dag) with all children of phenotypes to
# give the option to extend to look at all children for matches.  2006 10 26
#
# Changes in wording.  Check if there's a phenotype in a file before printing
# output, and if not say there isn't when mentioning the file.  2006 11 07
#
# Don't show entries without phenotype.  2006 11 08
#
# Getting too much data to print in a block, getting a ``[Mon Nov 13 10:57:30
# 2006] [error] [client 131.215.52.76] (70007)The timeout specified has expired:
# ap_content_length_filter: apr_bucket_read() failed, referer:
# http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/gene_rtv.cgi'' error.
# Changed to just print on the fly.  2006 11 13
...
my %phen;
my %contained;
my $contained_flag = '1';	# default get all contained phenotypes
my $file = 'all_object_annotations';		# default look at all objects
my $directory = '/home/azurebrd/public_html/cgi-bin/forms';
my $found_flag = 0;		# there are no results by default

# my $to_print = '';
my $has_phenotype = 0;

my @files = qw( allele_phenotypes allele_not_phenotypes transgene_phenotypes transgene_not_phenotypes rnai_phenotypes rnai_not_phenotypes );

my $text = 'cgi';
(my $oop, $text) = &getHtmlVar($query, 'text');
unless ($text) { $text = 'cgi'; }

if ($text eq 'text') {
    print "Content-type: text/plain\n\n"; &process(); }
  else { 
    print "Content-type: text/html\n\n";
    my $title = 'Gene to RNAi / Transgene / Variation form';
...
sub process {			# see if anything clicked
  ...
  if ($action eq 'Query Phenotype') { &queryPhen(); } 
  elsif ($action eq 'Query Sequence') { &querySeqGene('seq'); } 
  elsif ($action eq 'Query Gene') { &querySeqGene('gene'); } 

  else { &displayMenu(); }
} # sub process

sub displayMenu {
  print "<FORM METHOD=POST ACTION=\"gene_rtv.cgi\">\n";
  print "<CENTER>\n";
  print "<TABLE border=1>\n";
  print "<TR><TD colspan=3><FONT COLOR=blue>SELECT *</FONT></TD></TR>\n";
  print "<TR><TD>Which objects to look at : </TD><TD colspan=2>\n";
  print "<SELECT NAME=\"file\"><OPTION SELECTED>all_object_annotations</OPTION>\n";
  foreach my $file (@files) { print "<OPTION>$file</OPTION>\n"; }
  print "</SELECT></TD></TR>\n";
  print "<TR><TD colspan=2><FONT SIZE=-2>* Selecting a ``not'' file will output phenotypes communicated<BR>as not being associated with an allele, RNAi, or transgene object.</FONT></TD></TR>\n";
  print "</TABLE>\n";
  print "<FONT SIZE=+2 COLOR=red>AND</FONT>\n";
  print "<TABLE border=1>\n";
  print "<TR><TD>Which Sequence to look at :</TD><TD><INPUT NAME=\"sequence\"></TD>\n";
  print "<TD><INPUT TYPE=\"submit\" NAME=\"action\" VALUE=\"Query Sequence\"> e.g. M02B7.3</TD></TR>\n";
  print "<TR><TD colspan=3><FONT COLOR=blue>OR</FONT></TD></TR>\n";
  print "<TR><TD>Which locus to look at :</TD><TD><INPUT NAME=\"gene\"></TD>\n";
  print "<TD><INPUT TYPE=\"submit\" NAME=\"action\" VALUE=\"Query Gene\"> e.g. osm-3</TD></TR>\n";
  print "<TR><TD colspan=3><FONT COLOR=blue>OR</FONT></TD></TR>\n";
  &readObo('slim');
  print "<TR><TD>Which high-level WBPhenotype to look at :</TD><TD colspan=2><SELECT NAME=\"id\">\n";
#   foreach my $id (sort { $phen{$a}{name} <=> $phen{$b}{name} } keys %phen)
#   foreach my $id (sort keys %phen) {
#     next unless ($phen{$id}{name});
#     my $name = $phen{$id}{name};
#     print "<OPTION VALUE=\"$id\">$id $name</OPTION>\n";
#   }
  my %sort; 
  foreach my $id (sort keys %phen) { $sort{$phen{$id}{name}} = $id; }
  foreach my $name (sort keys %sort) {
    next unless ($name);
    my $id = $sort{$name};
    print "<OPTION VALUE=\"$id\">$id $name</OPTION>\n";
  }
  print "</SELECT><BR>\n";
  print "Extend search to children of this phenotype : <INPUT TYPE=checkbox NAME=contained_flag VALUE=yes CHECKED></TD></TR>\n"; 
  print "<TR><TD colspan=3 ALIGN=center><INPUT TYPE=\"submit\" NAME=\"action\" VALUE=\"Query Phenotype\"></TD></TR>\n";
  print "</TABLE>\n";
  print "</FORM>\n";
} # sub displayMenu
...
sub querySeqGene {
  ...
  &readObo('full');
  my $val = '';
  ($oop, $file) = &getHtmlVar($query, 'file');
  if ($seq_gene eq 'seq') {
    ($oop, $val) = &getHtmlVar($query, 'sequence'); 
    unless ($val) { print "No Sequence selected, click ``back'' and select a sequence<BR>\n"; last; }
    if ($text ne 'text') { print "<A HREF=http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/gene_rtv.cgi?action=Query+Sequence&text=text&file=$file&sequence=$val TARGET=NEW>text version</A><BR>\n"; } }
  elsif ($seq_gene eq 'gene') {
    ($oop, $val) = &getHtmlVar($query, 'gene');
    unless ($val) { print "No Gene selected, click ``back'' and select a gene<BR>\n"; last; } 
    if ($text ne 'text') { print "<A HREF=http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/gene_rtv.cgi?action=Query+Gene&text=text&file=$file&gene=$val TARGET=NEW>text version</A><BR>\n"; } }
  else { $val = 'novalueforseqorgene'; }
  &getFileLines($seq_gene, $val);
  if ($text ne 'text') { unless ($found_flag) { print "<FONT COLOR=brown>There are currently no annotations in WormBase.</FONT><BR>\n"; } }
} # sub querySeqGene

sub queryPhen {
  &readObo('full');
  my $id = '0000886';
  ($oop, $contained_flag) = &getHtmlVar($query, 'contained_flag');
  ($oop, $file) = &getHtmlVar($query, 'file');
  ($oop, $id) = &getHtmlVar($query, 'id');
  if ($text ne 'text') { print "<A HREF=http://tazendra.caltech.edu/~azurebrd/cgi-bin/forms/gene_rtv.cgi?action=Query+Phenotype&text=text&file=$file&id=$id&contained_flag=$contained_flag TARGET=NEW>text version</A><BR>\n"; }
  unless ($id) { 
    $id = '0000866';
    print "No Phenotype selected, using WBPhenotype$id by default\n"; }
  &getFileLines('phen', $id);
  if ($text ne 'text') { unless ($found_flag) { print "<FONT COLOR=brown>There are currently no annotations in WormBase.</FONT><BR>\n"; } }
} # sub queryPhen

sub getFileLines {
  my ($search_type, $search_value) = @_;
  if ($text ne 'text') {
    if ($search_type eq 'phen') { print "Querying WBPhenotype$search_value<BR>\n"; }
    elsif ($search_type eq 'seq') { print "Querying Sequence $search_value<BR>\n"; }
    elsif ($search_type eq 'gene') { print "Querying 3-letter Gene $search_value<BR>\n"; } }
  if ($file eq 'all_object_annotations') {
      foreach my $file ( @files ) { &getContainedLines($file, $search_type, $search_value); } }
    else { &getContainedLines($file, $search_type, $search_value); }
} # sub getFileLines

sub getContainedLines {
  my ($file, $search_type, $search_value) = @_;
  if ($text ne 'text') { print "<FONT COLOR=brown>OBJECT $file</FONT><BR>\n"; }
    else { print "OBJECT $file\n"; }
#   $to_print = '';
  $has_phenotype = 0;
  if ($contained_flag) { 
      if ($search_type eq 'phen') { 
          &getContained($search_value); 
          foreach my $inid (sort keys %contained) { &getLines($file, $search_type, $inid); } }
        else { &getLines($file, $search_type, $search_value); } }
    else { &getLines($file, $search_type, $search_value); }
  if ($text ne 'text') {
      unless ($has_phenotype) { 
        if ($file =~ m/_not_/) { print "<FONT COLOR=brown>OBJECT $file has no data curated for Phenotypes that were scored, but not observed</FONT><BR>\n"; }
          else { print "<FONT COLOR=brown>OBJECT $file has no data curated for Phenotype</FONT><BR>\n"; } } }
    else {
      unless ($has_phenotype) { 
          if ($file =~ m/_not_/) { print "OBJECT $file has no data curated for Phenotypes that were scored, but not observed\n"; }
            else { print "OBJECT $file has no data curated for Phenotype\n"; } } }
#   print $to_print;					# not working from too much output  2006 11 13
} # sub getContainedLines

sub getLines {
  my ($file, $search_type, $search_value) = @_;
  my $loc = '/home/azurebrd/public_html/sanger/gene_rnai_variation_transgene/' .  $file;
  open (IN, "<$loc") or die "Cannot open $loc : $!";
  while (my $line = <IN>) {
    chomp $line;
    if ($search_type eq 'phen') {
      if ($line =~ m/WBPhenotype$search_value/) { 
        if ($text ne 'text') {
#             my (@stuff) = split/\t/, $line; 
#             $stuff[1] = '<FONT COLOR=red>' .  $stuff[1] . '</FONT>'; 
#             $stuff[2] = '<FONT COLOR=red>' .  $stuff[2] . '</FONT>'; 
#             $line = join"\t", @stuff;
#             print "<FONT COLOR='blue'>$file</FONT>\t$line\t<FONT COLOR='green'>$phen{$search_value}{name}</FONT><BR>\n"; 
            my ($wbgene, $locus, $protein, $target, $phenotype) = split/\t/, $line;
            if ($phenotype) { $has_phenotype++; }
            next unless ($phenotype);				# don't show entries without phenotype 2006 11 08
            if ($file =~ m/allele/) { $found_flag++; print "$file <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/variation?name=$target;class=Variation\"><FONT COLOR=black>$target</FONT></A> <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$search_value}{name}</FONT></A><BR>\n"; }
            elsif ($file =~ m/rnai/) { $found_flag++; print "$file<A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> <A TARGET=new HREF=\"http://www.wormbase.org/db/seq/rnai?name=$target\"><FONT COLOR=black>$target</FONT></A> <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$search_value}{name}</FONT></A><BR>\n"; }
            else { $found_flag++; print "$file <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> $target <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$search_value}{name}</FONT></A><BR>\n"; } }
          else { 
            my ($wbgene, $locus, $protein, $target, $phenotype) = split/\t/, $line;
            if ($phenotype) { $has_phenotype++; }
            next unless ($phenotype);				# don't show entries without phenotype 2006 11 08
            print "$file\t$line\t$phen{$search_value}{name}\n"; } } }
    elsif ( ($search_type eq 'seq') || ($search_type eq 'gene') ) {
      if ($line =~ m/\t$search_value\t/) { 
        my $phen_name = '';
        if ($line =~ m/WBPhenotype(\d+)/) { my $id = $1; if ($phen{$id}{name}) { $phen_name = $phen{$id}{name}; } }
        if ($text ne 'text') {
#             my (@stuff) = split/\t/, $line; 
#             $stuff[1] = '<FONT COLOR=red>' .  $stuff[1] . '</FONT>'; 
#             $stuff[2] = '<FONT COLOR=red>' .  $stuff[2] . '</FONT>'; 
#             $line = join"\t", @stuff;
#             print "<FONT COLOR='blue'>$file</FONT>\t$line\t<FONT COLOR='green'>$phen_name</FONT><BR>\n";
            my ($wbgene, $locus, $protein, $target, $phenotype) = split/\t/, $line;
            next unless ($phenotype);				# don't show entries without phenotype 2006 11 08
            if ($phenotype) { $has_phenotype++; }
            my ($phen_id) = $phenotype =~ m/(\d+)/; 
            if ($file =~ m/allele/) { $found_flag++; print "$file <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/variation?name=$target;class=Variation\"><FONT COLOR=black>$target</FONT></A> <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$phen_id}{name}</FONT></A><BR>\n"; }
            elsif ($file =~ m/rnai/) { $found_flag++; print "$file<A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> <A TARGET=new HREF=\"http://www.wormbase.org/db/seq/rnai?name=$target\"><FONT COLOR=black>$target</FONT></A> <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$phen_id}{name}</FONT></A><BR>\n"; }
            else { $found_flag++; print "$file <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"> <FONT COLOR=red>$locus $protein</FONT></A> $target <A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$phen_id}{name}</FONT></A><BR>\n"; } }
          else {
            my ($wbgene, $locus, $protein, $target, $phenotype) = split/\t/, $line;
            next unless ($phenotype);				# don't show entries without phenotype 2006 11 08
            if ($phenotype) { $has_phenotype++; }
            print "$file\t$line\t$phen_name\n"; } } }
    else { 1; }
  } # while (my $line = <IN>)
  close (IN) or die "Cannot close $loc : $!";
} # sub getLines

sub getContained {
  my $id = shift;
  foreach my $oid ( sort keys %{ $phen{$id}{contains} } ) {
    $contained{$oid}++;
    &getContained($oid);
  } # foreach my $oid ( sort keys %{ $phen{$id}{contains} } )
  $contained{$id}++;
...
sub readObo {
  ...
  while (my $para = <IN>) { 
    ...
    if ($para =~ m/\nis_a: WBPhenotype(\d+)/) { 
      ...
      foreach my $isa (@isa) { $phen{$isa}{contains}{$id}++; } }
  } # while (my $para = <IN>)
...
is_a: WBPhenotype0000036 ! adult_body_morphology_abnormal
...
subset: phenotype_slim_wb
is_a: WBPhenotype0000197 ! cell_induction_abnormal
...
is_a: WBPhenotype0000198 ! vulval_precursor_cell_induction_abnormal
...
is_a: WBPhenotype0000695 ! vulva_morphogenesis_abnormal
...
name: pumping_serotonin_resistant
...
alt_id: WBPhenotype0000572
synonym: "neuronal_outgrowth_abnormal" EXACT []
is_a: WBPhenotype0000816 ! neuron_development_abnormal
...
name: abnormal_cell_survival_sexually_dimorphic
...
[Term]
id: WBPhenotype0004021
name: exaggerated_body_bends
is_a: WBPhenotype0000664 ! exaggerated_body_bends
