...
#
# exact filename search wasn't working because data has subdir+filename (and old data
# has just filename) so to fix we're looking for substring filename.  2012 09 29
...
#   my $result = $dbh->prepare( "SELECT joinkey FROM ccc_tair_gene_comp_go WHERE ccc_source_file = '$just_the_file_name' ORDER BY joinkey DESC;" );
  my $result = $dbh->prepare( "SELECT joinkey FROM ccc_tair_gene_comp_go WHERE ccc_source_file ~ '$just_the_file_name' ORDER BY joinkey DESC;" );	# substring search because we want to match the file, a given filename should only exist in one subdirectory, so substring match okay.  old data was entered without subdirectory, so we can't match to subdir/filename.  2012 09 28
...
#   $/ = "\n";
  ...
  while (my @row = $result->fetchrow) {
    ...
    print "$genename\t$goid\t$paperID\t$timestamp<br />\n";
  } # while (my @row = $result->fetchrow)
...
#     if ($comment) { push @pgcommands, "INSERT INTO ccc_comment VALUES ('$sentid', '$comment', CURRENT_TIMESTAMP);"; }
    if ($comment) { push @pgcommands, "INSERT INTO ccc_tair_comment VALUES ('$sentid', '$ccc_src_file', '$comment', CURRENT_TIMESTAMP);"; }
          ...
          unless ( ($goterm eq 'not go curatable') || ($goterm eq 'scrambled sentence') || ($goterm eq 'false positive') ) {	# already curated to have protein and component  for Kimberly  2009 05 21
            ...
            unless ($badData) {
              foreach my $each_gene (@genes) {			# for each of the chosen genes from the select
                push @pgcommands, "INSERT INTO ccc_tair_gene_comp_go VALUES ('$sentid', '$ccc_src_file', '$paps', '$each_gene', '$component', '$goterm', CURRENT_TIMESTAMP);"; } } }
        }
      } } # for my $box ( 1 .. $box_count )
...
sub newReadSentence {
  ...
  $src_file =~ s|/home/azurebrd/public_html/cgi-bin/data/tair_ccc_datafiles/||;
...
CREATE TABLE ccc_tair_comment (
    joinkey integer,
    ccc_source_file text,
    ccc_tair_comment text,
    ccc_timestamp timestamp with time zone DEFAULT ('now'::text)::timestamp without time zone
);
ALTER TABLE public.ccc_tair_comment OWNER TO postgres;
REVOKE ALL ON TABLE ccc_tair_comment FROM PUBLIC;
REVOKE ALL ON TABLE ccc_tair_comment FROM postgres;
GRANT ALL ON TABLE ccc_tair_comment TO postgres;
GRANT SELECT ON TABLE ccc_tair_comment TO acedb;
GRANT ALL ON TABLE ccc_tair_comment TO apache;
GRANT ALL ON TABLE ccc_tair_comment TO azurebrd;
GRANT ALL ON TABLE ccc_tair_comment TO "www-data";
