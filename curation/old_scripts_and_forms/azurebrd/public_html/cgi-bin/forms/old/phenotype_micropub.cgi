...
#
# Added BAWM checkbox for Daniela.  2018 03 21
#
# Removed BAWM checkbox for Daniela.  2018 05 09
#
# Add generated pgids to emails sent.  2018 05 29
#
# Added app_micropublication and rna_micropublication to be toggled by defaul on submissions.  for Daniela 2018 07 02
#
# File last updated 2018 07 02.  Daniela confirms this is not used and can be archived on 2022 02 18
...
sub getAnyLaboratoryAutocomplete {
  ...
  foreach my $table (@tables) {
    ...
    while ( (my @row = $result->fetchrow()) && (scalar keys %matches < $max_results) ) {
      ...
      my $elementText = qq($row[2]$invalid <span style='font-size:.75em'>( $row[2] )</span>);
      my $matchData = qq({ "eltext": "$elementText", "name": "$row[2]", "id": "$row[2]" }); $matches{$matchData}++; }
    ...
    while ( (my @row = $result->fetchrow()) && (scalar keys %matches < $max_results) ) {
      my $invalid = ''; if ($invalidLabs{$row[0]}) { $invalid = ' INVALID'; }
      my $elementText = qq($row[0]$invalid <span style='font-size:.75em'>( $row[1] )</span>);
      my $matchData = qq({ "eltext": "$elementText", "name": "$row[0]", "id": "$row[1]" }); $matches{$matchData}++; }
...
# sub printBawmField {           printArrayEditorNested('bawm');                                                                            }
...
#   $form_data    .= &tableDisplayArray('bawm'); 
...
#   my $ip            = $query->remote_host();			# get value for current user IP, not (potentially) loaded IP 
  ...
  my $micropublication = 'Micropublication';
  my $curator          = 'WBPerson29819';
#   my $bawm             = $fields{bawm}{termidvalue}{1};
  ...
  for my $i (1 .. $amountObs) {
    ...
    if ($obsphenotypetermValue) {
      foreach my $i (sort {$a<=>$b} keys %alleles) {
        ...
        &writePgRowFieldsApp($pgidApp, $nodump, $needsreview, $micropublication, $curator, $person, $email, $unregpaper, $wbpaper, $personEvi, $unregallele, $variation, $unregtransgene, $wbtransgene, $transgenegene, $nature, $func, $penetrance, $heat_sens, $cold_sens, $genotype, $strain, $comment);
      ...
      foreach my $i (sort {$a<=>$b} keys %transgenes) {	# no nature func heat_sens cold_sens for transgenes
        ...
        &writePgRowFieldsApp($pgidApp, $nodump, $needsreview, $micropublication, $curator, $person, $email, $unregpaper, $wbpaper, $personEvi, $unregallele, $variation, $unregtransgene, $wbtransgene, $transgenegene, '', '', $penetrance, '', '', $genotype, $strain, $comment);
        &writePgField($datatype, $pgidApp, 'term', $obsphenotypetermValue);
        &writePgField($datatype, $pgidApp, 'phen_remark', $obsphenotyperemarkValue); } 
      if ($rnai) {
        ...
        &writePgRowFieldsRna($pgidRna, $highRnaiId, $nodump, $needsreview, $micropublication, $curator, $person, $email, $unregpaper, $wbpaper, $personEvi, $rnai, $rnaiSpecies, $penetrance, $genotype, $strain, $comment);
...
#       &writePgRowFields($pgidApp, $nodump, $needsreview, $micropublication, $curator, $person, $email, $unregpaper, $wbpaper, $unregallele, $variation, $nature, $func, $penetrance, $heat_sens, $cold_sens, $comment);
...
#   $email   .= ", $fields{email}{inputvalue}{1}";
  ...
  $body .= qq(Click <a href='http://${hostfqdn}/~azurebrd/cgi-bin/forms/phenotype_micropub.cgi?action=bogusSubmission&pgidsApp=$newPgidsApp&pgidsRna=$newPgidsRna&ipAddress=$ip' target='_blank' style='font-weight: bold; text-decoration: underline;'>here</a> if you did not submit this data or if you would like to retract this submission.<br/><br/>For internal use.<br/>RNAi IDs $newPgidsRna<br/>Phenotype IDs $newPgidsApp<br/><br/>\n);	# additional link to report false data
...
sub writePgRowFieldsRna {
  my ($pgid, $highRnaiId, $nodump, $needsreview, $micropublication, $curator, $communitycurator, $email, $unregpaper, $wbpaper, $personEvi, $rnai, $rnaiSpecies, $penetrance, $genotype, $strain, $comment) = @_;
  my $datatype = 'rna';
  ...
  &writePgField($datatype, $pgid, 'micropublication', $micropublication);
...
sub writePgRowFieldsApp {
  my ($pgid, $nodump, $needsreview, $micropublication, $curator, $communitycurator, $email, $unregpaper, $wbpaper, $personEvi, $unregallele, $variation, $unregtransgene, $wbtransgene, $transgenegene, $nature, $func, $penetrance, $heat_sens, $cold_sens, $genotype, $strain, $comment) = @_;
  my $datatype = 'app';
  &writePgField($datatype, $pgid, 'nodump', $nodump);
  &writePgField($datatype, $pgid, 'needsreview', $needsreview);
  &writePgField($datatype, $pgid, 'micropublication', $micropublication);
...
#   $fields{bawm}{multi}                                        = '1';
#   $fields{bawm}{type}                                         = 'checkbox';
#   $fields{bawm}{label}                                        = 'BAWM contest entry';
#   $fields{bawm}{terminfo}                                     = qq(BAWM contest entry);
...
#   tie %{ $fields{laboratory}{field} }, "Tie::IxHash";
  ...
  $fields{laboratory}{ontology_type}                      = 'Laboratory';
