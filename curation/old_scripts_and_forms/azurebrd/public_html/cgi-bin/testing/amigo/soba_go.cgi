...
sub calculateNodesAndEdges {
  ...
  my (@parentNodesOrig) = split/,/, $rootsChosen;
  ...
  my $toReturn = '';
  my (@datatypes) = ( );
  if ($rootsChosen =~ m/GO:/)          { push @datatypes, 'go';        }
  if ($rootsChosen =~ m/WBPhenotype:/) { push @datatypes, 'phenotype'; }
...
#   my ($solr_url) = &getSolrUrl($focusTermId);
  foreach my $datatype (@datatypes) {
    my @parentNodes = @parentNodesOrig;
...
#         $lbl = "$id - $lbl";                                          # node label should have full id, not stripped of :, which is required for edge title text
  ...
  #       $lbl =~ s/ /<br\/>/g;                                                # replace spaces with html linebreaks in graph for more-square boxes
        ...
        if ($id && $lbl) { 
  # print qq(P $phenotypeId ID $id E\n);
    ...
    if (!$filterForLcaFlag) {
        foreach my $annotTerm (sort keys %nodesAll) {
          ...
          foreach my $phenotype (sort keys %{ $nodesAll{$annotTerm} }) {
            my $datatypeSubdir = '';
            if ($phenotype =~ m/WBPhenotype:/) { $datatypeSubdir = 'phenotype'; }
              elsif ($phenotype =~ m/GO:/)     { $datatypeSubdir = 'go_term';   }
            my $url = "http://www.wormbase.org/species/all/$datatypeSubdir/${phenotype}#03--10";                              # URL to link to wormbase page for object
            $nodes{$phenotype}{url} = $url;
            $allLca{$phenotype}++;
        ...
        while (@annotPhenotypes) {
          ...
          my $datatypeSubdir = '';
          if ($ph1 =~ m/WBPhenotype:/) { $datatypeSubdir = 'phenotype'; }
            elsif ($ph1 =~ m/GO:/)     { $datatypeSubdir = 'go_term';   }
          my $url = "http://www.wormbase.org/species/all/$datatypeSubdir/${ph1}#03--10";                              # URL to link to wormbase page for object
          $nodes{$ph1}{url} = $url;
          my $xlabel = $ph1; 	# FIX
          $nodes{$ph1}{annot}++;
          foreach my $ph2 (@annotPhenotypes) {				# compare each annotated term node to all other annotated term nodes
            ...
            foreach my $lca (sort keys %lca) {
              my $datatypeSubdir = '';
              if ($lca =~ m/WBPhenotype:/) { $datatypeSubdir = 'phenotype'; }
                elsif ($lca =~ m/GO:/)     { $datatypeSubdir = 'go_term';   }
              $url = "http://www.wormbase.org/species/all/$datatypeSubdir/${lca}#03--10";                              # URL to link to wormbase page for object
              $allLca{$lca}++;
              $nodes{$lca}{url} = $url;
...
#   $rootNodes{'GO:0008150'}++; $rootNodes{'GO:0005575'}++; $rootNodes{'GO:0003674'}++; 
  my $maxCountAnyRoot = 0;
  foreach my $root (@rootsChosen) { 
    if ($nodes{$root}{'counts'}{'any'}) {
      if ($nodes{$root}{'counts'}{'any'} > $maxCountAnyRoot) {
        $maxCountAnyRoot = $nodes{$root}{'counts'}{'any'}; } }
    $rootNodes{$root}++; }
  if ($fakeRootFlag) { $rootNodes{'GO:0000000'}++; }
  my $rootNode = '';						# root with highest count
  my $datatypePrefix;
  if ($datatype eq 'phenotype') { $datatypePrefix = 'WBPhenotype:'; $rootNode = 'WBPhenotype:0000886'; }
    elsif ($datatype eq 'go') {   $datatypePrefix = 'GO:';          $rootNode = 'GO:0008150';          }
...
#       my $cSource = $source; $cSource =~ s/$datatypePrefix//;
#       my $cTarget = $target; $cTarget =~ s/$datatypePrefix//;
#       $nodesWithEdges{"${datatypePrefix}$cSource"}++; $nodesWithEdges{"${datatypePrefix}$cTarget"}++;
      my $cSource = $source; 
      my $cTarget = $target; 
...
#       push @annotCounts, qq($annotationCount $type);
    ...
    my $annotCounts = join"; ", @annotCounts;
    my $url = $nodes{$node}{url};
    my $diameter = $diameterMultiplier * &calcNodeWidth($nodes{$node}{'counts'}{'any'}, $maxCountAnyRoot);
...
# http://131.215.12.204/~azurebrd/cgi-bin/amigo.cgi?action=annotSummaryCytoscape&focusTermId=WBGene00000899
  ...
  my ($var, $root_ph)              = &getHtmlVar($query, 'root_ph');
  ...
  my $checked_root_bp = ''; my $checked_root_cc = ''; my $checked_root_mf = ''; my $checked_root_ph = '';
  my @roots;
#   my $datatypePrefix = '';
...
#         elsif ($datatype eq 'phenotype') {
# #           $datatypePrefix = 'WBPhenotype:';
#           push @roots, "WBPhenotype:0000886";
#           $checked_root_ph = 'checked="checked"'; } }
          push @roots, "GO:0008150"; push @roots, "GO:0005575"; push @roots, "GO:0003674";
          $checked_root_bp = 'checked="checked"'; $checked_root_cc = 'checked="checked"'; $checked_root_mf = 'checked="checked"'; 
          push @roots, "WBPhenotype:0000886";
          $checked_root_ph = 'checked="checked"'; }
      ...
      if ($root_ph) { $checked_root_ph = 'checked="checked"'; push @roots, $root_ph; } }
  my $roots = join",", @roots;
...
#   my $jsonUrl = 'http://131.215.12.204/~azurebrd/cgi-bin/amigo.cgi?action=annotSummaryJson&focusTermId=' . $focusTermId;
  my $jsonUrl = 'soba_go.cgi?action=annotSummaryJson&focusTermId=' . $focusTermId . '&datatype=' . $datatype . '&radio_iea=' . $radio_iea . '&rootsChosen=' . $roots;
...
//           var qtipContent = annotCounts + '<br/><a target="_blank" href="http://www.wormbase.org/species/all/go_term/GO:' + nodeId + '#03--10">' + nodeName + '</a>';
          var qtipContent = annotCounts + '<br/><a target="_blank" href="' + node.data('url') + '">' + nodeName + '</a>';
          node.qtip({
               position: {
                 my: 'top center',
                 at: 'bottom center'
...
//             var qtipContent = annotCounts + '<br/><a target="_blank" href="http://www.wormbase.org/species/all/go_term/GO:' + nodeId + '#03--10">' + nodeName + '</a>';
  ...
  var updatingElements = ['radio_withiea', 'radio_excludeiea', 'fakeRootFlag', 'filterForLcaFlag', 'filterLongestFlag', 'root_bp', 'root_cc', 'root_mf', 'root_ph'];
//  var updatingElements = ['radio_withiea', 'radio_excludeiea', 'fakeRootFlag', 'root_bp', 'root_cc', 'root_mf'];
  ...
  function updateElements() {
    ...
    var rootsPossible = ['root_bp', 'root_cc', 'root_mf', 'root_ph'];
    var rootsChosen = [];
    ...
    var url = 'soba_go.cgi?action=annotSummaryJson&focusTermId=$focusTermId&radio_iea=' + radioExcludeIea + '&datatype=$datatype' + '&rootsChosen=' + rootsChosenGroup + '&fakeRootFlag=' + fakeRootFlagValue + '&filterLongestFlag=' + filterLongestFlagValue + '&filterForLcaFlag=' + filterForLcaFlagValue;
//     var url = 'soba_go.cgi?action=annotSummaryJson&focusTermId=$focusTermId&radio_iea=' + radioExcludeIea + '&datatype=' + $datatype + '&rootsChosen=' + rootsChosenGroup + '&fakeRootFlag=' + fakeRootFlagValue + '&filterLongestFlag=' + filterLongestFlagValue + '&filterForLcaFlag=' + filterForLcaFlagValue;
//     alert(url); 
    var graphPNew = \$.ajax({
      url: url,
  ...
  <div id="controldiv" style="z-index: 9999; border: 1px solid #aaa; position: relative; float: left; width: 200px;">
    ...
    <form method="get" action="soba_go.cgi">
      ...
      <div id="rootschosen" style="z-index: 9999; position: relative; top: 0; left: 0; width: 200px;">
        ...
        <input type="checkbox" name="root_ph" id="root_ph" value="WBPhenotype:0000886" $checked_root_ph >Phenotype</input><br/>
      </div><br/>
