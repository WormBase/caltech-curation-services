...
# block to check for commas separating life stages in same line
# at first line
            if ($textline =~ ',') {
      	    (@textline) = split(/,/, $textline);
              foreach $textline (@textline) {
                (@temp) = split(/\s+/, $textline);
                if ($temp[0] eq "") {
                  ($a, @lifestages) = @temp; 
                } else {
                  @lifestages = @temp;
...
# block to check for commas separating life stages in same line
# at line not containing the string "Life_stage"
...
#   if (@$row) {print "ROW : @$row \n"; @qoo = (@qoo, @$row);}
#   if (@$row =~ @qoo) {
#     @qoo = (@qoo, @$row);
#     print "QOO : Lifestages      \"@$row\" \n"; 
#   }
# }
#
# for $row (@qoo) { 
...
#  }
            ...
            } # until Cell_group, Cells, Type, Localisation, Sub
              $textline = <TEXT>;
              chomp($textline);
              if ($textline = "") {
                $textline = <TEXT>;
