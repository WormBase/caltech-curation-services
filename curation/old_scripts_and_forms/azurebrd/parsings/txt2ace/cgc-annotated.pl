...
while ($textline = <TEXT>) {		# while data
  chomp($textline);
...
while ($textline = <TEMP2>) {			# while lines
  ...
  if ($textline[0] =~ "Reference") {		# if Reference line
    # ($a, @reference) = @textline;				# delete me
    # $reference = join(" ", @reference);			# delete me
    # @ref = (@ref, $reference);				# delete me
...
#      print "$textline\n";					# delete me
    ...
    if ($life_stage) { 				# unless blank
      ...
      foreach $_ (@life_stage) {		# foreach of these
        ...
        #   @lif = (@lif, $_);			# add to @lif	# delete me
        # } 	# foreach $_ (@temp)				# delete me
