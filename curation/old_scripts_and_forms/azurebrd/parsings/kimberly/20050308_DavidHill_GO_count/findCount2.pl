...
#
# Count entries with all 3 types, not just any.  2005 03 09
...
#   if ($evidence eq 'IEA') { $identifier =~ s/(\.\d+)[a-z]+/$1/; }
  ...
  $theHash{"any"}{$identifier}{$type}++;
} # while (my $line = <IN>)
close (IN) or die "Cannot close $infile : $!";

my $all_count = 0;
foreach my $identifier (sort keys %{ $theHash{any} }) {
  if ( ($theHash{any}{$identifier}{C}) && ($theHash{any}{$identifier}{P}) &&
       ($theHash{any}{$identifier}{F}) ) { $all_count++; }
} # foreach my $identifier (sort keys %{ $theHash{any}{$identifier} })
print "All : $all_count\n";
