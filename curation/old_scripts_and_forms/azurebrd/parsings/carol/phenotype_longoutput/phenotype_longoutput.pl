...
# my @files = qw( gene_allele gene_allele_not gene_rnai gene_rnai_not );
# my @files = qw( gene_allele gene_rnai );
my @files = qw( gene_allele_not gene_rnai_not );
...
&readObo('full');
  ...
  foreach my $id (sort keys %phen) {
    ...
    @higher = ();
    &getLower($id);
    foreach my $low_id (@lower) { print "ID $id DESCENDANT $low_id\n"; }
    &getHigher($id);
    foreach my $high_id (@higher) { 
      print "ID $id PARENT $high_id\n"; 
      foreach my $file (@files) {
        foreach my $term (@{ $rtv{$file}{$id} }) {
          print "$file $id $term\n"; } }
    } # foreach my $high_id (@higher)
  }
...
sub getLower {
  my $id = shift;
  return unless ($phen{$id}{name});
  my $name = $phen{$id}{name};
  foreach my $low_id ( sort keys %{ $phen{$id}{contains} } ) {
    push @lower, $low_id;
    &getLower($low_id);
  } # foreach my $low_id ( sort keys %{ $phen{$id}{contains} } )
} # sub getLower
...
sub getHigher {
  ...
  return unless ($phen{$id}{name});
  my $name = $phen{$id}{name};
  foreach my $high_id ( sort keys %{ $phen{$id}{contained} } ) {
    push @higher, $high_id;
    &getHigher($high_id);
  } # foreach my $high_id ( sort keys %{ $phen{$id}{contained} } )
} # sub getHigher

sub read_rnai_allele {
  my $directory = '/home/azurebrd/public_html/sanger/gene_rnai_variation_transgene';
  foreach my $file (@files) {
    my $open = $directory . '/' . $file;
    open (IN, "<$open") or die "Cannot open $open : $!"; 
    while (my $line = <IN>) {
      if ($line =~ m/WBPhenotype(\d+)/) { 
        my $id = $1; 
        my ($wbg, $loc, $prot, $target, $phen) = split/\t/, $line;
        push @{ $rtv{$file}{$id} }, $target; }
    } # while (my $line = <IN>)
    close (IN) or die "Cannot close $open : $!"; 
  } # foreach my $file (@files)
} # sub rnai_allele

sub readObo {
  my $directory = '/home/azurebrd/work/parsings/carol/phenotype_longoutput';
...
#   `cvs -d /var/lib/cvsroot checkout PhenOnt`;
  ...
  while (my $para = <IN>) { 
    ...
    if ($para =~ m/\nis_a: WBPhenotype(\d+)/) { 
      ...
      foreach my $isa (@isa) { 
        $phen{$id}{contained}{$isa}++; 
        $phen{$isa}{contains}{$id}++; } }
  } # while (my $para = <IN>)
...
my $file = 'all_files';		# default look at all files
my $directory = '/home/azurebrd/public_html/cgi-bin/forms';

my @files = qw( gene_allele gene_allele_not gene_transgene gene_transgene_not gene_rnai gene_rnai_not );
...
sub displayMenu {
  ...
  print "<TR><TD>Which files to look at : </TD><TD colspan=2>\n";
  print "<SELECT NAME=\"file\"><OPTION SELECTED>all_files</OPTION>\n";
  ...
  print "<TR><TD>Which three-letter gene to look at :</TD><TD><INPUT NAME=\"gene\"></TD>\n";
...
#   }
  ...
  print "</SELECT><BR>\n";
  print "Look at all phenotype contained by that phenotype : <INPUT TYPE=checkbox NAME=contained_flag VALUE=yes CHECKED></TD></TR>\n"; 
...
sub getContainedLines {
  ...
  if ($text ne 'text') { print "<FONT COLOR=brown>FILE $file</FONT><BR>\n"; }
...
#             print "<FONT COLOR='blue'>$file</FONT>\t$line\t<FONT COLOR='green'>$phen_name</FONT><BR>\n";
            my ($wbgene, $locus, $protein, $target, $phenotype) = split/\t/, $line;
            print "<FONT COLOR=blue>$file</FONT> <A TARGET=new HREF=\"http://www.wormbase.org/db/gene/gene?name=$wbgene;class=Gene\"><FONT COLOR=black>$locus</FONT> <FONT COLOR=red>$protein</FONT></A> $target<A TARGET=new HREF = \"http://www.wormbase.org/db/misc/phenotype?name=$phenotype;class=Phenotype\"><FONT COLOR=green>$phen{$search_value}{name}</FONT></A><BR>\n"; }
          else {
