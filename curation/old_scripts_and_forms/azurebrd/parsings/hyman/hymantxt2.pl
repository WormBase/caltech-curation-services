...
foreach $_ (sort keys %in) {
  print TEMP "$in{$_}\n";
  if ($ta3{$_}) { print TEMP "Phenotype \t Emb \t Remark \t \"$ta3{$_}\"\n"; }
  if ($ta4{$_}) { print TEMP "Phenotype \t Emb \t Remark \t \"$ta4{$_}\"\n"; }
  print TEMP "\n";
}
close (TEMP) or die "Cannot close $tempfile : $!";
...
position of pronuclei; aberrant splindle position.\"\n";
    ...
    } elsif ($_ =~ m/^Phenotype Class:/) { 		# zap it
    } elsif ($_ =~ m/^Progeny Phenotype Class: Embryonic lethal/) {
      print OUT "Phenotype \t Emb \t Remark \t \"0-10 F1 larvae; dead eggs on plate.\"\n";
    } elsif ($_ =~ m/^Progeny Phenotype Class:/) { 	# zap it
    } elsif ($_ =~ m/^Progeny Phenotype Description:/) { 	
      print OUT "Phenotype \t Emb \t Remark \t \"$_\"\n";
    } elsif ($_ =~ m/^Progeny Phenotype Comment:/) { 	
      print OUT "Phenotype \t Emb \t Remark \t \"$_\"\n";
    } elsif ($_ =~ m/^Progeny Phenotype Comments:/) { 	
      print OUT "Phenotype \t Emb \t Remark \t \"$_\"\n";
    } else { 
      print OUT "$_\n"; 
    }
...
foreach $_ (sort keys %in) {
  # print "$in{$_}\n";
  # if ($ta3{$_}) { print "Phenotype \t Emb \t Remark \t \"$ta3{$_}\"\n"; }
  # if ($ta4{$_}) { print "Phenotype \t Emb \t Remark \t \"$ta4{$_}\"\n"; }
  # print "\n";
}
