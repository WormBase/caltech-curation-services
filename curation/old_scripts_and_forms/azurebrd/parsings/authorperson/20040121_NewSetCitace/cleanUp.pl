...
#
# It took 50 secs to do the 2004 07 06 batch.  2004 07 06
#
# It took about 3.5 minutes to do the 2004 08 05 batch because I forgot to
# redirect to a logfile.  2004 08 05
...
$/ = "";
my $infile = 'citacePapers20040805.ace';
...
while (my $entry = <IN>) {
  ...
  foreach my $ent (@entry) { 
    ...
    elsif ($ent =~ m/^Gene/) { next; }		# added 2004 07 06
    else { $entry .= $ent . "\n"; }
  }
  $entry .= "\n";
#   $entry =~ s/\nLocus.*?//g;
