...
# this is necessary for multiontology fields as objects in these fields need to be identifyable by double quotes as well as comma separated, the same goes for fields that are multi-dropdown, but when querying and fiddling with data transplants, these double quotes need to be removed so the correct information is actually shown, queryable and available for modification. 
...
my $result = $dbh->prepare( "SELECT trp_name.trp_name, trp_strain.trp_strain, trp_synonym.trp_synonym, trp_name.joinkey FROM trp_name, trp_strain, trp_synonym WHERE trp_strain.joinkey = trp_name.joinkey AND trp_synonym.joinkey = trp_name.joinkey" ); #prepares the query, can put any query within the double quotes, joinkey stands for pgid in postgres, in the OA the joinkey is called pgid
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; #executes the query or tells you the reasons why the query might have died if it didn't work
while (my @row = $result->fetchrow) { #will grab results one row at a time until there aren't anymore, storing the results in the row variable
  if ($row[0]) { #row[0] refers to the first value from the row of results from the query, in this case that is the value from trp_name, from there, row[1] refers to the second value, trp_strain, etc. these values are being stored in pghash and associated with the trp_name value defined as row[0]. the while loop 'while(my @row...) keeps this fetch and store action going until there are no more results to fetch. 
...
while (my $line = <IN>) {
  ...
  my ($strain, $name_syn, $third, @junk) = split/\t/, $line; #takes each line and splits the line at the tabs, so everything between tabs are grouped together, in this case there needed to be at least two tabs, if a tab was missing then this script gets grumpy since there would be nothing to put in the variable named third, so if there were more than two tabs (three columns) everything else goes into junk
  print OUT "$strain\n"; #an example of how to use the outfile, again note the direction of the angle bracket above
  if ($name_syn =~ m/^\"+/) { $name_syn =~ s/^\"+//; } #removing double qoutes from the beginning of the string as denoted by ^, introduced by the spreadsheet converting to tab delimited text (if these came from postgres, you would also need to strip them, but you would have to put them back)
...
__END__
	match ^(.*?)\[ to get the name.  If no name look at 3rd column	"if no name match in column 2, match to the first ([a-z]+(Is/Ex)\d+) in this column"	"after getting the name, store all strains / synoyms in separate hashes, and join with <space>|<space>"	"match name with trp_name, get trp_strain and trp_synonym and output to tab delimited text file the 5 columns"
