...
# look at all symlinks and check if their target files exist, since many are 
# pointing to non-existing files.  e.g. 15790806_Kim05_supp.pdf exists somewhere
# else.  Move these dead symlinks to /home/acedb/daniel/dead_symlinks/ 
# recreate supplemental symlinks with :
# cd ~/public_html/daniel
# ln -s /home/acedb/daniel/Reference/pubmed/supplemental/*.pdf .
# 2010 07 27
...
my (@symlinks) = <${folder}*.pdf>;
print scalar(@symlinks) . "\n";
foreach my $symlink (@symlinks) {
  next unless (-l $symlink);		# skip non-symlinks
  my ($filename) = $symlink =~ m/\/([^\/]*?)$/;
#   next unless ($symlink =~ m/15790806/);
  my $target = readlink( $symlink );
  unless (-e $target) { 
    print "$filename DEAD TARGET $symlink\t$target\n";
    $symlink =~ s/\'/\\'/g;
    $symlink =~ s/ /\\ /g;
# uncomment to move symlinks
#     `mv $symlink /home/acedb/daniel/dead_symlinks/`;
  }
}
