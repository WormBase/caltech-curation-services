...
my %curatorToId;
$curatorToId{"Kimberly Van Auken"} = 'WBPerson1843';
...
#   next if ($qual);
  unless ($qual) { $qual = ''; }
  next if ( ($qual =~ m/NOT/) || ($qual =~ m/colocalizes_with/) || ($qual =~ m/contributes_to/) );	# changed to only skip if qual substring matches on 3 thing s for Kimberly.  2013 11 11
  ...
  if ($assignedby eq 'WormBase') {  
    ...
    else { $otherAssigners{$assignedby}++; next; }					# skip lines with $assigneby not WormBase 2013 11 13
