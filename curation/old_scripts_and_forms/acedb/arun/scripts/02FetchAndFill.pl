...
SAMPLE INPUT:  $0 pubmed.pmid elegans/ author/ pubmed
\n
";}
##############################################################################
use strict;
...
my $yeardir = "/home/arun/daniel/scripts/elegans/year/";
print "yeardir = $yeardir\n";
...
foreach my $id (@pmids) 
{   # comply with NCBI's requirement of doing more than 100 requests only between 9 pm and 5 am EST on weekdays.
    my @lc = localtime;
	if (scalar @aux > 100)
...
#    die "Error while getting ", $response->request->uri," -- ", $response->status_line, "\nAborting" unless $response-> is_success;
    ...
    slp(); # To comply with NCBI's requirement of not making more than one request in less than 3 seconds
    return $page;
}
