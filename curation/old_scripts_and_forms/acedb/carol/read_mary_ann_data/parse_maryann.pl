...
#
# Missing s/\'/''/g; for inserting data into postgres.  2007 01 18
#
# Fixed getPgDate giving the month off by one month.  Fixed any input for testing
# flag entering data unless it specifically said ``testing''.  2007 05 24
#
# Default to real instead.  2007 07 24
...
unless ($ARGV[0]) {
  ...
  print "You may also enter testing or real (to read into postgres), defaults to real\n";
...
while (my $para = <IN>) {
  ...
  if ($existing{$allele}) { 
    ...
    unless ($found_nbp) { 		# no NBP data, add into new box
      $current_box++;
      if ($flag eq 'real') { 
        if ($variation_data =~ m/\'/) { $variation_data =~ s/\'/''/g; }
...
sub replace_or_warn {
  ...
  unless ($new eq $cur) {			# if data is different (minus acedb timestamps)
    ...
    print OUT "WARN $allele HAS $current_data BEING REPLACED BY $variation_data\n";
