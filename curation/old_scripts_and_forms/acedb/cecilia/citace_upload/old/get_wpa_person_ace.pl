...
my $error_file = 'errors_in_wpa_person.ace';
...
sub populatePaperHash {
  my %invalid_papers;		# filter out paper where wpa itself is not valid 2005 10 19
  my $result = $dbh->prepare( " SELECT * FROM wpa ORDER BY wpa_timestamp; " );
  $result->execute();
  while (my @row = $result->fetchrow) {
    my $paper = 'WBPaper' . $row[0];
    if ($row[3] ne 'valid') { $invalid_papers{$paper}++; }
      else { if ($invalid_papers{$paper}) { delete $invalid_papers{$paper}; } } }
  ...
  $result->execute();
  while (my @row = $result->fetchrow) { 
    $paper_valid{valid}{$row[1]} = $row[3]; 
    $paper_valid{paper}{$row[1]} = $row[0]; }
  foreach my $author_id (sort keys %{ $paper_valid{valid} }) {
    if ($paper_valid{valid}{$author_id} eq 'valid') {
      my $paper = $paper_valid{paper}{$author_id};
      if ($paper =~ m/\s+-COMMENT.*/) { $paper =~ s/\s+-COMMENT.*//g; }
      $paper = 'WBPaper' . $paper;
      $paper_index{$author_id} = $paper; } }

  my %author_valid; my %author_index;
  $result = $dbh->prepare( "SELECT * FROM wpa_author_index ORDER BY wpa_timestamp ;");
  $result->execute();
  while (my @row = $result->fetchrow) { 
  ...
  foreach my $author_id (sort keys %{ $author_valid{valid} }) {
    if ($author_valid{valid}{$author_id} eq 'valid') {
      my $name = $author_valid{name}{$author_id};
      if ($name =~ m/\s+-COMMENT.*/) { $name =~ s/\s+-COMMENT.*//g; }
      $author_index{$author_id} = $name; } }
