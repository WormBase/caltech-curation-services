...
# get list of journals with permission for picture that have not been curated, and download pictures.  2013 09 24
...
my $result; my $pgquery;

my $directory = '/home/acedb/draciti/picture_curatable/Pictures_next_level';
...
my %journalWithPermission;		# journals with permission mapping to the URL constructor
# $journalWithPermission{"Mol Biol Cell"}               = 'Mol_Biol_Cell/Molbiolcell';
# $journalWithPermission{"Genome Biol"}                 = "";
# $journalWithPermission{"J Biol"}                      = "";
# $journalWithPermission{"Neural Dev"}                  = "";
# $journalWithPermission{"BMC Biol"}                    = "";
# $journalWithPermission{"BMC Cell Biol"}               = "";
# $journalWithPermission{"BMC Dev Biol"}                = "";
# $journalWithPermission{"BMC Evol Biol"}               = "";
# $journalWithPermission{"BMC Genet"}                   = "";
# $journalWithPermission{"BMC Genomics"}                = "";
# $journalWithPermission{"BMC Mol Biol"}                = "";
# $journalWithPermission{"BMC Neurosci"}                = "";
# $journalWithPermission{"BMC Physiol"}                 = "";
# $journalWithPermission{"Genetics"}                    = 'Genetics/Genetics';
# $journalWithPermission{"G3"}                          = "";
# $journalWithPermission{"COMP FUNCT GENOM"}            = "";
# $journalWithPermission{"PLoS Biol"}                   = "";
# $journalWithPermission{"PLoS Comput Biol"}            = "";
# $journalWithPermission{"PLoS Genet"}                  = "";
# $journalWithPermission{"PLoS Pathog"}                 = "";
# $journalWithPermission{"PLoS One"}                    = "";
# $journalWithPermission{"J Cell Biol"}                 = 'JCellBiol/JCellBiol';
# $journalWithPermission{"J Gen Physiol"}               = "";
# $journalWithPermission{"Biochem Biophys Res Commun"}  = 'BiochemBiophysResCommun/BiochemBiophysResCommun';
# $journalWithPermission{"Biochim Biophys Acta"}        = "";
# $journalWithPermission{"Cancer Cell"}                 = "";
$journalWithPermission{"Cell"}                        = 'Cell/Cell';
# $journalWithPermission{"Cell Host & Microbe"}         = "";
# $journalWithPermission{"Cell Metab"}                  = "";
# $journalWithPermission{"Cell Signal"}                 = "";
# $journalWithPermission{"Cell Stem Cell"}              = "";
# $journalWithPermission{"Chemistry and Biology"}       = "";
# $journalWithPermission{"Curr Biol"}                   = 'CurrBiol/CurrBiol';
# $journalWithPermission{"Dev Cell"}                    = 'DevCell/DevCell';
# $journalWithPermission{"Exp Cell Res"}                = "";
# $journalWithPermission{"Gene"}                        = 'Gene/Gene';
# $journalWithPermission{"Gene Expr Patterns"}          = "";
# $journalWithPermission{"Genomics"}                    = "";
# $journalWithPermission{"Immunity"}                    = "";
# $journalWithPermission{"J Mol Biol"}                  = 'JMolBiol/JMolBiol';
# $journalWithPermission{"J Struct Biol"}               = "";
# $journalWithPermission{"Mech Dev"}                    = 'MechDev/MechDev';
# $journalWithPermission{"Mol Cell"}                    = 'MolCell/MolCell';
# $journalWithPermission{"Neuron"}                      = 'Neuron/Neuron';
# $journalWithPermission{"Structure"}                   = "";
# $journalWithPermission{"Trends Parasitol"}            = "";




# These can be processed but are not in the list of journals with permission
...
# my %journals;
# my $infile = 'journal_with_permission';
...
my $journals = join"','", sort keys %journalWithPermission;
...
my %papToPmid;
$pgquery = "SELECT * FROM pap_identifier WHERE joinkey IN ('$ids') AND pap_identifier ~ 'pmid'";
...
$pgquery = "SELECT * FROM pap_journal WHERE joinkey IN ('$ids') AND pap_journal IN ('$journals')";
# print qq( $pgquery\n );
$result = $dbh->prepare( $pgquery );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    ...
    my $wbpaper = $row[0];
    my $journal = $row[1];
    my $pmid = ''; if ($papToPmid{$wbpaper}) { $pmid = $papToPmid{$wbpaper}; }
    my $has_constructor = 'no_constructor'; if ($journalWithPermission{$journal}) { $has_constructor = 'yes_constructor'; }
    my $url_constructor = '';
    if ($has_constructor) {
      ($url_constructor) = &generateUrlConstructor($wbpaper, $pmid, $journal); 
      if ($url_constructor) {
        my $paper_dir = 'output/' . $wbpaper;
        unless (-d $paper_dir) { `mkdir $paper_dir`; }
        &getUrlAndFigures($journal, $url_constructor, $paper_dir, $pmid);
        ...
        sleep(2);
        $countProcessed++;
        last if ($countProcessed > $maxToProcess);
      } else {
        print "ERR no url_constructor for $wbpaper $pmid $journal\n";
      }
    }
    print "$wbpaper\t$pmid\t$url_constructor\t$journal\n";
  } # if ($row[0])
...
sub generateUrlConstructor {
  my ($wbpaper, $pmid, $journal) = @_;
  my $xmlDir = '/home/postgres/work/pgpopulation/wpa_papers/wpa_pubmed_final/xml/';
  my $infile = $xmlDir . $pmid;
  unless (-e $infile) { 
    my $otherDir = '/home/postgres/work/pgpopulation/wpa_papers/pmid_downloads/done/';
    $infile = $otherDir . $pmid; }
  unless (-e $infile) { print "no xml found for $wbpaper $pmid\n"; return ''; }	# no xml
  $/ = undef;
  open (IN, "<$infile") or die "Cannot open $infile : $!";
  my $entry = <IN>;
  close (IN) or die "Cannot close $infile : $!";
...
#   if ($entry =~ m/<PMID[^>]*?>(.*?)<\/PMID>/) { $pmid = $1; }
  ...
  my $url_constructor = '';
  if ( ($journal eq 'Cell') || ($journal eq 'Neuron') || ($journal eq 'Curr Biol') || ($journal eq 'Dev Cell') || ($journal eq 'Mol Cell') || ($journal eq 'Biochem Biophys Res Commun') || ($journal eq 'Gene') || ($journal eq 'J Mol Biol') || ($journal eq 'Mech Dev') ) {
      unless ($doi) { print "ERR no doi for $entry\n"; }; $url_constructor = $doi; }	# this applies to all Elsevier journals
    elsif ( ($journal =~ m/^BMC/) || ($journal eq "Genome Biol") || ($journal eq "J Biol") || ($journal eq "Neural Dev") ) { unless ($pmc) { print "ERR no pmc for $entry\n"; }; $url_constructor = $pmc; }	# this applies to all BMC journals
    elsif ($vol && $issue && $medpg && $pmid) { $url_constructor = $vol . '/' . $issue . '/' . $medpg; }	# the default case
  return $url_constructor;
}

sub getUrlAndFigures {
  my ($journal, $url_constructor, $paper_dir, $pmid) = @_;
  my $url = '';
  if ($journal eq 'Development') {
    $url = 'http://dev.biologists.org/content/' . $url_constructor . '.figures-only'; }
  elsif ($journal eq 'JBC') {
    $url = 'http://www.jbc.org/content/' . $url_constructor . '.long'; }
  elsif ($journal eq 'Genes Dev') {
    $url = 'http://genesdev.cshlp.org/content/' . $url_constructor . '.full'; }
  elsif ($journal eq 'Mol Biol Cell') {
    $url = 'http://www.molbiolcell.org/cgi/content/full/' . $url_constructor; }
  elsif ($journal eq 'The company of biologists') {
    $url = 'http://jcs.biologists.org/content/' . $url_constructor . '.long'; }
  elsif ($journal eq 'J Cell Biol') {
    $url = 'http://jcb.rupress.org/content/' . $url_constructor . '.long'; }
  elsif ($journal eq 'Cell') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor;				# doesn't have the full article
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor . '?np=y'; }	# gives a 404 ?
  elsif ($journal eq 'Neuron') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Curr Biol') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Dev Cell') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Mol Cell') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Biochem Biophys Res Commun') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Gene') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'J Mol Biol') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Mech Dev') {
    $url = 'http://www.sciencedirect.com/science/article/pii/' . $url_constructor; }
  elsif ($journal eq 'Science') {
    $url = 'http://www.sciencemag.org/content/' . $url_constructor . '.long'; }
  elsif ($journal eq 'Genetics') {
    $url = 'http://www.genetics.org/content/' . $url_constructor . '.long'; }
  elsif ( ($journal =~ m/^BMC/) || ($journal eq "Genome Biol") || ($journal eq "J Biol") || ($journal eq "Neural Dev") ) {
    $url = 'http://www.ncbi.nlm.nih.gov/pmc/articles/' . $url_constructor; }
  elsif ($journal eq 'JNeurosc') {
    $url = 'http://www.jneurosci.org/content/' . $url_constructor . '.long'; }
  elsif ($journal eq 'PNAS') {
...
# print "ALL $all_figs_page FIGS\n";
  ...
  if ( ($journal eq 'Development') || ($journal eq 'JBC') || ($journal eq 'PNAS') || ($journal eq 'JNeurosc') || ($journal eq 'Genetics') || ($journal eq 'The company of biologists') || ($journal eq 'J Cell Biol'  || ($journal eq 'Science' ) ) ) {
...
#       print "$number\t$text\n";
      ...
      elsif ($journal eq 'The company o fbiologists') {
...
#         print "DOWNLOADING $image_url IMAGE\n";
  ...
  elsif ($journal eq 'Cell' || $journal eq 'Neuron' || $journal eq 'Curr Biol' || $journal eq 'Dev Cell' || $journal eq 'Mol Cell') {
    my (@lines) = split/\n/, $all_figs_page;
...
# WHENEVER THE JOURNAL PAGE DOES NOT HAVE HIGH QUALITY PICTURES USE THE FOLLOWING SUBROUTINE
  elsif ($journal eq 'Biochem Biophys Res Commun' || $journal eq 'Gene' || $journal eq 'J Mol Biol' || $journal eq 'Mech Dev') {
    my (@lines) = split/\n/, $all_figs_page;
# print "ALL $all_figs_page ENDALL\n";
    my %figures; my %captions;
    foreach my $line (@lines) {
...
#       print "FIGCAP $fignum\n\n\n";
      ...
      if ( ($journal =~ m/^BMC/) || ($journal eq "Genome Biol") || ($journal eq "J Biol") || ($journal eq "Neural Dev") ) {
