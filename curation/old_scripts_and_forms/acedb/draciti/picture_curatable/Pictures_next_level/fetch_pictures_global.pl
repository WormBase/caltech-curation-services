...
#         print "DOWNLOADING $image_url IMAGE\n";
  ...
  elsif ($root_infile eq 'BiochemBiophysResCommun/BiochemBiophysResCommun' || $root_infile eq 'Cell/Cell' || $root_infile eq 'Neuron/Neuron' || $root_infile eq 'CurrBiol/CurrBiol' || $root_infile eq 'DevCell/DevCell' || $root_infile eq 'MolCell/MolCell') {
    my (@lines) = split/\n/, $all_figs_page;
...
# WHENEVER THE JOURNAL PAGE DOES NOT HAVE HIGH QUALITY PICTURES USE THE FOLLOWING SUBROUTINE
  elsif ($root_infile eq 'Gene/Gene' || $root_infile eq 'JMolBiol/JMolBiol' || $root_infile eq 'MechDev/MechDev') {
    my (@lines) = split/\n/, $all_figs_page;
# print "ALL $all_figs_page ENDALL\n";
    my %figures; my %captions;
    foreach my $line (@lines) {
