...
# print "ALL $all_figs_page FIGS\n";
  ...
  if ( ($root_infile eq 'Development/Development') || ($root_infile eq 'JBC/JBC') || ($root_infile eq 'Thecompanyofbiologists/Thecompanyofbiologists') || ($root_infile eq 'JCellBiol/JCellBiol') ) {
...
print "ALL $all_figs_page ENDALL\n";
    ...
    foreach my $line (@lines) {
      ...
      if ($line =~ m/openStrippedNS\('(.*?)'.*?Figure.*?(\d+)/i) { 
