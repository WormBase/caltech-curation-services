...
sub getUrlAndFigures {
  ...
  if ($root_infile eq 'Development/Development') {
    ...
    $url = 'http://www.jneurosci.org/content/' . $url_constructor; }
...
# print "ALL $all_figs_page FIGS\n";
  ...
  if ( ($root_infile eq 'Development/Development') || ($root_infile eq 'JBC/JBC') || ($root_infile eq 'PNAS/PNAS') || ($root_infile eq 'Genetics/Genetics') || ($root_infile eq 'Thecompanyofbiologists/Thecompanyofbiologists') || ($root_infile eq 'JCellBiol/JCellBiol'  || ($root_infile eq 'Science/Science' ) ) ) {
