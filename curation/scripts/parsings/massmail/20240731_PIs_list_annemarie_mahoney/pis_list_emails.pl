...
#
# modified for anne marie mahoney to get last, first, email, institutions.  2024 07 31
...
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    $row[0] =~ s///g; $row[0] =~ s/two//g;
    $last{$row[0]} = $row[2]; } }
$result = $dbh->prepare( "SELECT * FROM two_institution ORDER BY joinkey, two_order;" );
$result->execute() or die "Cannot prepare statement: $DBI::errstr\n"; 
while (my @row = $result->fetchrow) {
  if ($row[0]) { 
    $row[0] =~ s///g; $row[0] =~ s/two//g;
    push @{ $inst{$row[0]} }, $row[2]; } }
...
#   push @latest_pi_emails, $lower;
  print qq($last{$twonum}\t$first{$twonum}\t$latest_email\t$inst\n);
