...
#
# send output to file disease_counts and also to files/disease_count.<date>  2023 03 27
...
my $outfile = 'disease_counts';
my $outfile2 = 'files/disease_counts.' . $date;
...
$output .= qq(No. of genes in dis_wbgene $count_gene_entries\n);
foreach my $gene (sort keys %{ $hash{wbgene} }) {
  my (@pgids) = sort keys %{ $hash{wbgene}{$gene} };
  if (scalar @pgids > 1) { $output .= qq($gene has mapping to pgids @pgids\n); }
} # foreach my $gene (sort keys %{ $hash{wbgene} })
my $count_unique_gene_entries = scalar keys %{ $hash{wbgene} };
$output .= qq(No. of unique genes in dis_wbgene $count_unique_gene_entries\n);
...
$output .= qq(count all DO_terms $count_any_doid\n); 
my $count_unique_doid = scalar keys %doid;
$output .= qq(count unique DO_terms $count_unique_doid\n); 
...
$output .= qq(count any paper exp mod $count_any_paperexpmod\n); 
...
$output .= qq(count disease descriptions : $count_descriptions\n);
...
$output .= qq(count omim gene disrel : $count_genedisrel\n);
...
$output .= qq(Count omim diseases connected : $count_omim_connected\n);
...
$output .= qq(count any paper dis rel $count_any_paperdisrel\n); 


my $count_unique_papers_any = scalar keys %unique_papers_any;
$output .= qq(count unique papers disrel or expmod : $count_unique_papers_any\n);

my $count_unique_papers_review = scalar keys %unique_papers_review;
$output .= qq(count unique papers disrel or expmod that are review : $count_unique_papers_review\n);

my $count_unique_papers_nonreview = scalar keys %unique_papers_nonreview;
$output .= qq(count unique papers disrel or expmod that are not review : $count_unique_papers_nonreview\n);

$output .= qq(\nList of unique papers disrel or expmod that are review :\n);
foreach my $paper (sort keys %unique_papers_review) { $output .= "$paper\n"; }

$output .= qq(\nList of unique papers disrel or expmod that are not review :\n);
foreach my $paper (sort keys %unique_papers_nonreview) { $output .= "$paper\n"; }
...
close (OU2) or die "Cannot create $outfile2 : $!";
