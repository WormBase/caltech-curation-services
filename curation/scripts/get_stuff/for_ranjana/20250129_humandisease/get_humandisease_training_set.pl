...
# 2024 10 10
#
# modified for chris for humandisease.  https://agr-jira.atlassian.net/browse/SCRUM-4773
...
my $url = $ENV{THIS_HOST} . 'priv/cgi-bin/curation_status.cgi?action=listCurationStatisticsPapersPage&select_datatypesource=caltech&select_curator=two1823&listDatatype=humandisease&method=allval pos&checkbox_cfp=on&checkbox_afp=on&checkbox_str=on&checkbox_nnc=on&checkbox_svm=on';

my $positiveAref = &getJoinkeysFromUrl($url);
my @positive = @$positiveAref;
# print qq(@positive\n);

my $url = $ENV{THIS_HOST} . 'priv/cgi-bin/curation_status.cgi?action=listCurationStatisticsPapersPage&select_datatypesource=caltech&select_curator=two1823&listDatatype=humandisease&method=allval neg&checkbox_cfp=on&checkbox_afp=on&checkbox_str=on&checkbox_nnc=on&checkbox_svm=on';
