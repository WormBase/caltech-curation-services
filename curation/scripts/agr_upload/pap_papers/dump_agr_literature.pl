...
# dump literature data for alliance

# modeled after ace dumper at /home/postgres/work/citace_upload/papers/dumpPapAce.pl
# using https://github.com/alliance-genome/agr_schemas/blob/master/ingest/resourcesAndReferences/reference.json
# instead of LinkML  2021 04 17
#
# validator wasn't restricting bad file from uploading, now it is, so fixed tags that had name wrong,
# and writing 'Unknown' when required fields don't have data.  2021 07 16
#
# modified to output if a paper has multiple PMIDs or DOIs.  filter duplicate PMIDs or DOIs.  
# dump to file and file.date  2021 10 04
#
# No longer require datePublished  2022 10 19
# 
...
# ./dump_agr_literature.pl 
# upload
# curl -H "Authorization: Bearer <API_KEY>" -X POST "https://fms.alliancegenome.org/api/data/submit" -F "4.1.1_REFERENCE_WB=@agr_wb_literature.json"
...
use lib qw( /usr/lib/scripts/citace_upload/papers/ );
# use lib qw( /home/postgres/work/citace_upload/papers/ );
use get_brief_citation;

my $json = JSON::PP->new->ascii->pretty->allow_nonref;

my $outDir = $ENV{CALTECH_CURATION_FILES_INTERNAL_PATH} . "/pub/agr_upload/pap_papers/";
...
my $outfile1 = $outDir . 'agr_wb_literature.json';
my $outfile2 = $outDir . 'agr_wb_literature.json.' . $today_date;
