...
#
# modified to use AgrTetUtils package, but changing all the scripts to use this format for parameters from subroutines would be a big pain to change.
...
use FindBin;
use lib "$FindBin::RealBin/..";	# add parent directory to @INC
use AgrTetUtils qw( createSource tsToDigits generateCognitoToken generateOktaToken retryCreateTag createTag getSourceId );
...
my $source_id = &getSourceId($baseUrl, $source_evidence_assertion, $source_method, $data_provider, $secondary_data_provider);
unless ($source_id) { 
  ...
  &createSource($baseUrl, $source_json);
}

$source_evidence_assertion = 'ATP:0000035';
$source_method = 'author_first_pass';
$source_id = &getSourceId($baseUrl, $source_evidence_assertion, $source_method, $data_provider, $secondary_data_provider);
unless ($source_id) { 
  my %source_json = %{ dclone (\%source_default) };
  $source_json{source_evidence_assertion}	= $source_evidence_assertion;
  $source_json{source_method}  			= $source_method;
