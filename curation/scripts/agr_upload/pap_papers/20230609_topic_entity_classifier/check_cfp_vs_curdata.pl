...
# compareCurdataToCfp looks at everything that exists in cur_curdata where there's a negative result in cfp to see if something other than 'false positive' is worth searching for.
# compareCfpToCurdata looks at everything in cfp and if it says 'false positive' and there isn't a 'negative' curdata, output that for Kimberly to decide what to do with it.  2023 10 04
...
sub compareCfpToCurdata {	# look at everything in cfp and if it says false positive and there isn't a 'negative' curdata, output that.
  foreach my $datatype (sort keys %cfp) {
    foreach my $joinkey (sort keys %{ $cfp{$datatype} }) {
      next unless $cfp{$datatype}{$joinkey}{data};
      my $cfp = $cfp{$datatype}{$joinkey}{data}; $cfp =~ s/\n/ /g;
      my $cfpwho = $cfp{$datatype}{$joinkey}{curator};
      my $curdata = '';
      my $curwho = '';
      if ($curdata{$datatype}{$joinkey}{data}) {
        $curdata = $curdata{$datatype}{$joinkey}{data}; $curdata =~ s/\n/ /g; }
      if ($curdata{$datatype}{$joinkey}{curator}) {
        $curwho = $curdata{$datatype}{$joinkey}{curator}; }
      if ($cfp =~ m/false positive/i) {
#         print qq($datatype\t$joinkey\t($curwho) $curdata\t($cfpwho) $cfp\n);
        if ($curdata ne 'negative') { print qq(BAD $datatype\t$joinkey\t($curwho) $curdata\t($cfpwho) $cfp\n); }
  } } }
}


sub compareCurdataToCfp {	# look at everything that exist in cur_curdata where there's a negative result in cfp to see if something other than false positive is worth searching for
