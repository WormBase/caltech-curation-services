...
my @wbpapers = qw( 00001084 00004952 00031697 00032245 00032467 00032959 00033036 00033206 00033406 00034728 00035977 00040400 00053203 00054648 00059003 00059712 00060296 00065201 00067387 00067433 00068170 00068172 00068343 );	# same set at topic script  2025 11 14
...
# SELECT * FROM afp_genestudied   not in afpVersion
# try to map against   gin_locus gin_seqname gin_wbgene
# look at  populateAfpTransgene

# &populateAbcXref();	# no longer getting AGRKB, just sending WB:WBPaper to ABC  2025 12 17
&populatePapValid();
&populatePapMerge();
&populateMeetings();
&populateAfpOldAfp();
    ...
    foreach my $joinkey (sort keys %{ $theHash{$datatype} }) {
      ...
      foreach my $gene (sort keys %{ $theHash{$datatype}{$joinkey} }) {
        ...
        foreach my $curator (sort keys %{ $theHash{$datatype}{$joinkey}{$gene} }) {
          ...
          $object{'force_insertion'}            = TRUE;
          $object{'negated'}                    = FALSE;
          $object{'reference_curie'}            = "WB:WBPaper$actual_joinkey";
#           $object{'reference_curie'}            = $wbpToAgr{$actual_joinkey};
          ...
          if ($theHash{$datatype}{$joinkey}{$gene}{$curator}{note}) {
            my $note = join"\n", @{ $theHash{$datatype}{$joinkey}{$gene}{$curator}{note} };
            $object{'note'}                     = $note; }
