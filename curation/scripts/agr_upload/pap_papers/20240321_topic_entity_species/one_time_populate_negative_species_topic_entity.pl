...
# 2024 09 10
#
# This is a copy of the cronjob that populates for all dates, not just the last two weeks, the pap_species data
# under the case that timestamp is 2017-08-01  and the extra cases that don't fit any previous case, as created by
# Kimberly.  This is to backpopulate stage and prod in ABC, since the cronjob only covers two weeks of data.
# 2025 01 23
#
...
# delete species tet for 4 test papers
# DELETE FROM topic_entity_tag WHERE topic = 'ATP:0000123' AND topic_entity_tag_source_id IN ( SELECT topic_entity_tag_source_id FROM topic_entity_tag_source WHERE secondary_data_provider_id = ( SELECT mod_id FROM mod WHERE abbreviation = 'WB' )) AND reference_id IN (SELECT reference_id FROM cross_reference WHERE curie IN ( 'WB:WBPaper00063962', 'WB:WBPaper00064505', 'WB:WBPaper00065288', 'WB:WBPaper00067248' ) ) ;
# SELECT * FROM topic_entity_tag WHERE topic = 'ATP:0000123' AND topic_entity_tag_source_id IN ( SELECT topic_entity_tag_source_id FROM topic_entity_tag_source WHERE secondary_data_provider_id = ( SELECT mod_id FROM mod WHERE abbreviation = 'WB' )) AND reference_id IN (SELECT reference_id FROM cross_reference WHERE curie IN ( 'WB:WBPaper00063962', 'WB:WBPaper00064505', 'WB:WBPaper00065288', 'WB:WBPaper00067248' ) ) ;

# no, don't do this, this deletes a ton of stuff
