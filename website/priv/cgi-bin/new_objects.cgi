...
#
# previously showOaTermNoWBPhenotype only showed when there was a suggested definition.
# rewritten to show regardless.  For Chris and Gary.  2018 07 19
...
#       print "<TR><TD>Confirm</TD><TD>Reject</TD><TD>Comment</TD><TD>Suggested</TD><TD>Suggested Definition</TD><TD>Placeholder</TD><TD>Child Of</TD><TD>Curator</TD><TD>Paper Evidence</TD><TD>Entity</TD><TD>Datatype</TD><TD>PGIDs</TD></TR>\n";
      ...
      $query2 = qq(SELECT * FROM rna_suggested WHERE rna_suggested IS NOT NULL AND rna_suggested !~ 'ACCEPTED' AND rna_suggested !~ 'REJECTED' ORDER BY rna_timestamp;); }
      ...
      $query2 = qq(SELECT * FROM dis_suggested_phenotype WHERE dis_suggested_phenotype IS NOT NULL AND dis_suggested_phenotype !~ 'ACCEPTED' AND dis_suggested_phenotype !~ 'REJECTED' ORDER BY dis_timestamp;); }
      ...
      $query2 = qq(SELECT * FROM dit_suggested_phenotype WHERE dit_suggested_phenotype IS NOT NULL AND dit_suggested_phenotype !~ 'ACCEPTED' AND dit_suggested_phenotype !~ 'REJECTED' ORDER BY dit_timestamp;); }
    else { return; }
...
#   print qq(QUERY2 $query2 QUERY<br>);
  $result = $dbh->prepare( $query2 );
  $result->execute() or die "Cannot prepare statement: $DBI::errstr\n";
  while (my @row = $result->fetchrow) { $filter{$row[1]}{pgids}{$row[0]}++; $filter{$row[1]}{timestamp}{$row[2]}++; }
