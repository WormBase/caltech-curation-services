...
# use Encode qw( from_to is_utf8 );		# conversion to utf8 adds an extra A^
use URI::Escape;
use Sys::Hostname;
...
sub updatePostgresTableField {				# if updating postgres table values, update postgres and return OK if ok
  ...
  $newValue = uri_unescape($newValue);
...
sub updatePostgresByTableJoinkeyNewvalue {
  ...
  my $returnValue = '';
  if ($newValue) {									# remove leading and trailing whitespace
    if ($newValue =~ m/^\s+/) { $newValue =~ s/^\s+//; } 
    if ($newValue =~ m/\s+$/) { $newValue =~ s/\s+$//; } }
...
#       unless (is_utf8($newValue)) { from_to($newValue, "iso-8859-1", "utf8"); } 	# this conversion to utf8 adds an extra A^
    }	# may have non utf8 stuf that needs conversion.
  ...
  if ($returnValue eq 'OK') {				# if data saved to postgres, check for non ascii characters and tell curators
    if ($newValue =~ m/�/) { $returnValue = qq(Saved to postgres, but some non-utf-8 characters should be edited : $newValue); }
# this section was for converting non-ascii characters, but postgres handles utf-8, so fromUrlToPostgres decodes to utf-8, and non-utf-8 characters become �
#       if ($newValue =~ m/([^\x00-\x7F])/g) {
#         my $escaped = $newValue =~ s/(.)(?{ if (ord $1 >127 ){'*'}	# replaced non-ascii with *
#                                             else {$1} })/$^R/gr;
# #         my (@nonascii) = $newValue =~ m/([^\x00-\x7F])/g;	# these values always shows as a ? in a circle in the alert, so are useless
# #         my $nonascii = join'","', @nonascii; $nonascii = '"' . $nonascii . '"';
# #         $returnValue = qq(Saved to postgres, but some non-ascii characters should be edited : $nonascii); 
#         $returnValue = qq(Saved to postgres, but some non-ascii characters should be edited : $escaped); }
    }
    else {
      $returnValue = 'Did not update postgres ' . $returnValue . ' end postgres Error message'; }
  return $returnValue;
} # sub updatePostgresByTableJoinkeyNewvalue
...
sub asyncValidValue {
  ...
  ($var, my $userValue) = &getHtmlVar($query, 'userValue');
  ($var, my $datatype) = &getHtmlVar($query, 'datatype');
  if ($userValue =~ m/&gt;/) { $userValue =~ s/&gt;/>/g; } 	# somewhere along the line, < and > are converted into html 
  if ($userValue =~ m/&lt;/) { $userValue =~ s/&lt;/</g; } 
  my $matches;
...
#       my $queryValue = '"' . $userValue . '"';		# some single ontology/dropdown data ended up in multi without the data being changed to have doublequotes, so this doesn't fetch it
      my $queryValue = $userValue;				# unless the data gets fixed, try to just match it
...
# print qq(TABLE $pgtable VALUE $row[1]\n);
...
sub showEditorText {
  ...
  my $table_to_print = "<td width=\"150\" colspan=\"$colspan\">\n";	# there's some weird auto-sizing of the field where it shrinks to nothing if the td doesn't have a size, so min size is 150
  $table_to_print .= qq(<input id="input_$field" name="input_$field" $disabled size="$input_size" style="font-family:'Lucida Console';">\n);
  $table_to_print .= "</td>\n";
  return $table_to_print;
} # sub showEditorText
...
sub showControls {
  ($var, my $datatype) = &getHtmlVar($query, 'datatype');
  ($var, my $batch_unsafe_flag) = &getHtmlVar($query, 'batch_unsafe_flag');
  unless ($batch_unsafe_flag) { $batch_unsafe_flag = 'false'; }
  print "Content-type: text/html\n\n";
  print "<html>\n<body bgcolor=\"white\">\n";
